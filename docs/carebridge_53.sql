...
CREATE SEQUENCE public.data_permissions_legacy_consent_id_seq INCREMENT BY 1 MINVALUE 1 MAXVALUE 9223372036854775807 START 1 CACHE 1 NO CYCLE;
...
CREATE INDEX idx_account_deletion_requests_scheduled_for ON public.account_deletion_requests USING btree (scheduled_for)
WHERE (
...
CREATE TABLE public.content_items (
    ...
    version_no int4 NULL,
    stage varchar(30) NULL,
    CONSTRAINT content_items_pkey PRIMARY KEY (content_item_id)
);

CREATE INDEX idx_content_items_published_at ON public.content_items USING btree (published_at DESC NULLS LAST)
WHERE (
...
CREATE INDEX idx_content_items_stage_type_approved ON public.content_items USING btree (stage, content_type, status)
WHERE (
        (status)::text = 'APPROVED'::text
    );
...
CREATE TABLE public.emergency_contacts (
    ...
    primary_contact bool DEFAULT true NOT NULL,
...
CREATE TABLE public.specialties (
    ...
    created_at timestamptz DEFAULT now() NOT NULL,
    CONSTRAINT specialties_canonical_code_key UNIQUE (code),
...
CREATE TABLE public.vaccination_schedules (
    ...
    dose_number int2 NOT NULL,
    offset_days int4 NOT NULL,
    description text NULL,
    schedule_version varchar(30) DEFAULT '1'::character varying NOT NULL,
    active_from date NULL,
    active_to date NULL,
    created_at timestamptz DEFAULT now() NOT NULL,
    CONSTRAINT vaccination_schedules_key_uk UNIQUE (
        vaccine_name,
        dose_number,
        schedule_version
    ),
...
CREATE TABLE public.administrative_areas (
    ...
    CONSTRAINT administrative_areas_parent_area_id_fkey FOREIGN KEY (parent_area_id) REFERENCES public.administrative_areas (administrative_area_id)
);
...
CREATE TABLE public.care_item_templates (
    ...
    configuration_hash varchar(128) NULL,
    created_at timestamptz DEFAULT now() NOT NULL,
    updated_at timestamptz DEFAULT now() NOT NULL,
    created_by uuid NULL,
    difficulty_level varchar(30) NULL,
    duration_minutes int2 NULL,
    instruction_content text NULL,
    media_url text NULL,
    safety_warning text NULL,
    supports_posture_analysis bool NULL,
    template_status varchar(20) DEFAULT 'ACTIVE'::character varying NOT NULL,
    configured_by uuid NULL,
    analysis_mode varchar(30) NULL,
    rule_or_model_version varchar(80) NULL,
    confidence_threshold numeric NULL,
    feedback_level varchar(30) NULL,
    content_status varchar(20) DEFAULT 'DRAFT'::character varying NOT NULL,
    is_required bool NULL,
    CONSTRAINT care_item_templates_pkey PRIMARY KEY (template_id),
    CONSTRAINT care_item_templates_type_ck CHECK (
        (
            (entry_type)::text = ANY (
                (
                    ARRAY[
    ...
    CONSTRAINT chk_care_item_templates_posture_confidence_threshold CHECK (
        (
            (
    ...
    CONSTRAINT care_item_templates_parent_template_id_fkey FOREIGN KEY (parent_template_id) REFERENCES public.care_item_templates (template_id)
);

CREATE INDEX care_item_templates_content_status_ix ON public.care_item_templates USING btree (
    entry_type,
    content_status,
    stage,
    display_order
...
CREATE INDEX care_item_templates_exercise_filter_ix ON public.care_item_templates USING btree (
    template_status,
    stage,
    difficulty_level,
    created_at DESC
...
WHERE (
        (entry_type)::text = 'EXERCISE_TEMPLATE'::text
    );

CREATE INDEX care_item_templates_parent_order_ix ON public.care_item_templates USING btree (
    parent_template_id,
    display_order
);

CREATE INDEX care_item_templates_posture_version_ix ON public.care_item_templates USING btree (
    parent_template_id,
    template_status,
    effective_from DESC
)
...
CREATE TABLE public.community_topics (
    ...
    created_by uuid NULL,
    "type" varchar(20) DEFAULT 'TOPIC'::character varying NOT NULL,
    slug varchar(140) NOT NULL,
    parent_id uuid NULL,
    CONSTRAINT community_topics_parent_rule_check_v2 CHECK (
        (
            (
                (
    ...
    CONSTRAINT community_topics_type_check CHECK (
        (
            (type)::text = ANY (
                (
                    ARRAY[
    ...
    CONSTRAINT fk_community_topics_parent FOREIGN KEY (parent_id) REFERENCES public.community_topics (id) ON DELETE RESTRICT
);
...
CREATE TABLE public.content_item_topics (
    ...
    CONSTRAINT content_item_topics_content_item_id_fkey FOREIGN KEY (content_item_id) REFERENCES public.content_items (content_item_id),
    CONSTRAINT content_item_topics_topic_id_fkey FOREIGN KEY (topic_id) REFERENCES public.community_topics (id)
);
...
CREATE TABLE public.data_permissions (
    ...
    updated_at timestamptz(6) NULL,
    permission_series_id uuid NULL,
    version_number int4 NULL,
    supersedes_permission_id uuid NULL,
    revoked_by uuid NULL,
    policy_version varchar(80) NULL,
    consent_evidence_key varchar(255) NULL,
    legacy_consent_id int8 GENERATED BY DEFAULT AS IDENTITY (
        INCREMENT BY 1 MINVALUE 1 MAXVALUE 9223372036854775807 START 1 CACHE 1 NO CYCLE
    ) NOT NULL,
    permission_kind varchar(30) DEFAULT 'DATA_PERMISSION'::character varying NOT NULL,
    recipient varchar(120) NULL,
    scope_text text NULL,
    evidence_key uuid NULL,
    locale varchar(20) NULL,
    CONSTRAINT data_permissions_pkey PRIMARY KEY (permission_id),
    CONSTRAINT data_permissions_supersedes_fk FOREIGN KEY (supersedes_permission_id) REFERENCES public.data_permissions (permission_id)
);

CREATE INDEX data_permissions_consent_owner_ix ON public.data_permissions USING btree (
    owner_user_id,
    granted_at DESC
)
WHERE (
        (permission_kind)::text = 'CONSENT_GRANT'::text
    );

CREATE UNIQUE INDEX data_permissions_legacy_consent_id_uk ON public.data_permissions USING btree (legacy_consent_id)
WHERE (legacy_consent_id IS NOT NULL);
...
CREATE TABLE public.archived_records (
    ...
    archive_reason varchar(255) DEFAULT 'RUNTIME_COMPATIBILITY'::character varying NOT NULL,
    source_schema_version varchar(80) DEFAULT 'PHASE2_WAVE9'::character varying NULL,
    checksum varchar(128) DEFAULT md5(gen_random_uuid ()::text) NOT NULL,
    configured_by uuid NULL,
    channel_type varchar(30) NULL,
    duration_minutes int2 NULL,
    specialty_scope varchar(100) NULL,
    ...
    expert_profile_id uuid NULL,
    price_band_id uuid NULL,
    price_amount numeric NULL,
    cancellation_policy text NULL,
    version_no int4 NULL,
    requester_user_id uuid NULL,
    availability_id uuid NULL,
    expert_price_id uuid NULL,
    shared_summary_id uuid NULL,
    topic varchar(500) NULL,
    scheduled_start timestamptz NULL,
    scheduled_end timestamptz NULL,
    price_snapshot_amount numeric NULL,
    commission_rate_snapshot numeric NULL,
    cancellation_policy_snapshot text NULL,
    price_locked_at timestamptz NULL,
    booking_id uuid NULL,
    communication_room_id varchar(255) NULL,
    started_at timestamptz NULL,
    ended_at timestamptz NULL,
    session_status varchar(30) NULL,
    expert_summary text NULL,
    technical_log_json jsonb NULL,
    conversation_id uuid NULL,
    mother_user_id uuid NULL,
    expert_user_id uuid NULL,
    last_activity_at timestamptz NULL,
    mother_last_read_at timestamptz NULL,
    mother_last_read_message_id uuid NULL,
    expert_last_read_at timestamptz NULL,
    expert_last_read_message_id uuid NULL,
    ...
    CONSTRAINT archived_partner_shape_ck CHECK (
        (
            (
                (legacy_table)::text <> 'partner_organizations'::text
            )
            OR (
                (name IS NOT NULL)
                AND (
                    (organization_type)::text = ANY (
                        (
                            ARRAY[
                ...
                AND (
                    representative_user_id IS NOT NULL
                )
                AND (
                    original_created_at IS NOT NULL
                )
            )
        )
    ),
    CONSTRAINT archived_realtime_call_shape_ck CHECK (
        (
            (
                (legacy_table)::text <> 'conversation_calls'::text
            )
            OR (
    ...
    CONSTRAINT archived_realtime_conversation_shape_ck CHECK (
        (
            (
                (legacy_table)::text <> 'direct_conversations'::text
            )
            OR (
                (mother_user_id IS NOT NULL)
                ...
                AND (
                    ...
                    OR (
                        last_activity_at >= original_created_at
                    )
                )
            )
        )
    ),
    CONSTRAINT archived_realtime_message_shape_ck CHECK (
        (
            (
                (legacy_table)::text <> 'direct_messages'::text
            )
            OR (
                (conversation_id IS NOT NULL)
                ...
                AND (
                    (
                        length(btrim(message_body)) >= 1
                    )
                    AND (
                        length(btrim(message_body)) <= 2000
                    )
                )
                AND (
                    original_created_at IS NOT NULL
...
CREATE INDEX archived_consultation_booking_requester_ix ON public.archived_records USING btree (
    requester_user_id,
    status,
    original_created_at DESC
)
WHERE (
        (legacy_table)::text = 'consultation_bookings'::text
    );

CREATE UNIQUE INDEX archived_partner_email_uk ON public.archived_records USING btree (email)
WHERE (
        (legacy_table)::text = 'partner_organizations'::text
    );

CREATE UNIQUE INDEX archived_partner_representative_uk ON public.archived_records USING btree (representative_user_id)
WHERE (
        (legacy_table)::text = 'partner_organizations'::text
    );

CREATE INDEX archived_realtime_call_timeline_ix ON public.archived_records USING btree (
    conversation_id,
    initiated_at DESC
)
WHERE (
...
CREATE UNIQUE INDEX archived_realtime_conversation_pair_uk ON public.archived_records USING btree (
    mother_user_id,
    expert_user_id
)
WHERE (
...
CREATE UNIQUE INDEX archived_realtime_message_client_uk ON public.archived_records USING btree (
    conversation_id,
    sender_user_id,
    client_message_id
)
...
CREATE INDEX archived_realtime_message_timeline_ix ON public.archived_records USING btree (
    conversation_id,
    original_created_at DESC
)
WHERE (
...
CREATE TABLE public.audit_events (
    ...
    event_origin varchar(40) DEFAULT 'AUDIT_LOG'::character varying NOT NULL,
    CONSTRAINT audit_events_pkey PRIMARY KEY (audit_event_id),
    CONSTRAINT audit_events_security_note_text_ck CHECK (
        (
            (
                (event_category)::text <> 'SECURITY_INVESTIGATION_NOTE'::text
            )
            OR (
                (security_event_id IS NOT NULL)
                AND (
                    length(
                        TRIM(
                            BOTH
...
CREATE INDEX audit_events_origin_time_ix ON public.audit_events USING btree (
    event_origin,
    occurred_at DESC
);

CREATE INDEX audit_events_security_note_ix ON public.audit_events USING btree (
    security_event_id,
    occurred_at
)
WHERE (
...
CREATE TABLE public.auth_challenges (
    ...
    requested_role varchar(40) NULL,
    created_at timestamptz DEFAULT now() NOT NULL,
    ...
    CONSTRAINT auth_challenges_status_ck CHECK (
        (
            (status)::text = ANY (
                (
                    ARRAY[
...
CREATE INDEX auth_challenges_subject_expiry_ix ON public.auth_challenges USING btree (
    subject_identifier,
    challenge_type,
    expires_at
);
...
CREATE TABLE public.auth_sessions (
    ...
    status varchar(30) NOT NULL,
    created_ip_hash varchar(255) NULL,
    user_agent_hash varchar(255) NULL,
    legacy_source varchar(40) NULL,
    legacy_id varchar(100) NULL,
    detected_reuse bool DEFAULT false NOT NULL,
    ...
    CONSTRAINT auth_sessions_status_ck CHECK (
        (
            (status)::text = ANY (
                (
                    ARRAY[
...
CREATE INDEX auth_sessions_user_device_ix ON public.auth_sessions USING btree (
    user_id,
    device_identifier,
    status
);
...
CREATE TABLE public.care_facilities (
    ...
    verification_status varchar(30) DEFAULT 'UNVERIFIED'::character varying NOT NULL,
    created_at timestamptz DEFAULT now() NOT NULL,
    updated_at timestamptz DEFAULT now() NOT NULL,
    facility_level varchar(50) NULL,
    ownership_type varchar(30) NULL,
    province_id varchar(2) NULL,
    district_id varchar(4) NULL,
    external_source_id varchar(150) NULL,
    is_active bool DEFAULT true NOT NULL,
    is_searchable bool DEFAULT true NOT NULL,
    administrative_area_id uuid NULL,
    CONSTRAINT care_facilities_ownership_type_check CHECK (
        (
    ...
    CONSTRAINT care_facilities_searchable_coordinates_check CHECK (
        (
            (is_searchable = false)
            OR (
    ...
    CONSTRAINT care_facilities_source_type_check CHECK (
        (
            (source_type IS NULL)
            OR (
                (source_type)::text = ANY (
                    (
                        ARRAY[
                            ...
                            'TRACKASIA'::character varying,
                            'LEGACY_IMPORT'::character varying
                        ]
                    )::text []
                )
            )
...
CREATE INDEX idx_care_facilities_nearby_eligible ON public.care_facilities USING btree (
    facility_type,
    province_id,
    district_id
)
...
CREATE UNIQUE INDEX uq_care_facilities_external_source ON public.care_facilities USING btree (
    source_type,
    external_source_id
)
WHERE (
...
CREATE TABLE public.care_group_members (
    ...
    invitation_status varchar(20) DEFAULT 'PENDING'::character varying NOT NULL,
    permission_json jsonb NULL,
    joined_at timestamptz NULL,
    created_at timestamptz DEFAULT now() NOT NULL,
    updated_at timestamptz DEFAULT now() NOT NULL,
    invite_token varchar(64) NULL,
    invite_channel varchar(20) NULL,
    invite_expires_at timestamptz NULL,
    invited_phone varchar(20) NULL,
    data_permission_id uuid NULL,
...
CREATE UNIQUE INDEX uq_care_group_members_invite_token ON public.care_group_members USING btree (invite_token)
...
CREATE TABLE public.care_groups (
    ...
    linked_journey_id uuid NULL,
    linked_baby_profile_id uuid NULL,
    care_subject_id uuid NULL,
...
CREATE TABLE public.care_logs (
    ...
    created_at timestamptz DEFAULT now() NOT NULL,
    updated_at timestamptz DEFAULT now() NOT NULL,
    CONSTRAINT care_logs_pkey PRIMARY KEY (care_log_id)
);

CREATE INDEX care_logs_subject_type_time_ix ON public.care_logs USING btree (
    care_subject_id,
    log_type,
    started_at
);
...
CREATE TABLE public.care_tasks (
    ...
    status varchar(30) DEFAULT 'PENDING'::character varying NOT NULL,
    source_reference_type varchar(60) NULL,
    source_reference_id uuid NULL,
    vaccination_record_id uuid NULL,
    created_at timestamptz DEFAULT now() NOT NULL,
    updated_at timestamptz DEFAULT now() NOT NULL,
    journey_id uuid NULL,
    baby_id uuid NULL,
    ...
    fcm_job_id varchar(255) NULL,
    CONSTRAINT care_tasks_pkey1 PRIMARY KEY (task_id),
    CONSTRAINT care_tasks_type_ck CHECK (
        (
            (task_type)::text = ANY (
                (
                    ARRAY[
                        'SCHEDULED_REMINDER'::character varying,
                        'MANUAL_TASK'::character varying
                    ]
                )::text []
            )
        )
    ),
    CONSTRAINT care_tasks_vaccination_ck CHECK (
        (
            (
                (task_type)::text <> 'SCHEDULED_REMINDER'::text
            )
            OR (
                (source_reference_type)::text <> 'VACCINATION'::text
            )
            OR (
...
CREATE INDEX care_tasks_assignee_status_ix ON public.care_tasks USING btree (
    assignee_user_id,
    status,
    scheduled_at
)
...
CREATE INDEX care_tasks_context_ix ON public.care_tasks USING btree (
    owner_user_id,
    journey_id,
    baby_id,
    status,
...
CREATE INDEX care_tasks_owner_status_ix ON public.care_tasks USING btree (
    owner_user_id,
    status,
    scheduled_at
)
...
CREATE TABLE public.community_content (
    ...
    moderation_status varchar(30) DEFAULT 'PENDING'::character varying NOT NULL,
    created_at timestamptz DEFAULT now() NOT NULL,
    updated_at timestamptz DEFAULT now() NOT NULL,
    pregnancy_week int2 NULL,
    baby_age_months int2 NULL,
    like_count int4 DEFAULT 0 NOT NULL,
    answer_count int4 DEFAULT 0 NOT NULL,
    is_expert_labeled bool DEFAULT false NOT NULL,
    is_personal_experience bool DEFAULT false NOT NULL,
    CONSTRAINT community_content_pkey PRIMARY KEY (content_id),
    CONSTRAINT community_content_type_ck CHECK (
        (
...
CREATE TABLE public.community_interactions (
    ...
    topic_id uuid NULL,
    created_at timestamptz DEFAULT now() NOT NULL,
    target_content_type varchar(20) NULL,
    CONSTRAINT community_interactions_one_target_ck CHECK (
        (
            (content_id IS NOT NULL) <> (topic_id IS NOT NULL)
        )
    ),
    CONSTRAINT community_interactions_pkey PRIMARY KEY (interaction_id),
    CONSTRAINT community_interactions_type_ck CHECK (
        (
...
CREATE UNIQUE INDEX community_interactions_content_target_uk ON public.community_interactions USING btree (
    actor_user_id,
    interaction_type,
    content_id
)
...
CREATE UNIQUE INDEX community_interactions_topic_target_uk ON public.community_interactions USING btree (
    actor_user_id,
    interaction_type,
    topic_id
)
...
CREATE TABLE public.content_item_sources (
    ...
    source_publisher varchar(255) NULL,
...
CREATE TABLE public.development_milestones (
    ...
    created_at timestamptz DEFAULT now() NOT NULL,
    updated_at timestamptz DEFAULT now() NOT NULL,
    milestone_status varchar(20) DEFAULT 'ACHIEVED'::character varying NOT NULL,
    record_status varchar(20) DEFAULT 'ACTIVE'::character varying NOT NULL,
    care_subject_id uuid NOT NULL,
    CONSTRAINT development_milestones_pkey PRIMARY KEY (milestone_id)
);

CREATE INDEX development_milestones_subject_ix ON public.development_milestones USING btree (
    care_subject_id,
    milestone_type,
    achieved_date
);
...
CREATE TABLE public.expert_credentials (
    ...
    file_url text NULL,
    review_status varchar(30) DEFAULT 'PENDING'::character varying NOT NULL,
    ...
    file_id uuid NULL,
    user_id uuid NOT NULL,
    CONSTRAINT expert_credentials_pkey PRIMARY KEY (credential_id)
);
...
CREATE INDEX growth_measurements_chart_ix ON public.growth_measurements USING btree (
    care_subject_id,
    measured_date
)
...
CREATE TABLE public.health_context_memories (
    ...
    expires_at timestamptz NULL,
    deleted_at timestamptz NULL,
    mother_profile_id uuid NULL,
    baby_profile_id uuid NULL,
    CONSTRAINT health_context_memories_pkey PRIMARY KEY (memory_id)
);

CREATE INDEX health_context_memories_baby_ix ON public.health_context_memories USING btree (
    baby_profile_id,
    created_at DESC
)
WHERE (deleted_at IS NULL);

CREATE INDEX health_context_memories_mother_ix ON public.health_context_memories USING btree (
    mother_profile_id,
    created_at DESC
)
WHERE (deleted_at IS NULL);

CREATE INDEX health_context_memories_subject_expiry_ix ON public.health_context_memories USING btree (care_subject_id, expires_at)
WHERE (deleted_at IS NULL);
...
CREATE TABLE public.health_observations (
    ...
    care_subject_id uuid NOT NULL,
    device_connection_id uuid NULL,
    subject_type varchar(30) NOT NULL,
    observation_type varchar(60) NOT NULL,
    value_numeric numeric NULL,
    value_secondary numeric NULL,
    unit varchar(40) NULL,
    text_value text NULL,
    severity varchar(30) NULL,
    observed_at timestamptz NOT NULL,
    source_record_id uuid NULL,
    source_type varchar(60) DEFAULT 'SYSTEM'::character varying NOT NULL,
    quality_label varchar(30) NULL,
    ...
    CONSTRAINT health_observations_type_ck CHECK (
        (
            (subject_type)::text = ANY (
                (
                    ARRAY[
...
CREATE INDEX health_observations_device_time_ix ON public.health_observations USING btree (
    device_connection_id,
    observed_at
)
WHERE (
        device_connection_id IS NOT NULL
    );

CREATE INDEX health_observations_severity_ix ON public.health_observations USING btree (severity, observed_at)
WHERE (severity IS NOT NULL);

CREATE INDEX health_observations_subject_chart_ix ON public.health_observations USING btree (
    care_subject_id,
    observation_type,
    observed_at
);
...
CREATE TABLE public.health_records (
    ...
    updated_at timestamptz DEFAULT now() NOT NULL,
    care_subject_id uuid NULL,
    summary_period varchar(30) NULL,
    period_start date NULL,
    summary_json jsonb NULL,
    CONSTRAINT health_records_pkey PRIMARY KEY (health_record_id)
);

CREATE INDEX health_records_summary_filter_ix ON public.health_records USING btree (
    owner_user_id,
    summary_period,
    record_date DESC
)
...
CREATE INDEX knowledge_source_reviews_source_time_ix ON public.knowledge_source_reviews USING btree (
    knowledge_source_id,
    changed_at
);
...
CREATE TABLE public.moderation_cases (
    ...
    updated_at timestamptz DEFAULT now() NOT NULL,
    report_source varchar(20) DEFAULT 'USER'::character varying NOT NULL,
    reverted_at timestamptz NULL,
    reverted_by uuid NULL,
    CONSTRAINT moderation_cases_pkey PRIMARY KEY (moderation_case_id)
);

CREATE INDEX moderation_cases_report_source_ix ON public.moderation_cases USING btree (
    report_source,
    status,
    opened_at DESC
);

CREATE INDEX moderation_cases_target_ix ON public.moderation_cases USING btree (
    target_type,
    target_id,
    status
);
...
CREATE TABLE public.mother_journeys (
    ...
    date_source varchar(30) NULL,
    date_confidence varchar(20) NULL,
    pregnancy_outcome varchar(30) NULL,
    pregnancy_outcome_date date NULL,
    care_subject_id uuid NOT NULL,
    baseline_revision int8 NULL,
    baseline_schema_version varchar(40) NULL,
    baseline_source varchar(30) NULL,
    baseline_lifecycle_goal varchar(40) NULL,
    baseline_locale varchar(20) NULL,
    baseline_time_zone varchar(80) NULL,
    baseline_preferences varchar(300) NULL,
    baseline_submission_id uuid NULL,
    baseline_recorded_at timestamptz NULL,
    CONSTRAINT chk_mother_journeys_date_confidence CHECK (
        (
    ...
    CONSTRAINT chk_mother_journeys_date_source CHECK (
        (
            (date_source IS NULL)
            OR (
                (date_source)::text = ANY (
                    (
                        ARRAY[
                            'SELF_REPORTED'::character varying,
                            'CLINICIAN_CONFIRMED'::character varying,
                            'ULTRASOUND'::character varying,
                            'SYSTEM_DERIVED'::character varying,
    ...
    CONSTRAINT ck_mother_journey_live_birth_date CHECK (
        (
            (
                (pregnancy_outcome)::text <> 'LIVE_BIRTH'::text
            )
            OR (
                pregnancy_outcome_date IS NOT NULL
            )
        )
    ),
    CONSTRAINT ck_mother_journey_pregnancy_outcome CHECK (
        (
                ...
                (pregnancy_outcome)::text = ANY (
                    (
                        ARRAY[
                            ...
                            'UNKNOWN'::character varying,
                            'LIVE_BIRTH'::character varying,
                            'PREGNANCY_LOSS'::character varying,
                            'STILLBIRTH'::character varying
                        ]
                    )::text []
                )
            )
    ...
    CONSTRAINT mother_journeys_subject_uk UNIQUE (care_subject_id)
);
...
CREATE UNIQUE INDEX uq_mother_journeys_one_canonical_active ON public.mother_journeys USING btree (owner_user_id)
WHERE (
        (
            (status)::text = 'ACTIVE'::text
        )
        AND (
            (journey_type)::text = ANY (
                (
                    ARRAY[
                        'PRE_PREGNANCY'::character varying,
...
CREATE TABLE public.nearby_support_interactions (
    interaction_id uuid DEFAULT gen_random_uuid () NOT NULL,
    interaction_type varchar(30) NOT NULL,
    ...
    created_at timestamptz DEFAULT now() NOT NULL,
    updated_at timestamptz DEFAULT now() NOT NULL,
    CONSTRAINT nearby_support_interactions_pkey PRIMARY KEY (interaction_id),
    CONSTRAINT nearby_support_interactions_type_ck CHECK (
        (
            (interaction_type)::text = ANY (
                (
                    ARRAY[
...
CREATE TABLE public.notification_records (
    ...
    status varchar(20) DEFAULT 'SENT'::character varying NOT NULL,
    ...
    channel varchar(30) DEFAULT 'PUSH'::character varying NOT NULL,
    updated_at timestamptz DEFAULT now() NOT NULL,
    CONSTRAINT notification_records_channel_check CHECK (
        (
            (channel)::text = ANY (
                (
                    ARRAY[
    ...
    CONSTRAINT notification_records_status_check CHECK (
        (
            (status)::text = ANY (
                (
                    ARRAY[
    ...
    CONSTRAINT notification_records_type_check CHECK (
        (
            (type)::text = ANY (
                (
                    ARRAY[
...
CREATE INDEX idx_notification_records_user_unread ON public.notification_records USING btree (user_id, is_read)
WHERE (is_read = false);
...
CREATE INDEX preparation_checklist_filter_ix ON public.preparation_checklist_items USING btree (
    owner_user_id,
    mother_journey_id,
    baby_id,
    status,
...
CREATE INDEX preparation_checklist_owner_journey_ix ON public.preparation_checklist_items USING btree (
    owner_user_id,
    mother_journey_id,
    display_order
);

CREATE UNIQUE INDEX uq_preparation_checklist_import_scope ON public.preparation_checklist_items USING btree (
    owner_user_id,
    mother_journey_id,
    baby_id,
    template_entry_id
...
CREATE TABLE public.red_flag_rules (
    ...
    is_active bool DEFAULT true NOT NULL,
    is_system_default bool DEFAULT false NOT NULL,
    created_by uuid NULL,
    updated_by uuid NULL,
    created_at timestamptz DEFAULT now() NOT NULL,
    updated_at timestamptz DEFAULT now() NOT NULL,
    CONSTRAINT chk_red_flag_rules_action CHECK (
        (
            (action)::text = ANY (
                (
                    ARRAY[
    ...
    CONSTRAINT chk_red_flag_rules_severity CHECK (
        (
            (severity)::text = ANY (
                (
                    ARRAY[
...
CREATE TABLE public.safety_configs (
    ...
    user_id uuid NOT NULL,
    fall_detection_enabled bool DEFAULT false NOT NULL,
    sensitivity_level varchar(10) DEFAULT 'MEDIUM'::character varying NOT NULL,
    emergency_auto_alert bool DEFAULT true NOT NULL,
    countdown_seconds int4 DEFAULT 30 NOT NULL,
    sensor_permission_granted bool DEFAULT false NOT NULL,
    sensor_permission_recorded_at timestamptz NULL,
    updated_at timestamptz DEFAULT now() NOT NULL,
    updated_by uuid NULL,
    CONSTRAINT safety_configs_countdown_seconds_check CHECK (
        (
            countdown_seconds = ANY (ARRAY[15, 30, 60])
        )
    ),
    CONSTRAINT safety_configs_pkey PRIMARY KEY (safety_config_id),
    CONSTRAINT safety_configs_sensor_permission_ck CHECK (
        (
            (
                sensor_permission_granted = false
            )
            OR (
                sensor_permission_recorded_at IS NOT NULL
            )
        )
    ),
    CONSTRAINT safety_configs_user_id_key UNIQUE (user_id)
);
...
CREATE TABLE public.safety_events (
    ...
    user_id uuid NOT NULL,
    care_subject_id uuid NULL,
    monitoring_session_id uuid NULL,
    ...
    peak_acceleration numeric NULL,
    angular_velocity numeric NULL,
    inactivity_seconds int4 NULL,
    response_type varchar(30) NULL,
    response_at timestamptz NULL,
    false_positive_reason text NULL,
    status varchar(20) DEFAULT 'DETECTED'::character varying NOT NULL,
    location_snapshot_jsonb jsonb DEFAULT '{}'::jsonb NOT NULL,
    recipient_user_id uuid NULL,
    device_identifier varchar(255) NULL,
    notification_record_id uuid NULL,
    care_facility_id uuid NULL,
    ...
    created_at timestamptz DEFAULT now() NOT NULL,
    updated_at timestamptz DEFAULT now() NOT NULL,
    record_type varchar(30) DEFAULT 'IMU_EVENT'::character varying NOT NULL,
    magnitude numeric(10, 4) NULL,
    user_latitude numeric(10, 8) NULL,
    user_longitude numeric(11, 8) NULL,
    client_detected_at timestamptz NULL,
    resolved_at timestamptz NULL,
    notes text NULL,
    signal_key varchar(200) NULL,
    countdown_deadline_at timestamptz NULL,
    response_reason varchar(500) NULL,
    escalation_started_at timestamptz NULL,
    emergency_session_id uuid NULL,
    created_by_text varchar(50) NULL,
    ...
    expires_at timestamptz NULL,
    consent_status varchar(20) NULL,
    ...
    successful_recipient_count int4 NULL,
    failed_recipient_count int4 NULL,
    recipient_count int4 NULL,
    location_included bool NULL,
    triage_handoff_id uuid NULL,
    risk_level varchar(20) NULL,
    summary text NULL,
    action_status varchar(20) NULL,
    CONSTRAINT safety_events_action_type_ck CHECK (
        (
            (action_type IS NULL)
            OR (
    ...
    CONSTRAINT safety_events_attempt_ck CHECK (
        (
            (attempt_number IS NULL)
            OR (attempt_number >= 0)
        )
    ),
    CONSTRAINT safety_events_idempotency_key_key UNIQUE (idempotency_key),
    CONSTRAINT safety_events_parent_ck CHECK (
        (
            (action_type IS NULL)
            OR (
                (action_type)::text = ANY (
    ...
    CONSTRAINT safety_events_record_type_ck CHECK (
        (
            (record_type)::text = ANY (
                (
                    ARRAY[
                        'IMU_EVENT'::character varying,
                        'EMERGENCY_SESSION'::character varying
                    ]
                )::text []
            )
        )
...
CREATE UNIQUE INDEX safety_events_attempt_event_uk ON public.safety_events USING btree (parent_event_id)
WHERE (
        (action_type)::text = 'ALERT_ATTEMPT'::text
    );

CREATE UNIQUE INDEX safety_events_delivery_token_uk ON public.safety_events USING btree (
    parent_event_id,
    device_token_id
)
WHERE (
        (action_type)::text = 'DELIVERY'::text
    );

CREATE UNIQUE INDEX safety_events_family_alert_uk ON public.safety_events USING btree (parent_event_id)
WHERE (
        (action_type)::text = 'FAMILY_ALERT'::text
    );

CREATE INDEX safety_events_handoff_status_ix ON public.safety_events USING btree (
    action_status,
    created_at DESC
)
WHERE (
        (action_type)::text = 'MAP_HANDOFF'::text
    );

CREATE UNIQUE INDEX safety_events_imu_signal_uk ON public.safety_events USING btree (
    monitoring_session_id,
    signal_key
)
WHERE (
        (
            (record_type)::text = 'IMU_EVENT'::text
        )
        AND (signal_key IS NOT NULL)
    );

CREATE INDEX safety_events_owner_location_ix ON public.safety_events USING btree (
    owner_user_id,
    captured_at DESC
)
WHERE (
        (action_type)::text = 'LOCATION_SNAPSHOT'::text
    );

CREATE INDEX safety_events_parent_ix ON public.safety_events USING btree (parent_event_id);

CREATE INDEX safety_events_pending_countdown_ix ON public.safety_events USING btree (countdown_deadline_at)
WHERE (
        (
            (record_type)::text = 'IMU_EVENT'::text
        )
...
CREATE TABLE public.system_configurations (
    ...
    api_rate_limit int4 NOT NULL,
    connection_timeout_ms int4 NOT NULL,
    max_upload_size_mb int4 NOT NULL,
    administrator_email varchar(254) NOT NULL,
    email_alerts bool DEFAULT true NOT NULL,
    sms_alerts bool DEFAULT true NOT NULL,
    webhook_alerts bool DEFAULT false NOT NULL,
    ai_moderation_enabled bool DEFAULT true NOT NULL,
    maintenance_mode_enabled bool DEFAULT false NOT NULL,
    updated_by uuid NOT NULL,
    row_version int8 DEFAULT 0 NOT NULL,
    created_at timestamptz DEFAULT now() NOT NULL,
    updated_at timestamptz DEFAULT now() NOT NULL,
    CONSTRAINT system_configurations_api_rate_limit_check CHECK (
        (
            (api_rate_limit >= 1)
            AND (api_rate_limit <= 100000)
        )
    ),
    CONSTRAINT system_configurations_connection_timeout_ms_check CHECK (
        (
    ...
    CONSTRAINT system_configurations_max_upload_size_mb_check CHECK (
        (
...
CREATE TABLE public.triage_session_evidence (
    ...
    citation_domain varchar(255) NULL,
    source_version varchar(80) NULL,
    source_snapshot_jsonb jsonb DEFAULT '{}'::jsonb NOT NULL,
    content_hash varchar(128) NOT NULL,
    created_at timestamptz DEFAULT now() NOT NULL,
    CONSTRAINT triage_session_evidence_pkey PRIMARY KEY (evidence_id),
    CONSTRAINT triage_session_evidence_uk UNIQUE (
        triage_session_id,
        evidence_type,
        content_hash
    )
...
CREATE TABLE public.triage_sessions (
    ...
    care_subject_id uuid NULL,
    stage varchar(30) NULL,
    profile_context_id uuid NULL,
    risk_level varchar(20) NULL,
    status varchar(30) DEFAULT 'PENDING'::character varying NOT NULL,
    emergency bool DEFAULT false NOT NULL,
    disclaimer_version varchar(80) NULL,
    ...
    schema_version varchar(30) DEFAULT '1'::character varying NOT NULL,
    ...
    symptom_list jsonb NULL,
    duration_days int4 NULL,
    intensity varchar(20) NULL,
    emergency_flag bool NULL,
    extracted_at timestamptz NULL,
    structured_created_by varchar(50) NULL,
    CONSTRAINT triage_sessions_intensity_ck CHECK (
        (
            (intensity IS NULL)
            OR (
                (intensity)::text = ANY (
...
CREATE UNIQUE INDEX triage_sessions_owner_request_uk ON public.triage_sessions USING btree (user_id, client_request_id)
WHERE (client_request_id IS NOT NULL);

CREATE INDEX triage_sessions_risk_ix ON public.triage_sessions USING btree (
    risk_level,
    emergency,
    created_at
);
...
CREATE TABLE public.users (
    ...
    community_posting_restricted_until timestamptz NULL,
    ...
    interest_stage varchar(30) NULL,
    is_visible bool NULL,
    public_avatar_url varchar(500) NULL,
    region varchar(120) NULL,
    social_identities jsonb DEFAULT '[]'::jsonb NOT NULL,
    professional_title varchar(150) NULL,
    workplace varchar(200) NULL,
    experience_years int2 NULL,
    consultation_scope text NULL,
    verification_status varchar(30) DEFAULT 'PENDING'::character varying NOT NULL,
    verified_at timestamptz NULL,
    verified_by uuid NULL,
    rating_avg numeric NULL,
    specialty_ids _uuid NULL,
    specialty varchar(100) NULL,
    facility_id uuid NULL,
    trust_status varchar(20) DEFAULT 'ACTIVE'::character varying NOT NULL,
                ...
                (role)::text = ANY (
                    (
                        ARRAY[
                            ...
                            'CONTENT_ADMIN'::character varying,
...
CREATE TABLE public.vaccination_records (
    ...
    proof_record_id uuid NULL,
    created_at timestamptz DEFAULT now() NOT NULL,
    updated_at timestamptz DEFAULT now() NOT NULL,
    postpone_reason text NULL,
    care_subject_id uuid NOT NULL,
    vaccination_schedule_id uuid NULL,
...
CREATE INDEX vaccination_records_subject_status_ix ON public.vaccination_records USING btree (
    care_subject_id,
    status,
    scheduled_date
);
...
ALTER TABLE public.archived_records
ADD CONSTRAINT archived_consultation_booking_fk FOREIGN KEY (booking_id) REFERENCES public.archived_records (archive_id);

ALTER TABLE public.archived_records
ADD CONSTRAINT archived_consultation_configured_by_fk FOREIGN KEY (configured_by) REFERENCES public.users (user_id);

ALTER TABLE public.archived_records
ADD CONSTRAINT archived_consultation_expert_price_fk FOREIGN KEY (expert_price_id) REFERENCES public.archived_records (archive_id);

ALTER TABLE public.archived_records
ADD CONSTRAINT archived_consultation_price_band_fk FOREIGN KEY (price_band_id) REFERENCES public.archived_records (archive_id);
...
ALTER TABLE public.archived_records
ADD CONSTRAINT archived_realtime_conversation_fk FOREIGN KEY (conversation_id) REFERENCES public.archived_records (archive_id);
...
ALTER TABLE public.archived_records
ADD CONSTRAINT archived_records_owner_user_id_fkey FOREIGN KEY (owner_user_id) REFERENCES public.users (user_id);

-- public.attachments foreign keys

ALTER TABLE public.attachments
ADD CONSTRAINT attachments_health_record_id_fkey FOREIGN KEY (health_record_id) REFERENCES public.health_records (health_record_id);
...
ALTER TABLE public.audit_events
ADD CONSTRAINT audit_events_reviewed_by_fkey FOREIGN KEY (reviewed_by) REFERENCES public.users (user_id);

ALTER TABLE public.audit_events
ADD CONSTRAINT audit_events_security_event_fk FOREIGN KEY (security_event_id) REFERENCES public.audit_events (audit_event_id);
...
ADD CONSTRAINT care_facilities_partner_archive_fk FOREIGN KEY (partner_id) REFERENCES public.archived_records (archive_id);
...
ALTER TABLE public.care_subjects
ADD CONSTRAINT care_subjects_person_id_fkey FOREIGN KEY (person_id) REFERENCES public.users (user_id);

-- public.care_tasks foreign keys

ALTER TABLE public.care_tasks
ADD CONSTRAINT care_tasks_assignee_user_id_fkey FOREIGN KEY (assignee_user_id) REFERENCES public.users (user_id);

ALTER TABLE public.care_tasks
ADD CONSTRAINT care_tasks_care_group_id_fkey1 FOREIGN KEY (care_group_id) REFERENCES public.care_groups (care_group_id);

ALTER TABLE public.care_tasks
ADD CONSTRAINT care_tasks_care_subject_id_fkey FOREIGN KEY (care_subject_id) REFERENCES public.care_subjects (care_subject_id);

ALTER TABLE public.care_tasks
ADD CONSTRAINT care_tasks_creator_user_id_fkey FOREIGN KEY (creator_user_id) REFERENCES public.users (user_id);

ALTER TABLE public.care_tasks
ADD CONSTRAINT care_tasks_owner_user_id_fkey FOREIGN KEY (owner_user_id) REFERENCES public.users (user_id);

ALTER TABLE public.care_tasks
ADD CONSTRAINT care_tasks_vaccination_record_id_fkey FOREIGN KEY (vaccination_record_id) REFERENCES public.vaccination_records (vaccination_record_id);
...
ALTER TABLE public.health_observations
ADD CONSTRAINT health_observations_device_fk FOREIGN KEY (device_connection_id) REFERENCES public.device_connections (device_connection_id);

ALTER TABLE public.health_observations
ADD CONSTRAINT health_observations_session_fk FOREIGN KEY (source_record_id) REFERENCES public.maternal_exercise_sessions (exercise_session_id);
...
ALTER TABLE public.maternal_exercise_sessions
ADD CONSTRAINT maternal_exercise_sessions_safety_fk FOREIGN KEY (safety_observation_id) REFERENCES public.health_observations (health_observation_id);
...
ALTER TABLE public.nearby_support_interactions
ADD CONSTRAINT nearby_support_interactions_care_subject_id_fkey FOREIGN KEY (care_subject_id) REFERENCES public.care_subjects (care_subject_id);

ALTER TABLE public.nearby_support_interactions
ADD CONSTRAINT nearby_support_interactions_parent_interaction_id_fkey FOREIGN KEY (parent_interaction_id) REFERENCES public.nearby_support_interactions (interaction_id);

ALTER TABLE public.nearby_support_interactions
ADD CONSTRAINT nearby_support_interactions_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users (user_id);
...
ADD CONSTRAINT safety_events_care_facility_id_fkey FOREIGN KEY (care_facility_id) REFERENCES public.care_facilities (facility_id);
...
ALTER TABLE public.safety_events
ADD CONSTRAINT safety_events_device_token_fk FOREIGN KEY (device_token_id) REFERENCES public.device_tokens (id);
...
ALTER TABLE public.safety_events
ADD CONSTRAINT safety_events_notification_record_id_fkey FOREIGN KEY (notification_record_id) REFERENCES public.notification_records (id);

ALTER TABLE public.safety_events
ADD CONSTRAINT safety_events_owner_fk FOREIGN KEY (owner_user_id) REFERENCES public.users (user_id);

ALTER TABLE public.safety_events
ADD CONSTRAINT safety_events_owner_user_id_fkey FOREIGN KEY (owner_user_id) REFERENCES public.users (user_id);

ALTER TABLE public.safety_events
ADD CONSTRAINT safety_events_parent_event_id_fkey FOREIGN KEY (parent_event_id) REFERENCES public.safety_events (safety_event_id);

ALTER TABLE public.safety_events
ADD CONSTRAINT safety_events_recipient_user_id_fkey FOREIGN KEY (recipient_user_id) REFERENCES public.users (user_id);
...
ADD CONSTRAINT users_facility_id_fkey FOREIGN KEY (facility_id) REFERENCES public.care_facilities (facility_id);
