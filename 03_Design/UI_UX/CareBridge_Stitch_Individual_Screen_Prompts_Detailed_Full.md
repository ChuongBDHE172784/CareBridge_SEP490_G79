...
### Prompt 142 — `CB-142` — Security Events

**Platform:** Admin Web Portal  
**Primary role:** System Admin  
**Functional coverage:** UC-175
...
Reference screen name: “Security Events”
...
Required components and fields:
1. Security-events table with severity, event type, actor/source, target, timestamp, status, correlation ID, and assigned responder.
2. Filters severity/status/type/date and search; row opens secure event detail.
3. Never expose secret tokens, full IPs, or private medical data.
...
### Prompt 143 — `CB-143` — Security Event Detail
...
Primary role: System Admin
Feature: Audit & security
Reference screen name: “Security Event Detail”
...
Required components and fields:
1. Security-event detail with event summary, severity, affected target, sanitized evidence, related events, timeline, and current disposition.
2. Actions escalate/investigate/resolve depending on permissions; every sensitive action needs note/audit.
3. No log editing/deletion.
...
### Prompt 144 — `CB-144` — Security Incident Investigation

**Platform:** Admin Web Portal  
**Primary role:** System Admin  
**Functional coverage:** UC-174
...
Primary role: System Admin
Feature: Audit & security
Reference screen name: “Security Incident Investigation”
...
Required components and fields:
1. Incident investigation workspace with case header, scope, owner, evidence timeline, related events/accounts, hypotheses/actions, and containment status.
2. Assignment, note, escalation, and evidence-link actions; no leakage of sensitive user health data.
3. Use operational, not medical, language.
...
### Prompt 145 — `CB-145` — Security Incident Resolution
...
Primary role: System Admin
Feature: Audit & security
Reference screen name: “Security Incident Resolution”
...
Required components and fields:
1. Incident resolution form with root-cause category, resolution summary, affected scope, remediation tasks, notification decision, and close status.
2. Require completion summary and confirmation before closing; preserve timeline/audit trail.
3. No ability to silently erase the case.
...
### Prompt 151 — `CB-151` — Security Incident List
...
Primary role: System Admin
Feature: Audit & security
Reference screen name: “Security Incident List”
...
Required components and fields:
1. Security-incident list with case ID, severity, type, status, owner, opened time, affected scope, and SLA.
2. Filters/search; opens investigation or resolution based on state.
3. No raw sensitive evidence shown in table.
