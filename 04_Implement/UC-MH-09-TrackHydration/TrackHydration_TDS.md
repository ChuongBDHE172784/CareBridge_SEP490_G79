...
# TECHNICAL DESIGN SPECIFICATION — Track Hydration

| Field | Value |
| --- | --- |
| Document ID | `UC-MH-09-TDS` |
| Version | `0.1` |
| Date | `2026-08-23` |
| Status | `Draft` |
| Function ID | `UC-MH-09` |
| Canonical Use Case | `UC-MH-09 — Track Hydration` |
...
| Reviewer / Approver |  |
| Source Baseline | Current worktree on `2026-08-23`; SRS `UC-MH-09`; exact evidence in Section 1.4 |
...
- **Goal:** Record hydration intake through the dedicated tracker and persist the corresponding journey metric.
- **Trigger:** The actor enters Mobile metric add route with `metricType=HYDRATION`.
- **Outcome:** Persist and refresh hydration progress.
...
- Mobile metric add route with `metricType=HYDRATION`

- GET/POST/PUT `/api/v1/journeys/{journeyId}/metrics/**`

**Out of scope / limitations**

- Open / current limitation: No focused hydration-screen test was found.
...
| Postcondition | Persist and refresh hydration progress. |
...
| --- | --- | --- | --- | --- |
| `SRC-SRS-01` | Requirement | `02_Requirements/SRS/Report3_Functional_Specifications.md` — UC-MH-09 | 2026-08-23 | Draft code-first requirement |
...
| --- | --- | --- | --- | --- |
| `UC-MH-09-FR-01` | Open hydration tracking for an eligible journey. | `02_Requirements/SRS/Report3_Functional_Specifications.md` — UC-MH-09 Normal Flow 1 | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/health/controller/JourneyMetricController.java` | `COND-01` / `UC-MH-09-TC-001` |
| `UC-MH-09-FR-02` | Add or adjust the supported intake amount. | `02_Requirements/SRS/Report3_Functional_Specifications.md` — UC-MH-09 Normal Flow 2 | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/health/controller/JourneyMetricController.java` | `COND-02` / `UC-MH-09-TC-002` |
| `UC-MH-09-FR-03` | Persist and refresh hydration progress. | `02_Requirements/SRS/Report3_Functional_Specifications.md` — UC-MH-09 Normal Flow 3 | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/health/controller/JourneyMetricController.java` | `COND-03` / `UC-MH-09-TC-003` |
| `BR-01` | Amount/unit/range validation is server authoritative. | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/health/controller/JourneyMetricController.java` | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/health/controller/JourneyMetricController.java` | `COND-BR-01` / `UC-MH-09-TC-BR-001` |
| `BR-02` | Hydration tracking does not diagnose dehydration. | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/health/controller/JourneyMetricController.java` | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/health/controller/JourneyMetricController.java` | `COND-BR-02` / `UC-MH-09-TC-BR-002` |

## 3. Architecture Decision Records (ADR)

### ADR-UC-MH-09-01 — Use a distinct actor-goal boundary

| Item | Decision |
| --- | --- |
| Context | The retired 43-UC catalogue grouped multiple triggers, lifecycles, and permission boundaries, making implementation/test traceability generic. |
| Options | Keep the broad catalogue; split by screen; split by actor goal plus lifecycle/security boundary. |
| Decision | Use `UC-MH-09 — Track Hydration` as the canonical boundary because its operations share the stated actor outcome and current implementation evidence. |
| Consequences | Related supporting screens/endpoints stay in one TDS; different lifecycle/actor outcomes have separate UCs. |
| Source / Status | SRS Section 3.1 and current code audit / Draft |

### ADR-UC-MH-09-02 — Preserve unknowns as Open
...
| --- | --- | --- |
| `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/health/controller/JourneyMetricController.java` | Reuse | Current implementation evidence for Track Hydration; inspect the exact symbol before implementation changes. |
| `05_Development/CareBridgeMobileApp/lib/features/healthRecords/screens/hydration_tracker_screen.dart` | Reuse | Current implementation evidence for Track Hydration; inspect the exact symbol before implementation changes. |
...
class hydration_tracker_screen as "hydration_tracker_screen.dart"
JourneyMetricController --> hydration_tracker_screen
@enduml
```
...
participant Client
participant Domain
Actor -> Client: Enter Track Hydration
Client -> Domain: Open hydration tracking for an eligible journey.
Domain --> Client: Result for step 1
Client -> Domain: Add or adjust the supported intake amount.
Domain --> Client: Result for step 2
Client -> Domain: Persist and refresh hydration progress.
...
| --- | --- | --- |
| Cancel before mutation | No unintended write or provider side effect. | SRS UC-MH-09 Alternative Flow |
...
@startuml
[*] --> Eligible
Eligible --> InProgress : Open hydration tracking for an eligible journey.
InProgress --> Outcome : Persist and refresh hydration progress.
...
- Amount/unit/range validation is server authoritative.
- Hydration tracking does not diagnose dehydration.
...
| ---: | --- | --- | --- |
| 1 | Mobile metric add route with `metricType=HYDRATION` | Mother | Reachable current entry point |
...
| `05_Development/CareBridgeMobileApp/lib/features/healthRecords/screens/hydration_tracker_screen.dart` | Support the mapped operations without broadening authorization or lifecycle semantics. |
...
| Positive test mapping | `COND-API-001` / `UC-MH-09-TC-API-001` |
...
| Positive test mapping | `COND-API-002` / `UC-MH-09-TC-API-002` |
| Negative test mapping | `COND-API-002-VAL` / `UC-MH-09-TC-API-002-VAL`; plus `COND-AUTH` for protected access |
...
| Explicit/documented statuses | `200` |
| Positive test mapping | `COND-API-003` / `UC-MH-09-TC-API-003` |
| Negative test mapping | `COND-AUTH`; DTO/service-only negative paths require their exact cited oracle |
...
| Response payload fields | `metricId`: `UUID` (no field annotation in current DTO); `journeyId`: `UUID` (no field annotation in current DTO); `metricType`: `String` (no field annotation in current DTO); `valueNumeric`: `BigDecimal` (no field annotation in current DTO); `valueSecondary`: `BigDecimal` (no field annotation in current DTO); `unit`: `String` (no field annotation in current DTO); `measuredAt`: `Instant` (no field annotation in current DTO); `sourceType`: `String` (no field annotation in current DTO); `note`: `String` (no field annotation in current DTO); `createdAt`: `Instant` (no field annotation in current DTO); `updatedAt`: `Instant` (no field annotation in current DTO); `context`: `Map<String, Object>` (no field annotation in current DTO); `periodStart`: `Instant` (no field annotation in current DTO); `periodEnd`: `Instant` (no field annotation in current DTO); `qualityLabel`: `String` (no field annotation in current DTO); `disclaimer`: `String` (no field annotation in current DTO); `definitionVersion`: `Integer` (no field annotation in current DTO); `aiInsight`: `String` (no field annotation in current DTO); `redFlagAlert`: `boolean` (no field annotation in current DTO) |
| Explicit/documented statuses | `200` |
| Positive test mapping | `COND-API-004` / `UC-MH-09-TC-API-004` |
| Negative test mapping | `COND-API-004-VAL` / `UC-MH-09-TC-API-004-VAL`; plus `COND-AUTH` for protected access |
...
| --- | --- | --- | --- |
| `VG-01` | Open hydration tracking for an eligible journey. | `COND-01` | `UC-MH-09-TC-001` |
| `VG-02` | Add or adjust the supported intake amount. | `COND-02` | `UC-MH-09-TC-002` |
| `VG-03` | Persist and refresh hydration progress. | `COND-03` | `UC-MH-09-TC-003` |
| `VG-AUTH` | Reject wrong authentication/role/ownership/membership/consent scope | `COND-AUTH` | `UC-MH-09-TC-SEC-001` |
| `VG-GAP` | Characterize each known gap without claiming a false completed path | `COND-GAP` | `UC-MH-09-TC-GAP-001` |
