...
# TEST SPECIFICATION — Manage Professional Profile

| Field | Value |
| --- | --- |
| Document ID | `UC-EX-05-TEST-SPEC` |
| Version | `0.1` |
| Date | `2026-08-23` |
| Status | `Draft` |
| Feature / Gap ID | `UC-EX-05` |
| Module | `Expert and Consultation` |
| Paired TDS | `UC-EX-05-TDS` |
| Priority | `Medium` |
...
| Reviewer / Approver |  |
| Source Baseline | Current worktree `2026-08-23`; SRS/TDS `UC-EX-05` and exact code/test sources below |
...
| --- | --- | --- |
| Actor goal | View and update supported professional-profile fields owned by the authenticated expert. | SRS `UC-EX-05` |
| Current state | `Medium` confidence; gaps are listed in Section 2 | Exact current code/test sources below |
| Entry points | Mobile `/expert/profile`; Web `/expert/profile` | Current client/router evidence |
| Authorization boundary | `Verified Expert` plus exact authentication/role/ownership/membership/consent policy | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/expert/controller/ExpertProfileController.java` |
| Primary operations | Load the authenticated expert profile.; Edit supported professional fields.; Persist and reload the server-authoritative profile. | SRS `UC-EX-05` Normal Flow |
...
| --- | --- | --- | --- | --- |
| `SRC-SRS` | Requirement | `02_Requirements/SRS/Report3_Functional_Specifications.md` — UC-EX-05 | 2026-08-23 | Draft code-first requirement |
| `SRC-TDS` | Design | Paired `UC-EX-05-TDS` | 0.1 | Draft design |
...
| --- | --- | --- | --- | --- | --- |
| `LI-01` | Mobile avatar update currently calls nonexistent `PATCH /api/v1/users/me/profile`; avatar editing is Partial. | Could create false completed coverage or wrong contract | Keep as current limitation/Open until the cited client/server mismatch is resolved | SRS `UC-EX-05` gap plus current code | Open |
...
| --- | --- | --- | --- | --- | --- | --- |
| `RISK-01` | Failure of: Load the authenticated expert profile. | High | Medium | High | Unit/Integration/Contract/applicable UI | `COND-01` |
| `RISK-02` | Failure of: Edit supported professional fields. | High | Medium | High | Unit/Integration/Contract/applicable UI | `COND-02` |
| `RISK-03` | Failure of: Persist and reload the server-authoritative profile. | High | Medium | High | Unit/Integration/Contract/applicable UI | `COND-03` |
...
| --- | --- | --- | --- | --- |
| `BASIS-01` | `UC-EX-05-FR-01` — Load the authenticated expert profile. | SRS `UC-EX-05` Normal Flow 1; TDS Section 2 | Load the authenticated expert profile. | `COND-01` |
| `BASIS-02` | `UC-EX-05-FR-02` — Edit supported professional fields. | SRS `UC-EX-05` Normal Flow 2; TDS Section 2 | Edit supported professional fields. | `COND-02` |
| `BASIS-03` | `UC-EX-05-FR-03` — Persist and reload the server-authoritative profile. | SRS `UC-EX-05` Normal Flow 3; TDS Section 2 | Persist and reload the server-authoritative profile. | `COND-03` |
...
| --- | --- | --- | --- | --- | --- |
| `COND-01` | `BASIS-01` / `RISK-01` | Load the authenticated expert profile. | Mobile / Web / Backend | Positive + applicable boundary/state coverage | `UC-EX-05-TC-001` |
| `COND-02` | `BASIS-02` / `RISK-02` | Edit supported professional fields. | Mobile / Web / Backend | Positive + applicable boundary/state coverage | `UC-EX-05-TC-002` |
| `COND-03` | `BASIS-03` / `RISK-03` | Persist and reload the server-authoritative profile. | Mobile / Web / Backend | Positive + applicable boundary/state coverage | `UC-EX-05-TC-003` |
| `COND-API-001` | Exact handler/client composition contract | GET /api/v1/expert/profiles/me → getMyProfile contract | Applicable backend/client contract layer | Contract + DTO/status/authorization evidence | `UC-EX-05-TC-API-001` |
| `COND-API-002` | Exact handler/client composition contract | PATCH /api/v1/expert/profiles/me → updateProfile contract | Applicable backend/client contract layer | Contract + DTO/status/authorization evidence | `UC-EX-05-TC-API-002` |
| `COND-API-002-VAL` | Exact handler/client composition contract | PATCH /api/v1/expert/profiles/me rejects a declared request-field boundary | Applicable backend/client contract layer | Contract + DTO/status/authorization evidence | `UC-EX-05-TC-API-002-VAL` |
| `COND-BR-01` | `BR-01` | Only the owning expert may update mutable professional fields. | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/expert/controller/ExpertProfileController.java` | Negative / decision / state | `UC-EX-05-TC-BR-001` |
| `COND-BR-02` | `BR-02` | Verification/trust fields cannot be self-escalated. | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/expert/controller/ExpertProfileController.java` | Negative / decision / state | `UC-EX-05-TC-BR-002` |
| `COND-AUTH` | `RISK-AUTH` | Reject wrong authentication, role, ownership, membership, consent, or state scope | All protected layers | Security | `UC-EX-05-TC-SEC-001` |
| `COND-GAP` | `RISK-GAP` | Characterize current limitation/reachability without false completion | Applicable layer | Gap/Regression | `UC-EX-05-TC-GAP-001` |
...
### UC-EX-05-TC-001 — Load the authenticated expert profile

| Field | Specification |
| --- | --- |
| Stable ID | `UC-EX-05-TC-001` |
| Severity | `High` |
...
| Oracle Source | `SRS UC-EX-05 Normal Flow 1; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/expert/controller/ExpertProfileController.java` |
...
1. Load the authenticated expert profile.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | The observable outcome is exactly `Load the authenticated expert profile.` The applicable handler, request/response type, status, and authorization evidence are those enumerated in paired TDS Sections 2, 9, 10, and 16; no additional state is claimed. | `SRS UC-EX-05 Normal Flow 1; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/expert/controller/ExpertProfileController.java` |
...
| UI state | Reachable client displays the canonical outcome/error without false success. | SRS `UC-EX-05` entry point and current client evidence |
...
The contract differs from the cited expected result (The observable outcome is exactly `Load the authenticated expert profile.` The applicable handler, request/response type, status, and authorization evidence are those enumerated in paired TDS Sections 2, 9, 10, and 16; no additional state is claimed.), mutates data on a rejected/read path, leaks protected data, or triggers duplicate/unauthorized side effects.
...
### UC-EX-05-TC-002 — Edit supported professional fields

| Field | Specification |
| --- | --- |
| Stable ID | `UC-EX-05-TC-002` |
| Severity | `High` |
| Test Condition | `COND-02` |
| Test Level | `Unit / Integration / Contract / applicable UI` |
| Platform / Layer | `Mobile / Web / Backend` |
| Technique | `Requirement coverage + state/authorization partition as applicable` |
| Oracle Source | `SRS UC-EX-05 Normal Flow 2; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/expert/controller/ExpertProfileController.java` |
| Preconditions | Synthetic `Verified Expert` identity; eligible current resource state; isolated provider/database fixtures |
| Intended Test File | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/expert/controller/ExpertProfileControllerTest.java` |
| Initial Status | `🔴 Not written / existing evidence must be rerun` |
...
1. Edit supported professional fields.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | The observable outcome is exactly `Edit supported professional fields.` The applicable handler, request/response type, status, and authorization evidence are those enumerated in paired TDS Sections 2, 9, 10, and 16; no additional state is claimed. | `SRS UC-EX-05 Normal Flow 2; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/expert/controller/ExpertProfileController.java` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
The contract differs from the cited expected result (The observable outcome is exactly `Edit supported professional fields.` The applicable handler, request/response type, status, and authorization evidence are those enumerated in paired TDS Sections 2, 9, 10, and 16; no additional state is claimed.), mutates data on a rejected/read path, leaks protected data, or triggers duplicate/unauthorized side effects.
...
### UC-EX-05-TC-003 — Persist and reload the server-authoritative profile

| Field | Specification |
| --- | --- |
| Stable ID | `UC-EX-05-TC-003` |
| Severity | `High` |
| Test Condition | `COND-03` |
| Test Level | `Unit / Integration / Contract / applicable UI` |
| Platform / Layer | `Mobile / Web / Backend` |
| Technique | `Requirement coverage + state/authorization partition as applicable` |
| Oracle Source | `SRS UC-EX-05 Normal Flow 3; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/expert/controller/ExpertProfileController.java` |
| Preconditions | Synthetic `Verified Expert` identity; eligible current resource state; isolated provider/database fixtures |
| Intended Test File | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/expert/controller/ExpertProfileControllerTest.java` |
| Initial Status | `🔴 Not written / existing evidence must be rerun` |
...
1. Persist and reload the server-authoritative profile.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | The observable outcome is exactly `Persist and reload the server-authoritative profile.` The applicable handler, request/response type, status, and authorization evidence are those enumerated in paired TDS Sections 2, 9, 10, and 16; no additional state is claimed. | `SRS UC-EX-05 Normal Flow 3; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/expert/controller/ExpertProfileController.java` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
The contract differs from the cited expected result (The observable outcome is exactly `Persist and reload the server-authoritative profile.` The applicable handler, request/response type, status, and authorization evidence are those enumerated in paired TDS Sections 2, 9, 10, and 16; no additional state is claimed.), mutates data on a rejected/read path, leaks protected data, or triggers duplicate/unauthorized side effects.
...
### UC-EX-05-TC-API-001 — GET /api/v1/expert/profiles/me → getMyProfile contract

| Field | Specification |
| --- | --- |
| Stable ID | `UC-EX-05-TC-API-001` |
| Severity | `High` |
...
1. Invoke `GET /api/v1/expert/profiles/me` so `getMyProfile` receives principal `principal`: `Principal`; satisfy `hasRole('EXPERT')` and the extracted `None` constraints.
...
### UC-EX-05-TC-API-002 — PATCH /api/v1/expert/profiles/me → updateProfile contract

| Field | Specification |
| --- | --- |
| Stable ID | `UC-EX-05-TC-API-002` |
| Severity | `High` |
...
### UC-EX-05-TC-API-002-VAL — PATCH /api/v1/expert/profiles/me rejects a declared request-field boundary

| Field | Specification |
| --- | --- |
| Stable ID | `UC-EX-05-TC-API-002-VAL` |
| Severity | `High` |
...
1. Submit `UpdateExpertProfileRequest` with one field violating its cited validator: `specialtyId`: `String` (@Size(max = 80)); `hospitalId`: `String` (@Size(max = 150)); `trackAsiaName`: `String` (@Size(max = 255)); `trackAsiaAddress`: `String` (@Size(max = 500)); `trackAsiaLat`: `Double` (no field annotation in current DTO); `trackAsiaLng`: `Double` (no field annotation in current DTO); `specialty`: `String` (@Size(max = 100)); `professionalTitle`: `String` (@Size(max = 150)); `experienceYears`: `Integer` (@Min(0), @Max(80)); `workplace`: `String` (@Size(max = 200)); `workplaceProvinceId`: `String` (@Size(max = 16, message = "Province id must not exceed 16 characters")); `consultationScope`: `String` (@Size(max = 5000)); `ratingAvg`: `BigDecimal` (no field annotation in current DTO); `consultationFeeVnd`: `Long` (@PositiveOrZero)
...
### UC-EX-05-TC-BR-001 — Enforce business rule 1

| Field | Specification |
| --- | --- |
| Stable ID | `UC-EX-05-TC-BR-001` |
| Severity | `High` |
...
1. Exercise the eligible and ineligible partitions for this rule: Only the owning expert may update mutable professional fields.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | The server enforces: Only the owning expert may update mutable professional fields. No disallowed state or protected data is produced. | `TDS BR-01; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/expert/controller/ExpertProfileController.java` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
The contract differs from the cited expected result (The server enforces: Only the owning expert may update mutable professional fields. No disallowed state or protected data is produced.), mutates data on a rejected/read path, leaks protected data, or triggers duplicate/unauthorized side effects.
...
### UC-EX-05-TC-BR-002 — Enforce business rule 2

| Field | Specification |
| --- | --- |
| Stable ID | `UC-EX-05-TC-BR-002` |
| Severity | `High` |
| Test Condition | `COND-BR-02` |
| Test Level | `Unit / Integration / Contract / applicable UI` |
| Platform / Layer | `Mobile / Web / Backend` |
| Technique | `Requirement coverage + state/authorization partition as applicable` |
| Oracle Source | `TDS BR-02; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/expert/controller/ExpertProfileController.java` |
| Preconditions | Synthetic `Verified Expert` identity; eligible current resource state; isolated provider/database fixtures |
| Intended Test File | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/expert/controller/ExpertProfileControllerTest.java` |
| Initial Status | `🔴 Not written / existing evidence must be rerun` |
...
1. Exercise the eligible and ineligible partitions for this rule: Verification/trust fields cannot be self-escalated.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | The server enforces: Verification/trust fields cannot be self-escalated. No disallowed state or protected data is produced. | `TDS BR-02; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/expert/controller/ExpertProfileController.java` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
The contract differs from the cited expected result (The server enforces: Verification/trust fields cannot be self-escalated. No disallowed state or protected data is produced.), mutates data on a rejected/read path, leaks protected data, or triggers duplicate/unauthorized side effects.
...
### UC-EX-05-TC-SEC-001 — Reject wrong authentication, role, ownership, membership, or consent scope

| Field | Specification |
| --- | --- |
| Stable ID | `UC-EX-05-TC-SEC-001` |
| Severity | `Critical` |
...
### UC-EX-05-TC-GAP-001 — Characterize the current documented limitation

| Field | Specification |
| --- | --- |
| Stable ID | `UC-EX-05-TC-GAP-001` |
| Severity | `High` |
| Test Condition | `COND-GAP` |
| Test Level | `Unit / Integration / Contract / applicable UI` |
| Platform / Layer | `Mobile / Web / Backend` |
| Technique | `Requirement coverage + state/authorization partition as applicable` |
| Oracle Source | `SRS UC-EX-05 Known Gaps / Exclusions; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/expert/controller/ExpertProfileController.java` |
| Preconditions | Synthetic `Verified Expert` identity; eligible current resource state; isolated provider/database fixtures |
| Intended Test File | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/expert/controller/ExpertProfileControllerTest.java` |
| Initial Status | `🔴 Not written / existing evidence must be rerun` |
...
1. Mobile avatar update currently calls nonexistent `PATCH /api/v1/users/me/profile`; avatar editing is Partial.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | The test records the current limitation without inventing a completed path: Mobile avatar update currently calls nonexistent `PATCH /api/v1/users/me/profile`; avatar editing is Partial. | `SRS UC-EX-05 Known Gaps / Exclusions; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/expert/controller/ExpertProfileController.java` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
The contract differs from the cited expected result (The test records the current limitation without inventing a completed path: Mobile avatar update currently calls nonexistent `PATCH /api/v1/users/me/profile`; avatar editing is Partial.), mutates data on a rejected/read path, leaks protected data, or triggers duplicate/unauthorized side effects.
...
| --- | --- | --- |
| Load the authenticated expert profile. | `COND-01` | `UC-EX-05-TC-001` |
| Edit supported professional fields. | `COND-02` | `UC-EX-05-TC-002` |
| Persist and reload the server-authoritative profile. | `COND-03` | `UC-EX-05-TC-003` |
| GET /api/v1/expert/profiles/me → getMyProfile contract | `COND-API-001` | `UC-EX-05-TC-API-001` |
| PATCH /api/v1/expert/profiles/me → updateProfile contract | `COND-API-002` | `UC-EX-05-TC-API-002` |
| PATCH /api/v1/expert/profiles/me rejects a declared request-field boundary | `COND-API-002-VAL` | `UC-EX-05-TC-API-002-VAL` |
| Business-rule partitions | `COND-BR-*` | `UC-EX-05-TC-BR-*` |
| Authentication / authorization / ownership / consent | `COND-AUTH` | `UC-EX-05-TC-SEC-001` |
| Current gap / reachability boundary | `COND-GAP` | `UC-EX-05-TC-GAP-001` |

## 5. Red-Green-Refactor Tracker

| TC ID | Intended file | Red evidence | Green evidence | Refactor verification | Status |
| --- | --- | --- | --- | --- | --- |
| `UC-EX-05-TC-001` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/expert/controller/ExpertProfileControllerTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-EX-05-TC-002` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/expert/controller/ExpertProfileControllerTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-EX-05-TC-003` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/expert/controller/ExpertProfileControllerTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-EX-05-TC-API-001` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/expert/controller/ExpertProfileControllerTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-EX-05-TC-API-002` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/expert/controller/ExpertProfileControllerTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-EX-05-TC-API-002-VAL` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/expert/controller/ExpertProfileControllerTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-EX-05-TC-BR-001` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/expert/controller/ExpertProfileControllerTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-EX-05-TC-BR-002` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/expert/controller/ExpertProfileControllerTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-EX-05-TC-SEC-001` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/expert/controller/ExpertProfileControllerTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-EX-05-TC-GAP-001` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/expert/controller/ExpertProfileControllerTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
...
| Generic test matrix | Case titles/actions reference `Manage Professional Profile` operations and rules. | Pass |
