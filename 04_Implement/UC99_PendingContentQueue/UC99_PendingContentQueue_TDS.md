...
| 2026-07-14 | Codex | Clarification — khôi phục `Ẩn` trong hai tab `PENDING` để moderator loại nội dung trước khi công khai. `Khóa` chỉ được đưa vào tab `Đã xử lý`, chỉ cho QUESTION có trạng thái hiện tại là `APPROVED`; UI kiểm tra trạng thái trước khi mở xác nhận. |
| 2026-07-14 | Codex | Safety hardening — `LOCK` dùng conditional database update `WHERE status = APPROVED`; nếu không cập nhật được thì trả `MOD-031` và không tạo ModerationAction. Bulk update cũng đặt `updated_at = CURRENT_TIMESTAMP`, vì không đi qua JPA `@UpdateTimestamp`. Điều này chặn race giữa kiểm tra UI và xác nhận khóa. |
...
**Ràng buộc UI hiện hành:** Hai tab `Câu hỏi mới` và `Câu trả lời mới` chỉ chứa nội dung `PENDING` chưa hiển thị công khai; moderator có thể `Duyệt`, `Ẩn` (kèm lý do bắt buộc), hoặc `Yêu cầu sửa`. `Khóa` không được hiển thị tại đây. Ở tab `Đã xử lý`, chỉ record QUESTION từng được duyệt mới có nút `Khóa thảo luận`; UI kiểm tra trạng thái trước khi mở xác nhận và backend atomically yêu cầu trạng thái vẫn là `APPROVED`. ANSWER không hỗ trợ `LOCK`.
...
- `PendingContentQueuePage.tsx`: + tab "Đã xử lý" (3rd tab, cạnh "Câu hỏi mới"/"Câu trả lời mới"), hiển thị lịch sử audit gồm loại, preview, hành động, lý do, người xử lý và thời gian. `Ẩn` xuất hiện trong hai tab `PENDING`; `Khóa thảo luận` chỉ được mở từ history cho QUESTION vẫn `APPROVED`.
