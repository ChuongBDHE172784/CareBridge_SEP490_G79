...
# TEST SPECIFICATION — Browse Expert Directory and Public Profile

| Field | Value |
| --- | --- |
| Document ID | `UC-EX-07-TEST-SPEC` |
| Version | `0.1` |
| Date | `2026-08-23` |
| Status | `Draft` |
| Feature / Gap ID | `UC-EX-07` |
| Module | `Expert and Consultation` |
| Paired TDS | `UC-EX-07-TDS` |
| Priority | `Medium` |
...
| Reviewer / Approver |  |
| Source Baseline | Current worktree `2026-08-23`; SRS/TDS `UC-EX-07` and exact code/test sources below |
...
| --- | --- | --- |
| Actor goal | Search directory-eligible experts, inspect a public professional profile and availability, and choose an expert for consultation. | SRS `UC-EX-07` |
| Current state | `High` confidence; gaps are listed in Section 2 | Exact current code/test sources below |
| Entry points | Mobile `/experts`; Mobile `/expert/public/:expertProfileId` | Current client/router evidence |
| Authorization boundary | `Mother / Family` plus exact authentication/role/ownership/membership/consent policy | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/expert/controller/ExpertProfileController.java` |
| Primary operations | Search/filter the eligible expert directory.; Open a public expert profile and current availability.; Continue to the supported consultation-request flow. | SRS `UC-EX-07` Normal Flow |
...
| --- | --- | --- | --- | --- |
| `SRC-SRS` | Requirement | `02_Requirements/SRS/Report3_Functional_Specifications.md` — UC-EX-07 | 2026-08-23 | Draft code-first requirement |
| `SRC-TDS` | Design | Paired `UC-EX-07-TDS` | 0.1 | Draft design |
...
| --- | --- | --- | --- | --- | --- |
| `LI-01` | Broad 43-UC catalogue previously obscured this boundary | Generic TCs could not map to `Browse Expert Directory and Public Profile` | Split as `UC-EX-07` using the audited current code boundary | SRS 3.1 and `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/expert/controller/ExpertProfileController.java` | Resolved in Draft |
...
| --- | --- | --- | --- | --- | --- | --- |
| `RISK-01` | Failure of: Search/filter the eligible expert directory. | High | Medium | High | Unit/Integration/Contract/applicable UI | `COND-01` |
| `RISK-02` | Failure of: Open a public expert profile and current availability. | High | Medium | High | Unit/Integration/Contract/applicable UI | `COND-02` |
| `RISK-03` | Failure of: Continue to the supported consultation-request flow. | High | Medium | High | Unit/Integration/Contract/applicable UI | `COND-03` |
...
| --- | --- | --- | --- | --- |
| `BASIS-01` | `UC-EX-07-FR-01` — Search/filter the eligible expert directory. | SRS `UC-EX-07` Normal Flow 1; TDS Section 2 | Search/filter the eligible expert directory. | `COND-01` |
| `BASIS-02` | `UC-EX-07-FR-02` — Open a public expert profile and current availability. | SRS `UC-EX-07` Normal Flow 2; TDS Section 2 | Open a public expert profile and current availability. | `COND-02` |
| `BASIS-03` | `UC-EX-07-FR-03` — Continue to the supported consultation-request flow. | SRS `UC-EX-07` Normal Flow 3; TDS Section 2 | Continue to the supported consultation-request flow. | `COND-03` |
...
| --- | --- | --- | --- | --- | --- |
| `COND-01` | `BASIS-01` / `RISK-01` | Search/filter the eligible expert directory. | Mobile / Backend | Positive + applicable boundary/state coverage | `UC-EX-07-TC-001` |
| `COND-02` | `BASIS-02` / `RISK-02` | Open a public expert profile and current availability. | Mobile / Backend | Positive + applicable boundary/state coverage | `UC-EX-07-TC-002` |
| `COND-03` | `BASIS-03` / `RISK-03` | Continue to the supported consultation-request flow. | Mobile / Backend | Positive + applicable boundary/state coverage | `UC-EX-07-TC-003` |
| `COND-API-001` | Exact handler/client composition contract | GET /api/v1/expert/availability/{expertProfileId} → getPublicAvailability contract | Applicable backend/client contract layer | Contract + DTO/status/authorization evidence | `UC-EX-07-TC-API-001` |
| `COND-API-002` | Exact handler/client composition contract | GET /api/v1/expert/directory → getDirectory contract | Applicable backend/client contract layer | Contract + DTO/status/authorization evidence | `UC-EX-07-TC-API-002` |
| `COND-API-003` | Exact handler/client composition contract | GET /api/v1/expert/profiles/{expertProfileId} → getPublicProfile contract | Applicable backend/client contract layer | Contract + DTO/status/authorization evidence | `UC-EX-07-TC-API-003` |
| `COND-API-004` | Exact handler/client composition contract | GET /api/v1/master-data/specialties → getSpecialties contract | Applicable backend/client contract layer | Contract + DTO/status/authorization evidence | `UC-EX-07-TC-API-004` |
| `COND-BR-01` | `BR-01` | Only directory-eligible verified experts are returned. | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/expert/controller/ExpertProfileController.java` | Negative / decision / state | `UC-EX-07-TC-BR-001` |
| `COND-BR-02` | `BR-02` | The consumer directory is a Mobile flow; the Web portal is expert/admin oriented. | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/expertavailability/controller/ExpertAvailabilityController.java` | Negative / decision / state | `UC-EX-07-TC-BR-002` |
| `COND-AUTH` | `RISK-AUTH` | Reject wrong authentication, role, ownership, membership, consent, or state scope | All protected layers | Security | `UC-EX-07-TC-SEC-001` |
| `COND-GAP` | `RISK-GAP` | Characterize current limitation/reachability without false completion | Applicable layer | Gap/Regression | `UC-EX-07-TC-GAP-001` |
...
### UC-EX-07-TC-001 — Search/filter the eligible expert directory

| Field | Specification |
| --- | --- |
| Stable ID | `UC-EX-07-TC-001` |
| Severity | `High` |
...
| Oracle Source | `SRS UC-EX-07 Normal Flow 1; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/expertavailability/controller/ExpertAvailabilityController.java` |
| Preconditions | Synthetic `Mother / Family` identity; eligible current resource state; isolated provider/database fixtures |
| Intended Test File | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/expert/ExpertDirectoryEligibilityIntegrationTest.java` |
...
1. Search/filter the eligible expert directory.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | The observable outcome is exactly `Search/filter the eligible expert directory.` The applicable handler, request/response type, status, and authorization evidence are those enumerated in paired TDS Sections 2, 9, 10, and 16; no additional state is claimed. | `SRS UC-EX-07 Normal Flow 1; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/expertavailability/controller/ExpertAvailabilityController.java` |
...
| UI state | Reachable client displays the canonical outcome/error without false success. | SRS `UC-EX-07` entry point and current client evidence |
...
The contract differs from the cited expected result (The observable outcome is exactly `Search/filter the eligible expert directory.` The applicable handler, request/response type, status, and authorization evidence are those enumerated in paired TDS Sections 2, 9, 10, and 16; no additional state is claimed.), mutates data on a rejected/read path, leaks protected data, or triggers duplicate/unauthorized side effects.
...
### UC-EX-07-TC-002 — Open a public expert profile and current availability

| Field | Specification |
| --- | --- |
| Stable ID | `UC-EX-07-TC-002` |
| Severity | `High` |
| Test Condition | `COND-02` |
| Test Level | `Unit / Integration / Contract / applicable UI` |
| Platform / Layer | `Mobile / Backend` |
| Technique | `Requirement coverage + state/authorization partition as applicable` |
| Oracle Source | `SRS UC-EX-07 Normal Flow 2; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/expert/controller/ExpertProfileController.java` |
| Preconditions | Synthetic `Mother / Family` identity; eligible current resource state; isolated provider/database fixtures |
| Intended Test File | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/expert/service/ExpertProfileServiceImplDirectoryTest.java` |
| Initial Status | `🔴 Not written / existing evidence must be rerun` |
...
1. Open a public expert profile and current availability.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | The observable outcome is exactly `Open a public expert profile and current availability.` The applicable handler, request/response type, status, and authorization evidence are those enumerated in paired TDS Sections 2, 9, 10, and 16; no additional state is claimed. | `SRS UC-EX-07 Normal Flow 2; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/expert/controller/ExpertProfileController.java` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
The contract differs from the cited expected result (The observable outcome is exactly `Open a public expert profile and current availability.` The applicable handler, request/response type, status, and authorization evidence are those enumerated in paired TDS Sections 2, 9, 10, and 16; no additional state is claimed.), mutates data on a rejected/read path, leaks protected data, or triggers duplicate/unauthorized side effects.
...
### UC-EX-07-TC-003 — Continue to the supported consultation-request flow

| Field | Specification |
| --- | --- |
| Stable ID | `UC-EX-07-TC-003` |
| Severity | `High` |
| Test Condition | `COND-03` |
| Test Level | `Unit / Integration / Contract / applicable UI` |
| Platform / Layer | `Mobile / Backend` |
| Technique | `Requirement coverage + state/authorization partition as applicable` |
| Oracle Source | `SRS UC-EX-07 Normal Flow 3; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/expert/controller/ExpertProfileController.java` |
| Preconditions | Synthetic `Mother / Family` identity; eligible current resource state; isolated provider/database fixtures |
| Intended Test File | `05_Development/CareBridgeMobileApp/test/features/directChat/expert_directory_screen_test.dart` |
| Initial Status | `🔴 Not written / existing evidence must be rerun` |
...
1. Continue to the supported consultation-request flow.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | The observable outcome is exactly `Continue to the supported consultation-request flow.` The applicable handler, request/response type, status, and authorization evidence are those enumerated in paired TDS Sections 2, 9, 10, and 16; no additional state is claimed. | `SRS UC-EX-07 Normal Flow 3; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/expert/controller/ExpertProfileController.java` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
The contract differs from the cited expected result (The observable outcome is exactly `Continue to the supported consultation-request flow.` The applicable handler, request/response type, status, and authorization evidence are those enumerated in paired TDS Sections 2, 9, 10, and 16; no additional state is claimed.), mutates data on a rejected/read path, leaks protected data, or triggers duplicate/unauthorized side effects.
...
### UC-EX-07-TC-API-001 — GET /api/v1/expert/availability/{expertProfileId} → getPublicAvailability contract

| Field | Specification |
| --- | --- |
| Stable ID | `UC-EX-07-TC-API-001` |
| Severity | `High` |
...
### UC-EX-07-TC-API-002 — GET /api/v1/expert/directory → getDirectory contract

| Field | Specification |
| --- | --- |
| Stable ID | `UC-EX-07-TC-API-002` |
| Severity | `High` |
...
### UC-EX-07-TC-API-003 — GET /api/v1/expert/profiles/{expertProfileId} → getPublicProfile contract

| Field | Specification |
| --- | --- |
| Stable ID | `UC-EX-07-TC-API-003` |
| Severity | `High` |
...
1. Invoke `GET /api/v1/expert/profiles/{expertProfileId}` so `getPublicProfile` receives path `expertProfileId`: `UUID`; satisfy `isAuthenticated()` and the extracted `None` constraints.
...
### UC-EX-07-TC-API-004 — GET /api/v1/master-data/specialties → getSpecialties contract

| Field | Specification |
| --- | --- |
| Stable ID | `UC-EX-07-TC-API-004` |
| Severity | `High` |
...
1. Invoke `GET /api/v1/master-data/specialties` so `getSpecialties` receives No explicit handler parameter; satisfy `No @PreAuthorize on handler/class; effective access comes from the security chain` and the extracted `None` constraints.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | Success status `200` returns `ResponseEntity<ApiResponse<List<SpecialtyResponse>>>` with payload fields `specialtyId`: `String` (no field annotation in current DTO); `name`: `String` (no field annotation in current DTO); `description`: `String` (no field annotation in current DTO); `category`: `String` (no field annotation in current DTO); the request body is `None` with Not applicable — no request body. | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/masterdata/controller/MasterDataController.java` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
### UC-EX-07-TC-BR-001 — Enforce business rule 1

| Field | Specification |
| --- | --- |
| Stable ID | `UC-EX-07-TC-BR-001` |
| Severity | `High` |
...
1. Exercise the eligible and ineligible partitions for this rule: Only directory-eligible verified experts are returned.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | The server enforces: Only directory-eligible verified experts are returned. No disallowed state or protected data is produced. | `TDS BR-01; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/expert/controller/ExpertProfileController.java` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
The contract differs from the cited expected result (The server enforces: Only directory-eligible verified experts are returned. No disallowed state or protected data is produced.), mutates data on a rejected/read path, leaks protected data, or triggers duplicate/unauthorized side effects.
...
### UC-EX-07-TC-BR-002 — Enforce business rule 2

| Field | Specification |
| --- | --- |
| Stable ID | `UC-EX-07-TC-BR-002` |
| Severity | `High` |
...
1. Exercise the eligible and ineligible partitions for this rule: The consumer directory is a Mobile flow; the Web portal is expert/admin oriented.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | The server enforces: The consumer directory is a Mobile flow; the Web portal is expert/admin oriented. No disallowed state or protected data is produced. | `TDS BR-02; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/expertavailability/controller/ExpertAvailabilityController.java` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
The contract differs from the cited expected result (The server enforces: The consumer directory is a Mobile flow; the Web portal is expert/admin oriented. No disallowed state or protected data is produced.), mutates data on a rejected/read path, leaks protected data, or triggers duplicate/unauthorized side effects.
...
### UC-EX-07-TC-SEC-001 — Reject wrong authentication, role, ownership, membership, or consent scope

| Field | Specification |
| --- | --- |
| Stable ID | `UC-EX-07-TC-SEC-001` |
| Severity | `Critical` |
...
### UC-EX-07-TC-GAP-001 — Prove the current actor entry path and owning contract remain reachable

| Field | Specification |
| --- | --- |
| Stable ID | `UC-EX-07-TC-GAP-001` |
| Severity | `Medium` |
| Test Condition | `COND-GAP` |
| Test Level | `Unit / Integration / Contract / applicable UI` |
| Platform / Layer | `Mobile / Backend` |
| Technique | `Requirement coverage + state/authorization partition as applicable` |
| Oracle Source | `SRS UC-EX-07 Implemented Entry Points/Contracts; 05_Development/CareBridgeMobileApp/lib/features/directChat/screens/expert_directory_screen.dart` |
| Preconditions | Synthetic `Mother / Family` identity; eligible current resource state; isolated provider/database fixtures |
| Intended Test File | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/expert/ExpertDirectoryEligibilityIntegrationTest.java` |
| Initial Status | `🔴 Not written / existing evidence must be rerun` |
...
1. Start from `Mobile `/experts``, perform `Search/filter the eligible expert directory.`, and observe the owning contract `GET `/api/v1/expert/directory`` rather than a retired or unrelated route.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | The current entry path remains reachable for `Browse Expert Directory and Public Profile` and invokes only the documented owning contract; an unreachable, static, or wrong-method path fails this case. | `SRS UC-EX-07 Implemented Entry Points/Contracts; 05_Development/CareBridgeMobileApp/lib/features/directChat/screens/expert_directory_screen.dart` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
The contract differs from the cited expected result (The current entry path remains reachable for `Browse Expert Directory and Public Profile` and invokes only the documented owning contract; an unreachable, static, or wrong-method path fails this case.), mutates data on a rejected/read path, leaks protected data, or triggers duplicate/unauthorized side effects.
...
| --- | --- | --- |
| Search/filter the eligible expert directory. | `COND-01` | `UC-EX-07-TC-001` |
| Open a public expert profile and current availability. | `COND-02` | `UC-EX-07-TC-002` |
| Continue to the supported consultation-request flow. | `COND-03` | `UC-EX-07-TC-003` |
| GET /api/v1/expert/availability/{expertProfileId} → getPublicAvailability contract | `COND-API-001` | `UC-EX-07-TC-API-001` |
| GET /api/v1/expert/directory → getDirectory contract | `COND-API-002` | `UC-EX-07-TC-API-002` |
| GET /api/v1/expert/profiles/{expertProfileId} → getPublicProfile contract | `COND-API-003` | `UC-EX-07-TC-API-003` |
| GET /api/v1/master-data/specialties → getSpecialties contract | `COND-API-004` | `UC-EX-07-TC-API-004` |
| Business-rule partitions | `COND-BR-*` | `UC-EX-07-TC-BR-*` |
| Authentication / authorization / ownership / consent | `COND-AUTH` | `UC-EX-07-TC-SEC-001` |
| Current gap / reachability boundary | `COND-GAP` | `UC-EX-07-TC-GAP-001` |

## 5. Red-Green-Refactor Tracker

| TC ID | Intended file | Red evidence | Green evidence | Refactor verification | Status |
| --- | --- | --- | --- | --- | --- |
| `UC-EX-07-TC-001` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/expert/ExpertDirectoryEligibilityIntegrationTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-EX-07-TC-002` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/expert/ExpertDirectoryEligibilityIntegrationTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-EX-07-TC-003` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/expert/ExpertDirectoryEligibilityIntegrationTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-EX-07-TC-API-001` | Planned — `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/expertavailability/controller/ExpertAvailabilityControllerTest.java` (not present at Draft baseline) | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-EX-07-TC-API-002` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/expert/controller/ExpertProfileControllerTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-EX-07-TC-API-003` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/expert/controller/ExpertProfileControllerTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-EX-07-TC-API-004` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/expert/service/ExpertProfileServiceImplDirectoryTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-EX-07-TC-BR-001` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/expert/ExpertDirectoryEligibilityIntegrationTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-EX-07-TC-BR-002` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/expert/ExpertDirectoryEligibilityIntegrationTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-EX-07-TC-SEC-001` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/expert/ExpertDirectoryEligibilityIntegrationTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-EX-07-TC-GAP-001` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/expert/ExpertDirectoryEligibilityIntegrationTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
...
| Generic test matrix | Case titles/actions reference `Browse Expert Directory and Public Profile` operations and rules. | Pass |
