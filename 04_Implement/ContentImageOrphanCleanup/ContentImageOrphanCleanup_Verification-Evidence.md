...
**References:**
- `04_Implement/ContentImageOrphanCleanup/ContentImageOrphanCleanup_Architecture-Evidence.md` (bắt buộc đọc §3 ADR trước khi implement)
- `04_Implement/UC169_DeleteFile/UC169_DeleteFile_TDS.md` (ADR-FILE-008, OI-169-3 — gốc rễ của tính năng này)
- `04_Implement/ContentRichTextEditor/ContentRichTextEditor_Architecture-Evidence.md` (ADR-RTE-007 addendum — quyết định giữ ảnh cho content ARCHIVED, lý do job này tồn tại)
...
└── Integration (Testcontainers — CHƯA chạy được, cùng gap môi trường pre-existing đã ghi nhận
    ở ContentRichTextEditor_Verification-Evidence.md §5.1, không lặp lại chi tiết ở đây)
```

### TDS-02 — Test Basis
- `ContentImageOrphanCleanup_Architecture-Evidence.md` §3 (ADR-CLEAN-001, ADR-CLEAN-002), §5 (Static Modeling — chữ ký method chính xác)
...
### Entry Criteria
- [x] `ContentImageOrphanCleanup_Architecture-Evidence.md` §3 (ADR-CLEAN-001, ADR-CLEAN-002) đã `Approved` (Status: `Accepted` cho cả 2 ADR)
