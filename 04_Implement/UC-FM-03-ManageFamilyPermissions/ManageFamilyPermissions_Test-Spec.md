...
# TEST SPECIFICATION — Manage Family Member Permissions

| Field | Value |
| --- | --- |
| Document ID | `UC-FM-03-TEST-SPEC` |
| Version | `0.1` |
| Date | `2026-08-23` |
| Status | `Draft` |
| Feature / Gap ID | `UC-FM-03` |
| Module | `Family Cooperative Care` |
| Paired TDS | `UC-FM-03-TDS` |
| Priority | `Medium` |
...
| Reviewer / Approver |  |
| Source Baseline | Current worktree `2026-08-23`; SRS/TDS `UC-FM-03` and exact code/test sources below |
...
| --- | --- | --- |
| Actor goal | View and update the bounded sharing permissions of an eligible care-group member. | SRS `UC-FM-03` |
| Current state | `High` confidence; gaps are listed in Section 2 | Exact current code/test sources below |
| Entry points | Mobile Manage Family Permission screen | Current client/router evidence |
| Authorization boundary | `Mother / Group Owner` plus exact authentication/role/ownership/membership/consent policy | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/family/controller/CareGroupController.java` |
| Primary operations | Open an eligible member's permission settings.; Change only supported permission flags.; Persist and reload the effective permission set. | SRS `UC-FM-03` Normal Flow |
...
| --- | --- | --- | --- | --- |
| `SRC-SRS` | Requirement | `02_Requirements/SRS/Report3_Functional_Specifications.md` — UC-FM-03 | 2026-08-23 | Draft code-first requirement |
| `SRC-TDS` | Design | Paired `UC-FM-03-TDS` | 0.1 | Draft design |
...
| --- | --- | --- | --- | --- | --- |
| `LI-01` | Broad 43-UC catalogue previously obscured this boundary | Generic TCs could not map to `Manage Family Member Permissions` | Split as `UC-FM-03` using the audited current code boundary | SRS 3.1 and `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/family/controller/CareGroupController.java` | Resolved in Draft |
...
| --- | --- | --- | --- | --- | --- | --- |
| `RISK-01` | Failure of: Open an eligible member's permission settings. | High | Medium | High | Unit/Integration/Contract/applicable UI | `COND-01` |
| `RISK-02` | Failure of: Change only supported permission flags. | High | Medium | High | Unit/Integration/Contract/applicable UI | `COND-02` |
| `RISK-03` | Failure of: Persist and reload the effective permission set. | High | Medium | High | Unit/Integration/Contract/applicable UI | `COND-03` |
...
| --- | --- | --- | --- | --- |
| `BASIS-01` | `UC-FM-03-FR-01` — Open an eligible member's permission settings. | SRS `UC-FM-03` Normal Flow 1; TDS Section 2 | Open an eligible member's permission settings. | `COND-01` |
| `BASIS-02` | `UC-FM-03-FR-02` — Change only supported permission flags. | SRS `UC-FM-03` Normal Flow 2; TDS Section 2 | Change only supported permission flags. | `COND-02` |
| `BASIS-03` | `UC-FM-03-FR-03` — Persist and reload the effective permission set. | SRS `UC-FM-03` Normal Flow 3; TDS Section 2 | Persist and reload the effective permission set. | `COND-03` |
...
| --- | --- | --- | --- | --- | --- |
| `COND-01` | `BASIS-01` / `RISK-01` | Open an eligible member's permission settings. | Mobile / Backend | Positive + applicable boundary/state coverage | `UC-FM-03-TC-001` |
| `COND-02` | `BASIS-02` / `RISK-02` | Change only supported permission flags. | Mobile / Backend | Positive + applicable boundary/state coverage | `UC-FM-03-TC-002` |
| `COND-03` | `BASIS-03` / `RISK-03` | Persist and reload the effective permission set. | Mobile / Backend | Positive + applicable boundary/state coverage | `UC-FM-03-TC-003` |
| `COND-API-001` | Exact handler/client composition contract | GET /api/v1/care-groups/{groupId}/members/{memberId}/permissions → getFamilyPermission contract | Applicable backend/client contract layer | Contract + DTO/status/authorization evidence | `UC-FM-03-TC-API-001` |
| `COND-API-002` | Exact handler/client composition contract | PATCH /api/v1/care-groups/{groupId}/members/{memberId}/permissions → updateFamilyPermission contract | Applicable backend/client contract layer | Contract + DTO/status/authorization evidence | `UC-FM-03-TC-API-002` |
| `COND-BR-01` | `BR-01` | Only the authorized owner/mother may change member permissions. | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/family/controller/CareGroupController.java` | Negative / decision / state | `UC-FM-03-TC-BR-001` |
| `COND-BR-02` | `BR-02` | Revoked permissions must affect subsequent data reads immediately according to policy. | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/family/controller/CareGroupController.java` | Negative / decision / state | `UC-FM-03-TC-BR-002` |
| `COND-AUTH` | `RISK-AUTH` | Reject wrong authentication, role, ownership, membership, consent, or state scope | All protected layers | Security | `UC-FM-03-TC-SEC-001` |
| `COND-GAP` | `RISK-GAP` | Characterize current limitation/reachability without false completion | Applicable layer | Gap/Regression | `UC-FM-03-TC-GAP-001` |
...
| --- | --- | --- | --- |
| Actor | Synthetic `Mother / Group Owner` plus closest wrong-role/cross-owner identities | Authenticated, unauthenticated, wrong scope, consent revoked | Reset principals/tokens |
...
### UC-FM-03-TC-001 — Open an eligible member's permission settings

| Field | Specification |
| --- | --- |
| Stable ID | `UC-FM-03-TC-001` |
| Severity | `High` |
...
| Oracle Source | `SRS UC-FM-03 Normal Flow 1; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/family/controller/CareGroupController.java` |
| Preconditions | Synthetic `Mother / Group Owner` identity; eligible current resource state; isolated provider/database fixtures |
| Intended Test File | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/family/ManageFamilyPermissionIntegrationTest.java` |
...
1. Open an eligible member's permission settings.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | The observable outcome is exactly `Open an eligible member's permission settings.` The applicable handler, request/response type, status, and authorization evidence are those enumerated in paired TDS Sections 2, 9, 10, and 16; no additional state is claimed. | `SRS UC-FM-03 Normal Flow 1; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/family/controller/CareGroupController.java` |
...
| UI state | Reachable client displays the canonical outcome/error without false success. | SRS `UC-FM-03` entry point and current client evidence |
...
### UC-FM-03-TC-002 — Change only supported permission flags

| Field | Specification |
| --- | --- |
| Stable ID | `UC-FM-03-TC-002` |
| Severity | `High` |
| Test Condition | `COND-02` |
| Test Level | `Unit / Integration / Contract / applicable UI` |
| Platform / Layer | `Mobile / Backend` |
| Technique | `Requirement coverage + state/authorization partition as applicable` |
| Oracle Source | `SRS UC-FM-03 Normal Flow 2; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/family/controller/CareGroupController.java` |
| Preconditions | Synthetic `Mother / Group Owner` identity; eligible current resource state; isolated provider/database fixtures |
| Intended Test File | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/family/controller/CareGroupControllerPermissionTest.java` |
| Initial Status | `🔴 Not written / existing evidence must be rerun` |
...
1. Change only supported permission flags.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | The observable outcome is exactly `Change only supported permission flags.` The applicable handler, request/response type, status, and authorization evidence are those enumerated in paired TDS Sections 2, 9, 10, and 16; no additional state is claimed. | `SRS UC-FM-03 Normal Flow 2; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/family/controller/CareGroupController.java` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
The contract differs from the cited expected result (The observable outcome is exactly `Change only supported permission flags.` The applicable handler, request/response type, status, and authorization evidence are those enumerated in paired TDS Sections 2, 9, 10, and 16; no additional state is claimed.), mutates data on a rejected/read path, leaks protected data, or triggers duplicate/unauthorized side effects.
...
### UC-FM-03-TC-003 — Persist and reload the effective permission set

| Field | Specification |
| --- | --- |
| Stable ID | `UC-FM-03-TC-003` |
| Severity | `High` |
| Test Condition | `COND-03` |
| Test Level | `Unit / Integration / Contract / applicable UI` |
| Platform / Layer | `Mobile / Backend` |
| Technique | `Requirement coverage + state/authorization partition as applicable` |
| Oracle Source | `SRS UC-FM-03 Normal Flow 3; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/family/controller/CareGroupController.java` |
| Preconditions | Synthetic `Mother / Group Owner` identity; eligible current resource state; isolated provider/database fixtures |
| Intended Test File | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/family/service/CareGroupServiceImplPermissionTest.java` |
| Initial Status | `🔴 Not written / existing evidence must be rerun` |
...
1. Persist and reload the effective permission set.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | The observable outcome is exactly `Persist and reload the effective permission set.` The applicable handler, request/response type, status, and authorization evidence are those enumerated in paired TDS Sections 2, 9, 10, and 16; no additional state is claimed. | `SRS UC-FM-03 Normal Flow 3; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/family/controller/CareGroupController.java` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
The contract differs from the cited expected result (The observable outcome is exactly `Persist and reload the effective permission set.` The applicable handler, request/response type, status, and authorization evidence are those enumerated in paired TDS Sections 2, 9, 10, and 16; no additional state is claimed.), mutates data on a rejected/read path, leaks protected data, or triggers duplicate/unauthorized side effects.
...
### UC-FM-03-TC-API-001 — GET /api/v1/care-groups/{groupId}/members/{memberId}/permissions → getFamilyPermission contract

| Field | Specification |
| --- | --- |
| Stable ID | `UC-FM-03-TC-API-001` |
| Severity | `High` |
...
### UC-FM-03-TC-API-002 — PATCH /api/v1/care-groups/{groupId}/members/{memberId}/permissions → updateFamilyPermission contract

| Field | Specification |
| --- | --- |
| Stable ID | `UC-FM-03-TC-API-002` |
| Severity | `High` |
...
1. Invoke `PATCH /api/v1/care-groups/{groupId}/members/{memberId}/permissions` so `updateFamilyPermission` receives path `groupId`: `UUID`; path `memberId`: `UUID`; body `request`: `UpdateFamilyPermissionRequest`; principal `principal`: `Principal`; satisfy `hasRole('MOTHER')` and the extracted `UpdateFamilyPermissionRequest` constraints.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | Success status `200` returns `ResponseEntity<ApiResponse<FamilyPermissionResponse>>` with payload fields `memberId`: `UUID` (no field annotation in current DTO); `careGroupId`: `UUID` (no field annotation in current DTO); `calendar`: `boolean` (no field annotation in current DTO); `logs`: `boolean` (no field annotation in current DTO); `alerts`: `boolean` (no field annotation in current DTO); `records`: `boolean` (no field annotation in current DTO); `checklistView`: `boolean` (no field annotation in current DTO); `checklistComplete`: `boolean` (no field annotation in current DTO); `quickNotes`: `boolean` (no field annotation in current DTO); `quickNoteWeight`: `boolean` (no field annotation in current DTO); `quickNoteHydration`: `boolean` (no field annotation in current DTO); `quickNoteEpds`: `boolean` (no field annotation in current DTO); `quickNoteFetalMovement`: `boolean` (no field annotation in current DTO); `quickNoteBloodPressure`: `boolean` (no field annotation in current DTO); `quickNoteBloodGlucose`: `boolean` (no field annotation in current DTO); `updatedAt`: `Instant` (no field annotation in current DTO); the request body is `UpdateFamilyPermissionRequest` with `calendar`: `Boolean` (no field annotation in current DTO); `logs`: `Boolean` (no field annotation in current DTO); `alerts`: `Boolean` (no field annotation in current DTO); `records`: `Boolean` (no field annotation in current DTO); `checklistView`: `Boolean` (no field annotation in current DTO); `checklistComplete`: `Boolean` (no field annotation in current DTO); `quickNotes`: `Boolean` (no field annotation in current DTO); `quickNoteWeight`: `Boolean` (no field annotation in current DTO); `quickNoteHydration`: `Boolean` (no field annotation in current DTO); `quickNoteEpds`: `Boolean` (no field annotation in current DTO); `quickNoteFetalMovement`: `Boolean` (no field annotation in current DTO); `quickNoteBloodPressure`: `Boolean` (no field annotation in current DTO); `quickNoteBloodGlucose`: `Boolean` (no field annotation in current DTO). | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/family/controller/CareGroupController.java` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
### UC-FM-03-TC-BR-001 — Enforce business rule 1

| Field | Specification |
| --- | --- |
| Stable ID | `UC-FM-03-TC-BR-001` |
| Severity | `High` |
...
1. Exercise the eligible and ineligible partitions for this rule: Only the authorized owner/mother may change member permissions.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | The server enforces: Only the authorized owner/mother may change member permissions. No disallowed state or protected data is produced. | `TDS BR-01; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/family/controller/CareGroupController.java` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
The contract differs from the cited expected result (The server enforces: Only the authorized owner/mother may change member permissions. No disallowed state or protected data is produced.), mutates data on a rejected/read path, leaks protected data, or triggers duplicate/unauthorized side effects.
...
### UC-FM-03-TC-BR-002 — Enforce business rule 2

| Field | Specification |
| --- | --- |
| Stable ID | `UC-FM-03-TC-BR-002` |
| Severity | `High` |
...
1. Exercise the eligible and ineligible partitions for this rule: Revoked permissions must affect subsequent data reads immediately according to policy.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | The server enforces: Revoked permissions must affect subsequent data reads immediately according to policy. No disallowed state or protected data is produced. | `TDS BR-02; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/family/controller/CareGroupController.java` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
The contract differs from the cited expected result (The server enforces: Revoked permissions must affect subsequent data reads immediately according to policy. No disallowed state or protected data is produced.), mutates data on a rejected/read path, leaks protected data, or triggers duplicate/unauthorized side effects.
...
### UC-FM-03-TC-SEC-001 — Reject wrong authentication, role, ownership, membership, or consent scope

| Field | Specification |
| --- | --- |
| Stable ID | `UC-FM-03-TC-SEC-001` |
| Severity | `Critical` |
...
### UC-FM-03-TC-GAP-001 — Prove the current actor entry path and owning contract remain reachable

| Field | Specification |
| --- | --- |
| Stable ID | `UC-FM-03-TC-GAP-001` |
| Severity | `Medium` |
| Test Condition | `COND-GAP` |
| Test Level | `Unit / Integration / Contract / applicable UI` |
| Platform / Layer | `Mobile / Backend` |
| Technique | `Requirement coverage + state/authorization partition as applicable` |
| Oracle Source | `SRS UC-FM-03 Implemented Entry Points/Contracts; 05_Development/CareBridgeMobileApp/lib/features/familySync/screens/manage_family_permission_screen.dart` |
| Preconditions | Synthetic `Mother / Group Owner` identity; eligible current resource state; isolated provider/database fixtures |
| Intended Test File | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/family/ManageFamilyPermissionIntegrationTest.java` |
| Initial Status | `🔴 Not written / existing evidence must be rerun` |
...
1. Start from `Mobile Manage Family Permission screen`, perform `Open an eligible member's permission settings.`, and observe the owning contract `GET/PATCH `/api/v1/care-groups/{groupId}/members/{memberId}/permissions`` rather than a retired or unrelated route.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | The current entry path remains reachable for `Manage Family Member Permissions` and invokes only the documented owning contract; an unreachable, static, or wrong-method path fails this case. | `SRS UC-FM-03 Implemented Entry Points/Contracts; 05_Development/CareBridgeMobileApp/lib/features/familySync/screens/manage_family_permission_screen.dart` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
The contract differs from the cited expected result (The current entry path remains reachable for `Manage Family Member Permissions` and invokes only the documented owning contract; an unreachable, static, or wrong-method path fails this case.), mutates data on a rejected/read path, leaks protected data, or triggers duplicate/unauthorized side effects.
...
| --- | --- | --- |
| Open an eligible member's permission settings. | `COND-01` | `UC-FM-03-TC-001` |
| Change only supported permission flags. | `COND-02` | `UC-FM-03-TC-002` |
| Persist and reload the effective permission set. | `COND-03` | `UC-FM-03-TC-003` |
| GET /api/v1/care-groups/{groupId}/members/{memberId}/permissions → getFamilyPermission contract | `COND-API-001` | `UC-FM-03-TC-API-001` |
| PATCH /api/v1/care-groups/{groupId}/members/{memberId}/permissions → updateFamilyPermission contract | `COND-API-002` | `UC-FM-03-TC-API-002` |
| Business-rule partitions | `COND-BR-*` | `UC-FM-03-TC-BR-*` |
| Authentication / authorization / ownership / consent | `COND-AUTH` | `UC-FM-03-TC-SEC-001` |
| Current gap / reachability boundary | `COND-GAP` | `UC-FM-03-TC-GAP-001` |

## 5. Red-Green-Refactor Tracker

| TC ID | Intended file | Red evidence | Green evidence | Refactor verification | Status |
| --- | --- | --- | --- | --- | --- |
| `UC-FM-03-TC-001` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/family/ManageFamilyPermissionIntegrationTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-FM-03-TC-002` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/family/ManageFamilyPermissionIntegrationTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-FM-03-TC-003` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/family/ManageFamilyPermissionIntegrationTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-FM-03-TC-API-001` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/family/service/CareGroupServiceImplPermissionTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-FM-03-TC-API-002` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/family/controller/CareGroupControllerPermissionTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-FM-03-TC-BR-001` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/family/ManageFamilyPermissionIntegrationTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-FM-03-TC-BR-002` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/family/ManageFamilyPermissionIntegrationTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-FM-03-TC-SEC-001` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/family/ManageFamilyPermissionIntegrationTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-FM-03-TC-GAP-001` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/family/ManageFamilyPermissionIntegrationTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
...
| Generic test matrix | Case titles/actions reference `Manage Family Member Permissions` operations and rules. | Pass |
