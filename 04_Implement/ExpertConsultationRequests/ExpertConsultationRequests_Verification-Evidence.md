> [!IMPORTANT]
> Historical lifecycle verification evidence for `UC-EX-08` and `UC-EX-09`; this is not a canonical current Test-Spec. Current code and the canonical code-first specifications override conflicts.
...
- `04_Implement/ExpertConsultationRequests/ExpertConsultationRequests_Architecture-Evidence.md` (CB-CONREQ-IMP-001) — spec gốc
...
| `ExpertConsultationRequests_Architecture-Evidence.md` ADR-CONREQ-008/009 (v1.5 — `Status: Accepted`, `Deciders: User/Product decision`) | Oracle cho việc xóa bỏ mọi test comment/oracle-note còn ghi "Open"/"chờ Product xác nhận" — `CONREQ-TC-001`/`FX-006` phải đọc đúng cùng config source, không hardcode độc lập |
