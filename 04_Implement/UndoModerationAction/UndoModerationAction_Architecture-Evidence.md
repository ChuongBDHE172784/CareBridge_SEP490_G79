> [!IMPORTANT]
> Historical shared-subflow evidence for `UC-AD-16` and `UC-AD-18`; this is not a canonical current TDS. Current code and the canonical code-first specifications override conflicts.
...
3. Unit test đầy đủ theo `UndoModerationAction_Verification-Evidence.md`.
...
> Chi tiết đầy đủ nằm trong `UndoModerationAction_Verification-Evidence.md`. Tóm tắt:
