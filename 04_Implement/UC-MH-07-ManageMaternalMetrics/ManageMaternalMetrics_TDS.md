...
# TECHNICAL DESIGN SPECIFICATION — Record and Review General Maternal Metrics

| Field | Value |
| --- | --- |
| Document ID | `UC-MH-07-TDS` |
| Version | `0.1` |
| Date | `2026-08-23` |
| Status | `Draft` |
| Function ID | `UC-MH-07` |
| Canonical Use Case | `UC-MH-07 — Record and Review General Maternal Metrics` |
...
| Reviewer / Approver |  |
| Source Baseline | Current worktree on `2026-08-23`; SRS `UC-MH-07`; exact evidence in Section 1.4 |
...
- **Goal:** Create, view, update, delete when allowed, and trend supported maternal metric observations for an owned journey.
- **Trigger:** The actor enters Mobile `/journeys/:journeyId/metrics/add`.
- **Outcome:** Review detail/trend or perform an allowed edit/delete.
...
- Mobile `/journeys/:journeyId/metrics/add`
- Mobile `/health-metrics/:id` and edit route
- Mobile `/journeys/:journeyId/metrics/trend`

- GET/POST/PUT `/api/v1/journeys/{journeyId}/metrics/**`
- GET/DELETE `/api/v1/health-metrics/{metricId}`
...
| Postcondition | Review detail/trend or perform an allowed edit/delete. |
...
| --- | --- | --- | --- | --- |
| `SRC-SRS-01` | Requirement | `02_Requirements/SRS/Report3_Functional_Specifications.md` — UC-MH-07 | 2026-08-23 | Draft code-first requirement |
...
| --- | --- | --- | --- | --- |
| `UC-MH-07-FR-01` | Choose a supported metric type for the owned journey. | `02_Requirements/SRS/Report3_Functional_Specifications.md` — UC-MH-07 Normal Flow 1 | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/health/controller/HealthMetricController.java` | `COND-01` / `UC-MH-07-TC-001` |
| `UC-MH-07-FR-02` | Enter a value/unit/time and submit after capability validation. | `02_Requirements/SRS/Report3_Functional_Specifications.md` — UC-MH-07 Normal Flow 2 | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/health/controller/HealthMetricController.java` | `COND-02` / `UC-MH-07-TC-002` |
| `UC-MH-07-FR-03` | Review detail/trend or perform an allowed edit/delete. | `02_Requirements/SRS/Report3_Functional_Specifications.md` — UC-MH-07 Normal Flow 3 | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/health/controller/JourneyMetricController.java` | `COND-03` / `UC-MH-07-TC-003` |
| `BR-01` | Metric type, unit, ranges, journey ownership, and edit/delete capability are server authoritative. | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/health/controller/HealthMetricController.java` | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/health/controller/HealthMetricController.java` | `COND-BR-01` / `UC-MH-07-TC-BR-001` |
| `BR-02` | Specialized fetal movement, hydration, and EPDS flows are specified separately. | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/health/controller/JourneyMetricController.java` | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/health/controller/JourneyMetricController.java` | `COND-BR-02` / `UC-MH-07-TC-BR-002` |

## 3. Architecture Decision Records (ADR)

### ADR-UC-MH-07-01 — Use a distinct actor-goal boundary

| Item | Decision |
| --- | --- |
| Context | The retired 43-UC catalogue grouped multiple triggers, lifecycles, and permission boundaries, making implementation/test traceability generic. |
| Options | Keep the broad catalogue; split by screen; split by actor goal plus lifecycle/security boundary. |
| Decision | Use `UC-MH-07 — Record and Review General Maternal Metrics` as the canonical boundary because its operations share the stated actor outcome and current implementation evidence. |
| Consequences | Related supporting screens/endpoints stay in one TDS; different lifecycle/actor outcomes have separate UCs. |
| Source / Status | SRS Section 3.1 and current code audit / Draft |

### ADR-UC-MH-07-02 — Preserve unknowns as Open
...
| --- | --- | --- |
| `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/health/controller/JourneyMetricController.java` | Reuse | Current implementation evidence for Record and Review General Maternal Metrics; inspect the exact symbol before implementation changes. |
| `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/health/controller/HealthMetricController.java` | Reuse | Current implementation evidence for Record and Review General Maternal Metrics; inspect the exact symbol before implementation changes. |
| `05_Development/CareBridgeMobileApp/lib/features/healthRecords/screens/add_maternal_health_metric_screen.dart` | Reuse | Current implementation evidence for Record and Review General Maternal Metrics; inspect the exact symbol before implementation changes. |
| `05_Development/CareBridgeMobileApp/lib/features/healthRecords/screens/health_metric_trend_screen.dart` | Reuse | Current implementation evidence for Record and Review General Maternal Metrics; inspect the exact symbol before implementation changes. |
...
class HealthMetricController as "HealthMetricController.java"
JourneyMetricController --> HealthMetricController
class add_maternal_health_metric_screen as "add_maternal_health_metric_screen.dart"
HealthMetricController --> add_maternal_health_metric_screen
class health_metric_trend_screen as "health_metric_trend_screen.dart"
add_maternal_health_metric_screen --> health_metric_trend_screen
@enduml
```
...
| Item | Assessment |
| --- | --- |
| Current stores/entities | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/health/entity/MetricType.java` |
...
participant Client
participant Domain
Actor -> Client: Enter Record and Review General Maternal Metrics
Client -> Domain: Choose a supported metric type for the owned journey.
Domain --> Client: Result for step 1
Client -> Domain: Enter a value/unit/time and submit after capability validation.
Domain --> Client: Result for step 2
Client -> Domain: Review detail/trend or perform an allowed edit/delete.
...
| --- | --- | --- |
| Cancel before mutation | No unintended write or provider side effect. | SRS UC-MH-07 Alternative Flow |
...
@startuml
[*] --> Eligible
Eligible --> InProgress : Choose a supported metric type for the owned journey.
InProgress --> Outcome : Review detail/trend or perform an allowed edit/delete.
...
- Metric type, unit, ranges, journey ownership, and edit/delete capability are server authoritative.
- Specialized fetal movement, hydration, and EPDS flows are specified separately.
...
| ---: | --- | --- | --- |
| 1 | Mobile `/journeys/:journeyId/metrics/add` | Mother | Reachable current entry point |
| 2 | Mobile `/health-metrics/:id` and edit route | Mother | Reachable current entry point |
| 3 | Mobile `/journeys/:journeyId/metrics/trend` | Mother | Reachable current entry point |
...
| `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/health/controller/HealthMetricController.java` | Support the mapped operations without broadening authorization or lifecycle semantics. |
...
| Parameters | path `metricId`: `UUID`; principal `principal`: `Principal` |
...
| Positive test mapping | `COND-API-001` / `UC-MH-07-TC-API-001` |
| Negative test mapping | `COND-AUTH`; DTO/service-only negative paths require their exact cited oracle |

### 9.2 Handler Contract — `GET /api/v1/health-metrics/{metricId}`
...
| Request body type | `None` |
| Request fields and validators | Not applicable — no request body |
| Response type | `ResponseEntity<ApiResponse<MetricDetailResponse>>` |
| Response payload fields | `id`: `UUID` (no field annotation in current DTO); `journeyId`: `UUID` (no field annotation in current DTO); `metricType`: `String` (no field annotation in current DTO); `valueNumeric`: `BigDecimal` (no field annotation in current DTO); `valueSecondary`: `BigDecimal` (no field annotation in current DTO); `unit`: `String` (no field annotation in current DTO); `measuredAt`: `Instant` (no field annotation in current DTO); `sourceType`: `String` (no field annotation in current DTO); `note`: `String` (no field annotation in current DTO); `createdAt`: `Instant` (no field annotation in current DTO); `context`: `Map<String, Object>` (no field annotation in current DTO); `periodStart`: `Instant` (no field annotation in current DTO); `periodEnd`: `Instant` (no field annotation in current DTO); `qualityLabel`: `String` (no field annotation in current DTO); `disclaimer`: `String` (no field annotation in current DTO); `definitionVersion`: `Integer` (no field annotation in current DTO) |
| Explicit/documented statuses | `200` |
| Positive test mapping | `COND-API-002` / `UC-MH-07-TC-API-002` |
| Negative test mapping | `COND-AUTH`; DTO/service-only negative paths require their exact cited oracle |

### 9.3 Handler Contract — `GET /api/v1/journeys/{journeyId}/metrics`
...
| Explicit/documented statuses | `200` |
| Positive test mapping | `COND-API-003` / `UC-MH-07-TC-API-003` |
| Negative test mapping | `COND-AUTH`; DTO/service-only negative paths require their exact cited oracle |

### 9.4 Handler Contract — `POST /api/v1/journeys/{journeyId}/metrics`
...
| Positive test mapping | `COND-API-004` / `UC-MH-07-TC-API-004` |
| Negative test mapping | `COND-API-004-VAL` / `UC-MH-07-TC-API-004-VAL`; plus `COND-AUTH` for protected access |

### 9.5 Handler Contract — `GET /api/v1/journeys/{journeyId}/metrics/capabilities`
...
| Response payload fields | Not applicable or unresolved from the handler import |
| Explicit/documented statuses | `200` |
| Positive test mapping | `COND-API-005` / `UC-MH-07-TC-API-005` |
| Negative test mapping | `COND-AUTH`; DTO/service-only negative paths require their exact cited oracle |

### 9.6 Handler Contract — `PUT /api/v1/journeys/{journeyId}/metrics/{metricId}`
...
| Response payload fields | `metricId`: `UUID` (no field annotation in current DTO); `journeyId`: `UUID` (no field annotation in current DTO); `metricType`: `String` (no field annotation in current DTO); `valueNumeric`: `BigDecimal` (no field annotation in current DTO); `valueSecondary`: `BigDecimal` (no field annotation in current DTO); `unit`: `String` (no field annotation in current DTO); `measuredAt`: `Instant` (no field annotation in current DTO); `sourceType`: `String` (no field annotation in current DTO); `note`: `String` (no field annotation in current DTO); `createdAt`: `Instant` (no field annotation in current DTO); `updatedAt`: `Instant` (no field annotation in current DTO); `context`: `Map<String, Object>` (no field annotation in current DTO); `periodStart`: `Instant` (no field annotation in current DTO); `periodEnd`: `Instant` (no field annotation in current DTO); `qualityLabel`: `String` (no field annotation in current DTO); `disclaimer`: `String` (no field annotation in current DTO); `definitionVersion`: `Integer` (no field annotation in current DTO); `aiInsight`: `String` (no field annotation in current DTO); `redFlagAlert`: `boolean` (no field annotation in current DTO) |
| Explicit/documented statuses | `200` |
| Positive test mapping | `COND-API-006` / `UC-MH-07-TC-API-006` |
| Negative test mapping | `COND-API-006-VAL` / `UC-MH-07-TC-API-006-VAL`; plus `COND-AUTH` for protected access |
...
| --- | --- | --- | --- |
| `VG-01` | Choose a supported metric type for the owned journey. | `COND-01` | `UC-MH-07-TC-001` |
| `VG-02` | Enter a value/unit/time and submit after capability validation. | `COND-02` | `UC-MH-07-TC-002` |
| `VG-03` | Review detail/trend or perform an allowed edit/delete. | `COND-03` | `UC-MH-07-TC-003` |
| `VG-AUTH` | Reject wrong authentication/role/ownership/membership/consent scope | `COND-AUTH` | `UC-MH-07-TC-SEC-001` |
| `VG-GAP` | Characterize each known gap without claiming a false completed path | `COND-GAP` | `UC-MH-07-TC-GAP-001` |
...
- `cd 05_Development/CareBridgeAPI && ./mvnw -Dtest=HealthMetricAddServiceTest test`
- `cd 05_Development/CareBridgeAPI && ./mvnw -Dtest=HealthMetricUpdateServiceTest test`
- `cd 05_Development/CareBridgeAPI && ./mvnw -Dtest=MetricTrendServiceTest test`
...
| Request | `DELETE /api/v1/health-metrics/{metricId}` → `deleteMetric`; `None` with Not applicable — no request body; authorization: hasRole('MOTHER'). |
| Success response | `ResponseEntity<Void>` with Not applicable or unresolved from the handler import; explicit/documented statuses `204`. |
...
| --- | --- | --- |
| Mother | `DELETE /api/v1/health-metrics/{metricId}` | hasRole('MOTHER'); handler oracle `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/health/controller/HealthMetricController.java` |
| Mother | `GET /api/v1/health-metrics/{metricId}` | hasRole('MOTHER'); handler oracle `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/health/controller/HealthMetricController.java` |
