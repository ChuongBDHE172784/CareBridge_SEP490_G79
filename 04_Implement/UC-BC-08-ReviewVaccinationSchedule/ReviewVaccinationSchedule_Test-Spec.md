...
# TEST SPECIFICATION — Review Vaccination Schedule and Create Next-Dose Reminder

| Field | Value |
| --- | --- |
| Document ID | `UC-BC-08-TEST-SPEC` |
| Version | `0.1` |
| Date | `2026-08-23` |
| Status | `Draft` |
| Feature / Gap ID | `UC-BC-08` |
| Module | `Baby Care` |
| Paired TDS | `UC-BC-08-TDS` |
| Priority | `Medium` |
...
| Reviewer / Approver |  |
| Source Baseline | Current worktree `2026-08-23`; SRS/TDS `UC-BC-08` and exact code/test sources below |
...
| --- | --- | --- |
| Actor goal | Review the baby's vaccination schedule and create a supported reminder for an upcoming suggested dose. | SRS `UC-BC-08` |
| Current state | `High` confidence; gaps are listed in Section 2 | Exact current code/test sources below |
| Entry points | Baby-detail vaccination tab; Mobile `/reminders/vaccination/add` | Current client/router evidence |
...
| Primary operations | Load the authorized baby's computed schedule.; Select an upcoming eligible dose/suggestion.; Create and display the linked reminder. | SRS `UC-BC-08` Normal Flow |
...
| --- | --- | --- | --- | --- |
| `SRC-SRS` | Requirement | `02_Requirements/SRS/Report3_Functional_Specifications.md` — UC-BC-08 | 2026-08-23 | Draft code-first requirement |
| `SRC-TDS` | Design | Paired `UC-BC-08-TDS` | 0.1 | Draft design |
...
| --- | --- | --- | --- | --- | --- |
| `LI-01` | Broad 43-UC catalogue previously obscured this boundary | Generic TCs could not map to `Review Vaccination Schedule and Create Next-Dose Reminder` | Split as `UC-BC-08` using the audited current code boundary | SRS 3.1 and `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/vaccination/controller/VaccinationController.java` | Resolved in Draft |
...
| --- | --- | --- | --- | --- | --- | --- |
| `RISK-01` | Failure of: Load the authorized baby's computed schedule. | High | Medium | High | Unit/Integration/Contract/applicable UI | `COND-01` |
| `RISK-02` | Failure of: Select an upcoming eligible dose/suggestion. | High | Medium | High | Unit/Integration/Contract/applicable UI | `COND-02` |
| `RISK-03` | Failure of: Create and display the linked reminder. | High | Medium | High | Unit/Integration/Contract/applicable UI | `COND-03` |
...
| --- | --- | --- | --- | --- |
| `BASIS-01` | `UC-BC-08-FR-01` — Load the authorized baby's computed schedule. | SRS `UC-BC-08` Normal Flow 1; TDS Section 2 | Load the authorized baby's computed schedule. | `COND-01` |
| `BASIS-02` | `UC-BC-08-FR-02` — Select an upcoming eligible dose/suggestion. | SRS `UC-BC-08` Normal Flow 2; TDS Section 2 | Select an upcoming eligible dose/suggestion. | `COND-02` |
| `BASIS-03` | `UC-BC-08-FR-03` — Create and display the linked reminder. | SRS `UC-BC-08` Normal Flow 3; TDS Section 2 | Create and display the linked reminder. | `COND-03` |
...
| --- | --- | --- | --- | --- | --- |
| `COND-01` | `BASIS-01` / `RISK-01` | Load the authorized baby's computed schedule. | Mobile / Backend | Positive + applicable boundary/state coverage | `UC-BC-08-TC-001` |
| `COND-02` | `BASIS-02` / `RISK-02` | Select an upcoming eligible dose/suggestion. | Mobile / Backend | Positive + applicable boundary/state coverage | `UC-BC-08-TC-002` |
| `COND-03` | `BASIS-03` / `RISK-03` | Create and display the linked reminder. | Mobile / Backend | Positive + applicable boundary/state coverage | `UC-BC-08-TC-003` |
| `COND-API-001` | Exact handler/client composition contract | POST /api/v1/reminders/vaccination → createVaccinationReminder contract | Applicable backend/client contract layer | Contract + DTO/status/authorization evidence | `UC-BC-08-TC-API-001` |
| `COND-API-001-VAL` | Exact handler/client composition contract | POST /api/v1/reminders/vaccination rejects a declared request-field boundary | Applicable backend/client contract layer | Contract + DTO/status/authorization evidence | `UC-BC-08-TC-API-001-VAL` |
| `COND-API-002` | Exact handler/client composition contract | GET /api/v1/reminders/vaccination/suggestions → getVaccinationSuggestions contract | Applicable backend/client contract layer | Contract + DTO/status/authorization evidence | `UC-BC-08-TC-API-002` |
| `COND-API-003` | Exact handler/client composition contract | GET /api/v1/vaccination/babies/{babyId}/schedule → getVaccinationSchedule contract | Applicable backend/client contract layer | Contract + DTO/status/authorization evidence | `UC-BC-08-TC-API-003` |
| `COND-BR-01` | `BR-01` | Schedule/suggestion state comes from canonical vaccination records and catalogue rules. | `05_Development/CareBridgeMobileApp/lib/features/healthRecords/services/vaccination_service.dart` | Negative / decision / state | `UC-BC-08-TC-BR-001` |
| `COND-BR-02` | `BR-02` | Creating a reminder does not mark a dose completed. | `05_Development/CareBridgeMobileApp/lib/features/reminder/services/reminder_service.dart` | Negative / decision / state | `UC-BC-08-TC-BR-002` |
| `COND-AUTH` | `RISK-AUTH` | Reject wrong authentication, role, ownership, membership, consent, or state scope | All protected layers | Security | `UC-BC-08-TC-SEC-001` |
| `COND-GAP` | `RISK-GAP` | Characterize current limitation/reachability without false completion | Applicable layer | Gap/Regression | `UC-BC-08-TC-GAP-001` |
...
### UC-BC-08-TC-001 — Load the authorized baby's computed schedule

| Field | Specification |
| --- | --- |
| Stable ID | `UC-BC-08-TC-001` |
| Severity | `High` |
...
| Oracle Source | `SRS UC-BC-08 Normal Flow 1; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/reminder/controller/ReminderController.java` |
...
| Intended Test File | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/vaccination/VaccinationBookServiceTest.java` |
...
1. Load the authorized baby's computed schedule.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | The observable outcome is exactly `Load the authorized baby's computed schedule.` The applicable handler, request/response type, status, and authorization evidence are those enumerated in paired TDS Sections 2, 9, 10, and 16; no additional state is claimed. | `SRS UC-BC-08 Normal Flow 1; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/reminder/controller/ReminderController.java` |
...
| UI state | Reachable client displays the canonical outcome/error without false success. | SRS `UC-BC-08` entry point and current client evidence |
...
The contract differs from the cited expected result (The observable outcome is exactly `Load the authorized baby's computed schedule.` The applicable handler, request/response type, status, and authorization evidence are those enumerated in paired TDS Sections 2, 9, 10, and 16; no additional state is claimed.), mutates data on a rejected/read path, leaks protected data, or triggers duplicate/unauthorized side effects.
...
### UC-BC-08-TC-002 — Select an upcoming eligible dose/suggestion

| Field | Specification |
| --- | --- |
| Stable ID | `UC-BC-08-TC-002` |
| Severity | `High` |
| Test Condition | `COND-02` |
| Test Level | `Unit / Integration / Contract / applicable UI` |
| Platform / Layer | `Mobile / Backend` |
| Technique | `Requirement coverage + state/authorization partition as applicable` |
| Oracle Source | `SRS UC-BC-08 Normal Flow 2; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/reminder/controller/ReminderController.java` |
| Preconditions | Synthetic `Mother / Authorized Caregiver` identity; eligible current resource state; isolated provider/database fixtures |
| Intended Test File | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/vaccination/VaccinationReminderDispatchTest.java` |
| Initial Status | `🔴 Not written / existing evidence must be rerun` |
...
1. Select an upcoming eligible dose/suggestion.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | The observable outcome is exactly `Select an upcoming eligible dose/suggestion.` The applicable handler, request/response type, status, and authorization evidence are those enumerated in paired TDS Sections 2, 9, 10, and 16; no additional state is claimed. | `SRS UC-BC-08 Normal Flow 2; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/reminder/controller/ReminderController.java` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
The contract differs from the cited expected result (The observable outcome is exactly `Select an upcoming eligible dose/suggestion.` The applicable handler, request/response type, status, and authorization evidence are those enumerated in paired TDS Sections 2, 9, 10, and 16; no additional state is claimed.), mutates data on a rejected/read path, leaks protected data, or triggers duplicate/unauthorized side effects.
...
### UC-BC-08-TC-003 — Create and display the linked reminder

| Field | Specification |
| --- | --- |
| Stable ID | `UC-BC-08-TC-003` |
| Severity | `High` |
| Test Condition | `COND-03` |
| Test Level | `Unit / Integration / Contract / applicable UI` |
| Platform / Layer | `Mobile / Backend` |
| Technique | `Requirement coverage + state/authorization partition as applicable` |
| Oracle Source | `SRS UC-BC-08 Normal Flow 3; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/vaccination/controller/VaccinationController.java` |
| Preconditions | Synthetic `Mother / Authorized Caregiver` identity; eligible current resource state; isolated provider/database fixtures |
...
1. Create and display the linked reminder.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | The observable outcome is exactly `Create and display the linked reminder.` The applicable handler, request/response type, status, and authorization evidence are those enumerated in paired TDS Sections 2, 9, 10, and 16; no additional state is claimed. | `SRS UC-BC-08 Normal Flow 3; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/vaccination/controller/VaccinationController.java` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
The contract differs from the cited expected result (The observable outcome is exactly `Create and display the linked reminder.` The applicable handler, request/response type, status, and authorization evidence are those enumerated in paired TDS Sections 2, 9, 10, and 16; no additional state is claimed.), mutates data on a rejected/read path, leaks protected data, or triggers duplicate/unauthorized side effects.
...
### UC-BC-08-TC-API-001 — POST /api/v1/reminders/vaccination → createVaccinationReminder contract

| Field | Specification |
| --- | --- |
| Stable ID | `UC-BC-08-TC-API-001` |
| Severity | `High` |
...
### UC-BC-08-TC-API-001-VAL — POST /api/v1/reminders/vaccination rejects a declared request-field boundary

| Field | Specification |
| --- | --- |
| Stable ID | `UC-BC-08-TC-API-001-VAL` |
| Severity | `High` |
...
### UC-BC-08-TC-API-002 — GET /api/v1/reminders/vaccination/suggestions → getVaccinationSuggestions contract

| Field | Specification |
| --- | --- |
| Stable ID | `UC-BC-08-TC-API-002` |
| Severity | `High` |
...
### UC-BC-08-TC-API-003 — GET /api/v1/vaccination/babies/{babyId}/schedule → getVaccinationSchedule contract

| Field | Specification |
| --- | --- |
| Stable ID | `UC-BC-08-TC-API-003` |
| Severity | `High` |
...
1. Invoke `GET /api/v1/vaccination/babies/{babyId}/schedule` so `getVaccinationSchedule` receives path `babyId`: `UUID`; principal `principal`: `Principal`; satisfy `isAuthenticated()` and the extracted `None` constraints.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | Success status `200` returns `ResponseEntity<ApiResponse<VaccinationScheduleResponse>>` with payload fields `babyId`: `UUID` (no field annotation in current DTO); `babyNickname`: `String` (no field annotation in current DTO); `doses`: `List<VaccinationDoseDto>` (no field annotation in current DTO); the request body is `None` with Not applicable — no request body. | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/vaccination/controller/VaccinationController.java` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
### UC-BC-08-TC-BR-001 — Enforce business rule 1

| Field | Specification |
| --- | --- |
| Stable ID | `UC-BC-08-TC-BR-001` |
| Severity | `High` |
| Test Condition | `COND-BR-01` |
| Test Level | `Unit / Integration / Contract / applicable UI` |
| Platform / Layer | `Mobile / Backend` |
| Technique | `Requirement coverage + state/authorization partition as applicable` |
| Oracle Source | `TDS BR-01; 05_Development/CareBridgeMobileApp/lib/features/healthRecords/services/vaccination_service.dart` |
| Preconditions | Synthetic `Mother / Authorized Caregiver` identity; eligible current resource state; isolated provider/database fixtures |
| Intended Test File | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/vaccination/VaccinationBookServiceTest.java` |
| Initial Status | `🔴 Not written / existing evidence must be rerun` |
...
1. Exercise the eligible and ineligible partitions for this rule: Schedule/suggestion state comes from canonical vaccination records and catalogue rules.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | The server enforces: Schedule/suggestion state comes from canonical vaccination records and catalogue rules. No disallowed state or protected data is produced. | `TDS BR-01; 05_Development/CareBridgeMobileApp/lib/features/healthRecords/services/vaccination_service.dart` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
The contract differs from the cited expected result (The server enforces: Schedule/suggestion state comes from canonical vaccination records and catalogue rules. No disallowed state or protected data is produced.), mutates data on a rejected/read path, leaks protected data, or triggers duplicate/unauthorized side effects.
...
### UC-BC-08-TC-BR-002 — Enforce business rule 2

| Field | Specification |
| --- | --- |
| Stable ID | `UC-BC-08-TC-BR-002` |
| Severity | `High` |
| Test Condition | `COND-BR-02` |
| Test Level | `Unit / Integration / Contract / applicable UI` |
| Platform / Layer | `Mobile / Backend` |
| Technique | `Requirement coverage + state/authorization partition as applicable` |
| Oracle Source | `TDS BR-02; 05_Development/CareBridgeMobileApp/lib/features/reminder/services/reminder_service.dart` |
| Preconditions | Synthetic `Mother / Authorized Caregiver` identity; eligible current resource state; isolated provider/database fixtures |
| Intended Test File | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/vaccination/VaccinationReminderDispatchTest.java` |
| Initial Status | `🔴 Not written / existing evidence must be rerun` |
...
1. Exercise the eligible and ineligible partitions for this rule: Creating a reminder does not mark a dose completed.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | The server enforces: Creating a reminder does not mark a dose completed. No disallowed state or protected data is produced. | `TDS BR-02; 05_Development/CareBridgeMobileApp/lib/features/reminder/services/reminder_service.dart` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
The contract differs from the cited expected result (The server enforces: Creating a reminder does not mark a dose completed. No disallowed state or protected data is produced.), mutates data on a rejected/read path, leaks protected data, or triggers duplicate/unauthorized side effects.
...
### UC-BC-08-TC-SEC-001 — Reject wrong authentication, role, ownership, membership, or consent scope

| Field | Specification |
| --- | --- |
| Stable ID | `UC-BC-08-TC-SEC-001` |
| Severity | `Critical` |
...
### UC-BC-08-TC-GAP-001 — Prove the current actor entry path and owning contract remain reachable

| Field | Specification |
| --- | --- |
| Stable ID | `UC-BC-08-TC-GAP-001` |
| Severity | `Medium` |
| Test Condition | `COND-GAP` |
| Test Level | `Unit / Integration / Contract / applicable UI` |
| Platform / Layer | `Mobile / Backend` |
| Technique | `Requirement coverage + state/authorization partition as applicable` |
| Oracle Source | `SRS UC-BC-08 Implemented Entry Points/Contracts; 05_Development/CareBridgeMobileApp/lib/features/healthRecords/services/vaccination_service.dart` |
| Preconditions | Synthetic `Mother / Authorized Caregiver` identity; eligible current resource state; isolated provider/database fixtures |
| Intended Test File | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/vaccination/VaccinationBookServiceTest.java` |
| Initial Status | `🔴 Not written / existing evidence must be rerun` |
...
1. Start from `Baby-detail vaccination tab`, perform `Load the authorized baby's computed schedule.`, and observe the owning contract `GET `/api/v1/vaccination/babies/{babyId}/schedule`` rather than a retired or unrelated route.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | The current entry path remains reachable for `Review Vaccination Schedule and Create Next-Dose Reminder` and invokes only the documented owning contract; an unreachable, static, or wrong-method path fails this case. | `SRS UC-BC-08 Implemented Entry Points/Contracts; 05_Development/CareBridgeMobileApp/lib/features/healthRecords/services/vaccination_service.dart` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
The contract differs from the cited expected result (The current entry path remains reachable for `Review Vaccination Schedule and Create Next-Dose Reminder` and invokes only the documented owning contract; an unreachable, static, or wrong-method path fails this case.), mutates data on a rejected/read path, leaks protected data, or triggers duplicate/unauthorized side effects.
...
| --- | --- | --- |
| Load the authorized baby's computed schedule. | `COND-01` | `UC-BC-08-TC-001` |
| Select an upcoming eligible dose/suggestion. | `COND-02` | `UC-BC-08-TC-002` |
| Create and display the linked reminder. | `COND-03` | `UC-BC-08-TC-003` |
| POST /api/v1/reminders/vaccination → createVaccinationReminder contract | `COND-API-001` | `UC-BC-08-TC-API-001` |
| POST /api/v1/reminders/vaccination rejects a declared request-field boundary | `COND-API-001-VAL` | `UC-BC-08-TC-API-001-VAL` |
| GET /api/v1/reminders/vaccination/suggestions → getVaccinationSuggestions contract | `COND-API-002` | `UC-BC-08-TC-API-002` |
| GET /api/v1/vaccination/babies/{babyId}/schedule → getVaccinationSchedule contract | `COND-API-003` | `UC-BC-08-TC-API-003` |
| Business-rule partitions | `COND-BR-*` | `UC-BC-08-TC-BR-*` |
| Authentication / authorization / ownership / consent | `COND-AUTH` | `UC-BC-08-TC-SEC-001` |
| Current gap / reachability boundary | `COND-GAP` | `UC-BC-08-TC-GAP-001` |

## 5. Red-Green-Refactor Tracker

| TC ID | Intended file | Red evidence | Green evidence | Refactor verification | Status |
| --- | --- | --- | --- | --- | --- |
| `UC-BC-08-TC-001` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/vaccination/VaccinationBookServiceTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-BC-08-TC-002` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/vaccination/VaccinationBookServiceTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-BC-08-TC-003` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/vaccination/VaccinationBookServiceTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-BC-08-TC-API-001` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/reminder/VaccinationReminderServiceTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-BC-08-TC-API-001-VAL` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/reminder/VaccinationReminderServiceTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-BC-08-TC-API-002` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/reminder/VaccinationReminderServiceTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-BC-08-TC-API-003` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/vaccination/VaccinationBookServiceTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-BC-08-TC-BR-001` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/vaccination/VaccinationBookServiceTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-BC-08-TC-BR-002` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/vaccination/VaccinationBookServiceTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-BC-08-TC-SEC-001` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/vaccination/VaccinationBookServiceTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-BC-08-TC-GAP-001` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/vaccination/VaccinationBookServiceTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
...
| Generic test matrix | Case titles/actions reference `Review Vaccination Schedule and Create Next-Dose Reminder` operations and rules. | Pass |
