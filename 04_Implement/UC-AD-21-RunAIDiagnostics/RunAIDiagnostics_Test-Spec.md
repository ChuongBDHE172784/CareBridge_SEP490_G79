...
# TEST SPECIFICATION — Run AI Diagnostic and Clinical Simulators

| Field | Value |
| --- | --- |
| Document ID | `UC-AD-21-TEST-SPEC` |
| Version | `0.1` |
| Date | `2026-08-23` |
| Status | `Draft` |
| Feature / Gap ID | `UC-AD-21` |
| Module | `Administration and Operations` |
| Paired TDS | `UC-AD-21-TDS` |
| Priority | `Medium` |
...
| Reviewer / Approver |  |
| Source Baseline | Current worktree `2026-08-23`; SRS/TDS `UC-AD-21` and exact code/test sources below |
...
| --- | --- | --- |
| Actor goal | Test prompt/model configuration and run deterministic metric simulation to inspect actual versus expected screening behavior. | SRS `UC-AD-21` |
...
| Authorization boundary | `Authorized Technical Operator` plus exact authentication/role/ownership/membership/consent policy | `05_Development/CareBridgeAITriageService/app/api/v1/chat.py` |
| Primary operations | Authenticate with the configured internal operator key.; Inspect model configuration or submit a bounded diagnostic prompt/simulation batch.; Review actual versus expected results without promoting them to production guarantees. | SRS `UC-AD-21` Normal Flow |
...
| --- | --- | --- | --- | --- |
| `SRC-SRS` | Requirement | `02_Requirements/SRS/Report3_Functional_Specifications.md` — UC-AD-21 | 2026-08-23 | Draft code-first requirement |
| `SRC-TDS` | Design | Paired `UC-AD-21-TDS` | 0.1 | Draft design |
...
| --- | --- | --- | --- | --- | --- |
| `LI-01` | Broad 43-UC catalogue previously obscured this boundary | Generic TCs could not map to `Run AI Diagnostic and Clinical Simulators` | Split as `UC-AD-21` using the audited current code boundary | SRS 3.1 and `05_Development/CareBridgeAITriageService/app/api/v1/chat.py` | Resolved in Draft |
...
| `RISK-02` | Failure of: Inspect model configuration or submit a bounded diagnostic prompt/simulation batch. | High | Medium | High | Unit/Integration/Contract/applicable UI | `COND-02` |
| `RISK-03` | Failure of: Review actual versus expected results without promoting them to production guarantees. | High | Medium | High | Unit/Integration/Contract/applicable UI | `COND-03` |
...
| --- | --- | --- | --- | --- |
| `BASIS-01` | `UC-AD-21-FR-01` — Authenticate with the configured internal operator key. | SRS `UC-AD-21` Normal Flow 1; TDS Section 2 | Authenticate with the configured internal operator key. | `COND-01` |
| `BASIS-02` | `UC-AD-21-FR-02` — Inspect model configuration or submit a bounded diagnostic prompt/simulation batch. | SRS `UC-AD-21` Normal Flow 2; TDS Section 2 | Inspect model configuration or submit a bounded diagnostic prompt/simulation batch. | `COND-02` |
| `BASIS-03` | `UC-AD-21-FR-03` — Review actual versus expected results without promoting them to production guarantees. | SRS `UC-AD-21` Normal Flow 3; TDS Section 2 | Review actual versus expected results without promoting them to production guarantees. | `COND-03` |
...
| --- | --- | --- | --- | --- | --- |
| `COND-01` | `BASIS-01` / `RISK-01` | Authenticate with the configured internal operator key. | Python AI Service / FastAPI Swagger | Positive + applicable boundary/state coverage | `UC-AD-21-TC-001` |
| `COND-02` | `BASIS-02` / `RISK-02` | Inspect model configuration or submit a bounded diagnostic prompt/simulation batch. | Python AI Service / FastAPI Swagger | Positive + applicable boundary/state coverage | `UC-AD-21-TC-002` |
| `COND-03` | `BASIS-03` / `RISK-03` | Review actual versus expected results without promoting them to production guarantees. | Python AI Service / FastAPI Swagger | Positive + applicable boundary/state coverage | `UC-AD-21-TC-003` |
| `COND-API-001` | Exact handler/client composition contract | GET /api/v1/chat/models → list_available_models contract | Applicable backend/client contract layer | Contract + DTO/status/authorization evidence | `UC-AD-21-TC-API-001` |
| `COND-API-002` | Exact handler/client composition contract | POST /api/v1/chat/test-prompt → test_custom_prompt contract | Applicable backend/client contract layer | Contract + DTO/status/authorization evidence | `UC-AD-21-TC-API-002` |
| `COND-API-003` | Exact handler/client composition contract | POST /api/v1/metrics/simulate-batch → simulate_clinical_cases contract | Applicable backend/client contract layer | Contract + DTO/status/authorization evidence | `UC-AD-21-TC-API-003` |
| `COND-BR-01` | `BR-01` | Diagnostic endpoints are operational tools, not consumer clinical flows. | `05_Development/CareBridgeAITriageService/app/services/metrics_screening_service.py` | Negative / decision / state | `UC-AD-21-TC-BR-001` |
| `COND-BR-02` | `BR-02` | Historical counts, latency, uptime, or accuracy are not current requirements unless rerun and dated. | `05_Development/CareBridgeAITriageService/app/services/metrics_screening_service.py` | Negative / decision / state | `UC-AD-21-TC-BR-002` |
| `COND-AUTH` | `RISK-AUTH` | Reject wrong authentication, role, ownership, membership, consent, or state scope | All protected layers | Security | `UC-AD-21-TC-SEC-001` |
| `COND-GAP` | `RISK-GAP` | Characterize current limitation/reachability without false completion | Applicable layer | Gap/Regression | `UC-AD-21-TC-GAP-001` |
...
### UC-AD-21-TC-001 — Authenticate with the configured internal operator key

| Field | Specification |
| --- | --- |
| Stable ID | `UC-AD-21-TC-001` |
| Severity | `High` |
| Test Condition | `COND-01` |
| Test Level | `Unit / Integration / Contract / applicable UI` |
| Platform / Layer | `Python AI Service / FastAPI Swagger` |
| Technique | `Requirement coverage + state/authorization partition as applicable` |
| Oracle Source | `SRS UC-AD-21 Normal Flow 1; 05_Development/CareBridgeAITriageService/app/api/v1/chat.py` |
...
| --- | --- | --- |
| Response / return | The observable outcome is exactly `Authenticate with the configured internal operator key.` The applicable handler, request/response type, status, and authorization evidence are those enumerated in paired TDS Sections 2, 9, 10, and 16; no additional state is claimed. | `SRS UC-AD-21 Normal Flow 1; 05_Development/CareBridgeAITriageService/app/api/v1/chat.py` |
...
| UI state | Reachable client displays the canonical outcome/error without false success. | SRS `UC-AD-21` entry point and current client evidence |
...
### UC-AD-21-TC-002 — Inspect model configuration or submit a bounded diagnostic prompt/simulation batch

| Field | Specification |
| --- | --- |
| Stable ID | `UC-AD-21-TC-002` |
| Severity | `High` |
| Test Condition | `COND-02` |
| Test Level | `Unit / Integration / Contract / applicable UI` |
| Platform / Layer | `Python AI Service / FastAPI Swagger` |
| Technique | `Requirement coverage + state/authorization partition as applicable` |
| Oracle Source | `SRS UC-AD-21 Normal Flow 2; 05_Development/CareBridgeAITriageService/app/api/v1/chat.py` |
| Preconditions | Synthetic authorized technical operator; configured internal API key; isolated AI-service fixtures |
...
1. Inspect model configuration or submit a bounded diagnostic prompt/simulation batch.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | The observable outcome is exactly `Inspect model configuration or submit a bounded diagnostic prompt/simulation batch.` The applicable handler, request/response type, status, and authorization evidence are those enumerated in paired TDS Sections 2, 9, 10, and 16; no additional state is claimed. | `SRS UC-AD-21 Normal Flow 2; 05_Development/CareBridgeAITriageService/app/api/v1/chat.py` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
The contract differs from the cited expected result (The observable outcome is exactly `Inspect model configuration or submit a bounded diagnostic prompt/simulation batch.` The applicable handler, request/response type, status, and authorization evidence are those enumerated in paired TDS Sections 2, 9, 10, and 16; no additional state is claimed.), mutates data on a rejected/read path, leaks protected data, or triggers duplicate/unauthorized side effects.
...
### UC-AD-21-TC-003 — Review actual versus expected results without promoting them to production guarantees

| Field | Specification |
| --- | --- |
| Stable ID | `UC-AD-21-TC-003` |
| Severity | `High` |
| Test Condition | `COND-03` |
| Test Level | `Unit / Integration / Contract / applicable UI` |
| Platform / Layer | `Python AI Service / FastAPI Swagger` |
| Technique | `Requirement coverage + state/authorization partition as applicable` |
| Oracle Source | `SRS UC-AD-21 Normal Flow 3; 05_Development/CareBridgeAITriageService/app/api/v1/metrics.py` |
| Preconditions | Synthetic authorized technical operator; configured internal API key; isolated AI-service fixtures |
| Intended Test File | `05_Development/CareBridgeAITriageService/tests/test_metrics_screening.py` |
| Initial Status | `🔴 Not written / existing evidence must be rerun` |
...
1. Review actual versus expected results without promoting them to production guarantees.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | The observable outcome is exactly `Review actual versus expected results without promoting them to production guarantees.` The applicable handler, request/response type, status, and authorization evidence are those enumerated in paired TDS Sections 2, 9, 10, and 16; no additional state is claimed. | `SRS UC-AD-21 Normal Flow 3; 05_Development/CareBridgeAITriageService/app/api/v1/metrics.py` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
The contract differs from the cited expected result (The observable outcome is exactly `Review actual versus expected results without promoting them to production guarantees.` The applicable handler, request/response type, status, and authorization evidence are those enumerated in paired TDS Sections 2, 9, 10, and 16; no additional state is claimed.), mutates data on a rejected/read path, leaks protected data, or triggers duplicate/unauthorized side effects.
...
### UC-AD-21-TC-API-001 — GET /api/v1/chat/models → list_available_models contract

| Field | Specification |
| --- | --- |
| Stable ID | `UC-AD-21-TC-API-001` |
| Severity | `High` |
| Test Condition | `COND-API-001` |
| Test Level | `Unit / Integration / Contract / applicable UI` |
| Platform / Layer | `Python AI Service / FastAPI Swagger` |
| Technique | `Requirement coverage + state/authorization partition as applicable` |
| Oracle Source | `05_Development/CareBridgeAITriageService/app/api/v1/chat.py` |
| Preconditions | Synthetic authorized technical operator; configured internal API key; isolated AI-service fixtures |
| Intended Test File | `05_Development/CareBridgeAITriageService/tests/test_api_endpoints.py` |
| Initial Status | `🔴 Not written / existing evidence must be rerun` |
...
1. Invoke `GET /api/v1/chat/models` so `list_available_models` receives dependency `_auth`: `str`; satisfy `Internal API key via `verify_internal_api_key` dependency` and the extracted `None` constraints.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | Success status `200` returns `dict` with payload fields Not applicable or primitive/framework-managed payload; the request body is `None` with Not applicable — no request body. | `05_Development/CareBridgeAITriageService/app/api/v1/chat.py` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
### UC-AD-21-TC-API-002 — POST /api/v1/chat/test-prompt → test_custom_prompt contract

| Field | Specification |
| --- | --- |
| Stable ID | `UC-AD-21-TC-API-002` |
| Severity | `High` |
...
1. Invoke `POST /api/v1/chat/test-prompt` so `test_custom_prompt` receives body `request`: `CustomPromptTestRequest`; dependency `_auth`: `str`; satisfy `Internal API key via `verify_internal_api_key` dependency` and the extracted `CustomPromptTestRequest` constraints.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | Success status `200` returns `CustomPromptTestResponse` with payload fields `model_used`: `str` (`required`); `temperature`: `float` (`required`); `answer`: `str` (`required`); `generated_at`: `datetime` (`Field(default_factory=datetime.utcnow)`); the request body is `CustomPromptTestRequest` with `user_message`: `str` (`required`); `system_instruction`: `Optional[str]` (`Field(default=None, description="System prompt tùy chỉnh để thử nghiệm")`); `temperature`: `Optional[float]` (`Field(default=0.3, ge=0.0, le=1.0)`); `model`: `Optional[str]` (`Field(default=None, description="Model muốn thử (gemini-flash-lite-latest, gemini-2.5-flash, gemini-3.7-flash)")`). | `05_Development/CareBridgeAITriageService/app/api/v1/chat.py` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
The contract differs from the cited expected result (Success status `200` returns `CustomPromptTestResponse` with payload fields `model_used`: `str` (`required`); `temperature`: `float` (`required`); `answer`: `str` (`required`); `generated_at`: `datetime` (`Field(default_factory=datetime.utcnow)`); the request body is `CustomPromptTestRequest` with `user_message`: `str` (`required`); `system_instruction`: `Optional[str]` (`Field(default=None, description="System prompt tùy chỉnh để thử nghiệm")`); `temperature`: `Optional[float]` (`Field(default=0.3, ge=0.0, le=1.0)`); `model`: `Optional[str]` (`Field(default=None, description="Model muốn thử (gemini-flash-lite-latest, gemini-2.5-flash, gemini-3.7-flash)")`).), mutates data on a rejected/read path, leaks protected data, or triggers duplicate/unauthorized side effects.
...
### UC-AD-21-TC-API-003 — POST /api/v1/metrics/simulate-batch → simulate_clinical_cases contract

| Field | Specification |
| --- | --- |
| Stable ID | `UC-AD-21-TC-API-003` |
| Severity | `High` |
...
1. Invoke `POST /api/v1/metrics/simulate-batch` so `simulate_clinical_cases` receives dependency `_auth`: `str`; dependency `db`: `AsyncSession`; satisfy `Internal API key via `verify_internal_api_key` dependency` and the extracted `None` constraints.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | Success status `200` returns `BatchSimulationResponse` with payload fields `total_cases`: `int` (`required`); `passed_cases`: `int` (`required`); `all_passed`: `bool` (`required`); `results`: `List[BatchSimulationCase]` (`required`); the request body is `None` with Not applicable — no request body. | `05_Development/CareBridgeAITriageService/app/api/v1/metrics.py` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
### UC-AD-21-TC-BR-001 — Enforce business rule 1

| Field | Specification |
| --- | --- |
| Stable ID | `UC-AD-21-TC-BR-001` |
| Severity | `High` |
...
1. Exercise the eligible and ineligible partitions for this rule: Diagnostic endpoints are operational tools, not consumer clinical flows.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | The server enforces: Diagnostic endpoints are operational tools, not consumer clinical flows. No disallowed state or protected data is produced. | `TDS BR-01; 05_Development/CareBridgeAITriageService/app/services/metrics_screening_service.py` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
The contract differs from the cited expected result (The server enforces: Diagnostic endpoints are operational tools, not consumer clinical flows. No disallowed state or protected data is produced.), mutates data on a rejected/read path, leaks protected data, or triggers duplicate/unauthorized side effects.
...
### UC-AD-21-TC-BR-002 — Enforce business rule 2

| Field | Specification |
| --- | --- |
| Stable ID | `UC-AD-21-TC-BR-002` |
| Severity | `High` |
...
1. Exercise the eligible and ineligible partitions for this rule: Historical counts, latency, uptime, or accuracy are not current requirements unless rerun and dated.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | The server enforces: Historical counts, latency, uptime, or accuracy are not current requirements unless rerun and dated. No disallowed state or protected data is produced. | `TDS BR-02; 05_Development/CareBridgeAITriageService/app/services/metrics_screening_service.py` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
The contract differs from the cited expected result (The server enforces: Historical counts, latency, uptime, or accuracy are not current requirements unless rerun and dated. No disallowed state or protected data is produced.), mutates data on a rejected/read path, leaks protected data, or triggers duplicate/unauthorized side effects.
...
### UC-AD-21-TC-SEC-001 — Reject wrong authentication, role, ownership, membership, or consent scope

| Field | Specification |
| --- | --- |
| Stable ID | `UC-AD-21-TC-SEC-001` |
| Severity | `Critical` |
| Test Condition | `COND-AUTH` |
| Test Level | `Unit / Integration / Contract / applicable UI` |
| Platform / Layer | `Python AI Service / FastAPI Swagger` |
| Technique | `Requirement coverage + state/authorization partition as applicable` |
| Oracle Source | `TDS Sections 4 and 16; 05_Development/CareBridgeAITriageService/app/api/v1/chat.py` |
| Preconditions | Synthetic authorized technical operator; configured internal API key; isolated AI-service fixtures |
| Intended Test File | `05_Development/CareBridgeAITriageService/tests/test_api_endpoints.py` |
| Initial Status | `🔴 Not written / existing evidence must be rerun` |
...
| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | The request fails closed using the exact current mapped status/code and returns no protected resource fields or side effects. | `TDS Sections 4 and 16; 05_Development/CareBridgeAITriageService/app/api/v1/chat.py` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
### UC-AD-21-TC-GAP-001 — Prove the current actor entry path and owning contract remain reachable

| Field | Specification |
| --- | --- |
| Stable ID | `UC-AD-21-TC-GAP-001` |
| Severity | `Medium` |
| Test Condition | `COND-GAP` |
| Test Level | `Unit / Integration / Contract / applicable UI` |
| Platform / Layer | `Python AI Service / FastAPI Swagger` |
| Technique | `Requirement coverage + state/authorization partition as applicable` |
| Oracle Source | `SRS UC-AD-21 Implemented Entry Points/Contracts; 05_Development/CareBridgeAITriageService/app/api/v1/chat.py` |
| Preconditions | Synthetic authorized technical operator; configured internal API key; isolated AI-service fixtures |
| Intended Test File | `05_Development/CareBridgeAITriageService/tests/test_api_endpoints.py` |
| Initial Status | `🔴 Not written / existing evidence must be rerun` |
...
1. Start from `FastAPI Swagger `/docs`; no dedicated Web Admin route`, perform `Authenticate with the configured internal operator key.`, and observe the owning contract `POST `/api/v1/chat/test-prompt`` rather than a retired or unrelated route.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | The current entry path remains reachable for `Run AI Diagnostic and Clinical Simulators` and invokes only the documented owning contract; an unreachable, static, or wrong-method path fails this case. | `SRS UC-AD-21 Implemented Entry Points/Contracts; 05_Development/CareBridgeAITriageService/app/api/v1/chat.py` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
| --- | --- | --- |
| Authenticate with the configured internal operator key. | `COND-01` | `UC-AD-21-TC-001` |
| Inspect model configuration or submit a bounded diagnostic prompt/simulation batch. | `COND-02` | `UC-AD-21-TC-002` |
| Review actual versus expected results without promoting them to production guarantees. | `COND-03` | `UC-AD-21-TC-003` |
| GET /api/v1/chat/models → list_available_models contract | `COND-API-001` | `UC-AD-21-TC-API-001` |
| POST /api/v1/chat/test-prompt → test_custom_prompt contract | `COND-API-002` | `UC-AD-21-TC-API-002` |
| POST /api/v1/metrics/simulate-batch → simulate_clinical_cases contract | `COND-API-003` | `UC-AD-21-TC-API-003` |
| Business-rule partitions | `COND-BR-*` | `UC-AD-21-TC-BR-*` |
| Authentication / authorization / ownership / consent | `COND-AUTH` | `UC-AD-21-TC-SEC-001` |
| Current gap / reachability boundary | `COND-GAP` | `UC-AD-21-TC-GAP-001` |

## 5. Red-Green-Refactor Tracker

| TC ID | Intended file | Red evidence | Green evidence | Refactor verification | Status |
| --- | --- | --- | --- | --- | --- |
| `UC-AD-21-TC-001` | `05_Development/CareBridgeAITriageService/tests/test_api_endpoints.py` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-AD-21-TC-002` | `05_Development/CareBridgeAITriageService/tests/test_api_endpoints.py` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-AD-21-TC-003` | `05_Development/CareBridgeAITriageService/tests/test_api_endpoints.py` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-AD-21-TC-API-001` | `05_Development/CareBridgeAITriageService/tests/test_api_endpoints.py` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-AD-21-TC-API-002` | `05_Development/CareBridgeAITriageService/tests/test_api_endpoints.py` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-AD-21-TC-API-003` | `05_Development/CareBridgeAITriageService/tests/test_api_endpoints.py` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-AD-21-TC-BR-001` | `05_Development/CareBridgeAITriageService/tests/test_api_endpoints.py` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-AD-21-TC-BR-002` | `05_Development/CareBridgeAITriageService/tests/test_api_endpoints.py` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-AD-21-TC-SEC-001` | `05_Development/CareBridgeAITriageService/tests/test_api_endpoints.py` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-AD-21-TC-GAP-001` | `05_Development/CareBridgeAITriageService/tests/test_api_endpoints.py` | Not run | Not run | Not run | 🔴 Not written / rerun required |
...
| Generic test matrix | Case titles/actions reference `Run AI Diagnostic and Clinical Simulators` operations and rules. | Pass |
