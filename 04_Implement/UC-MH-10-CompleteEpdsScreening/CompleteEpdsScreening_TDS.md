...
# TECHNICAL DESIGN SPECIFICATION — Complete and Review EPDS Screening

| Field | Value |
| --- | --- |
| Document ID | `UC-MH-10-TDS` |
| Version | `0.1` |
| Date | `2026-08-23` |
| Status | `Draft` |
| Function ID | `UC-MH-10` |
| Canonical Use Case | `UC-MH-10 — Complete and Review EPDS Screening` |
...
| Reviewer / Approver |  |
| Source Baseline | Current worktree on `2026-08-23`; SRS `UC-MH-10`; exact evidence in Section 1.4 |
...
- **Goal:** Answer the EPDS questionnaire, store the computed screening metric, and follow the implemented severity/safety response.
- **Trigger:** The actor enters EPDS screen nested from maternal metric surfaces.
- **Outcome:** Display the server severity guidance and supported next action.
...
- EPDS screen nested from maternal metric surfaces
...
| Postcondition | Display the server severity guidance and supported next action. |
...
| --- | --- | --- | --- | --- |
| `SRC-SRS-01` | Requirement | `02_Requirements/SRS/Report3_Functional_Specifications.md` — UC-MH-10 | 2026-08-23 | Draft code-first requirement |
...
| --- | --- | --- | --- | --- |
| `UC-MH-10-FR-01` | Complete all required EPDS responses. | `02_Requirements/SRS/Report3_Functional_Specifications.md` — UC-MH-10 Normal Flow 1 | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/health/controller/JourneyMetricController.java` | `COND-01` / `UC-MH-10-TC-001` |
| `UC-MH-10-FR-02` | Compute and submit the score plus safety-relevant answer state. | `02_Requirements/SRS/Report3_Functional_Specifications.md` — UC-MH-10 Normal Flow 2 | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/health/controller/JourneyMetricController.java` | `COND-02` / `UC-MH-10-TC-002` |
| `UC-MH-10-FR-03` | Display the server severity guidance and supported next action. | `02_Requirements/SRS/Report3_Functional_Specifications.md` — UC-MH-10 Normal Flow 3 | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/health/controller/JourneyMetricController.java` | `COND-03` / `UC-MH-10-TC-003` |
| `BR-01` | EPDS is screening, not diagnosis. | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/health/policy/EpdsSeverityPolicy.java` | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/health/policy/EpdsSeverityPolicy.java` | `COND-BR-01` / `UC-MH-10-TC-BR-001` |
| `BR-02` | The self-harm item follows the deterministic safety-event floor even if the total score is lower. | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/health/policy/EpdsSeverityPolicy.java` | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/health/policy/EpdsSeverityPolicy.java` | `COND-BR-02` / `UC-MH-10-TC-BR-002` |

## 3. Architecture Decision Records (ADR)

### ADR-UC-MH-10-01 — Use a distinct actor-goal boundary

| Item | Decision |
| --- | --- |
| Context | The retired 43-UC catalogue grouped multiple triggers, lifecycles, and permission boundaries, making implementation/test traceability generic. |
| Options | Keep the broad catalogue; split by screen; split by actor goal plus lifecycle/security boundary. |
| Decision | Use `UC-MH-10 — Complete and Review EPDS Screening` as the canonical boundary because its operations share the stated actor outcome and current implementation evidence. |
| Consequences | Related supporting screens/endpoints stay in one TDS; different lifecycle/actor outcomes have separate UCs. |
| Source / Status | SRS Section 3.1 and current code audit / Draft |

### ADR-UC-MH-10-02 — Preserve unknowns as Open
...
| --- | --- | --- |
| `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/health/controller/JourneyMetricController.java` | Reuse | Current implementation evidence for Complete and Review EPDS Screening; inspect the exact symbol before implementation changes. |
| `05_Development/CareBridgeMobileApp/lib/features/healthRecords/screens/epds_screen.dart` | Reuse | Current implementation evidence for Complete and Review EPDS Screening; inspect the exact symbol before implementation changes. |
| `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/health/policy/EpdsSeverityPolicy.java` | Reuse | Current implementation evidence for Complete and Review EPDS Screening; inspect the exact symbol before implementation changes. |
...
class epds_screen as "epds_screen.dart"
JourneyMetricController --> epds_screen
class EpdsSeverityPolicy as "EpdsSeverityPolicy.java"
epds_screen --> EpdsSeverityPolicy
@enduml
```
...
participant Client
participant Domain
Actor -> Client: Enter Complete and Review EPDS Screening
Client -> Domain: Complete all required EPDS responses.
Domain --> Client: Result for step 1
Client -> Domain: Compute and submit the score plus safety-relevant answer state.
Domain --> Client: Result for step 2
Client -> Domain: Display the server severity guidance and supported next action.
...
| --- | --- | --- |
| Cancel before mutation | No unintended write or provider side effect. | SRS UC-MH-10 Alternative Flow |
...
@startuml
[*] --> Eligible
Eligible --> InProgress : Complete all required EPDS responses.
InProgress --> Outcome : Display the server severity guidance and supported next action.
...
- EPDS is screening, not diagnosis.
- The self-harm item follows the deterministic safety-event floor even if the total score is lower.
...
| ---: | --- | --- | --- |
| 1 | EPDS screen nested from maternal metric surfaces | Mother | Reachable current entry point |
...
| `05_Development/CareBridgeMobileApp/lib/features/healthRecords/screens/epds_screen.dart` | Support the mapped operations without broadening authorization or lifecycle semantics. |
| `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/health/policy/EpdsSeverityPolicy.java` | Support the mapped operations without broadening authorization or lifecycle semantics. |
...
| Positive test mapping | `COND-API-001` / `UC-MH-10-TC-API-001` |
...
| Positive test mapping | `COND-API-002` / `UC-MH-10-TC-API-002` |
| Negative test mapping | `COND-API-002-VAL` / `UC-MH-10-TC-API-002-VAL`; plus `COND-AUTH` for protected access |
...
| Explicit/documented statuses | `200` |
| Positive test mapping | `COND-API-003` / `UC-MH-10-TC-API-003` |
| Negative test mapping | `COND-AUTH`; DTO/service-only negative paths require their exact cited oracle |
...
| Response payload fields | `metricId`: `UUID` (no field annotation in current DTO); `journeyId`: `UUID` (no field annotation in current DTO); `metricType`: `String` (no field annotation in current DTO); `valueNumeric`: `BigDecimal` (no field annotation in current DTO); `valueSecondary`: `BigDecimal` (no field annotation in current DTO); `unit`: `String` (no field annotation in current DTO); `measuredAt`: `Instant` (no field annotation in current DTO); `sourceType`: `String` (no field annotation in current DTO); `note`: `String` (no field annotation in current DTO); `createdAt`: `Instant` (no field annotation in current DTO); `updatedAt`: `Instant` (no field annotation in current DTO); `context`: `Map<String, Object>` (no field annotation in current DTO); `periodStart`: `Instant` (no field annotation in current DTO); `periodEnd`: `Instant` (no field annotation in current DTO); `qualityLabel`: `String` (no field annotation in current DTO); `disclaimer`: `String` (no field annotation in current DTO); `definitionVersion`: `Integer` (no field annotation in current DTO); `aiInsight`: `String` (no field annotation in current DTO); `redFlagAlert`: `boolean` (no field annotation in current DTO) |
| Explicit/documented statuses | `200` |
| Positive test mapping | `COND-API-004` / `UC-MH-10-TC-API-004` |
| Negative test mapping | `COND-API-004-VAL` / `UC-MH-10-TC-API-004-VAL`; plus `COND-AUTH` for protected access |
...
| --- | --- | --- | --- |
| `VG-01` | Complete all required EPDS responses. | `COND-01` | `UC-MH-10-TC-001` |
| `VG-02` | Compute and submit the score plus safety-relevant answer state. | `COND-02` | `UC-MH-10-TC-002` |
| `VG-03` | Display the server severity guidance and supported next action. | `COND-03` | `UC-MH-10-TC-003` |
| `VG-AUTH` | Reject wrong authentication/role/ownership/membership/consent scope | `COND-AUTH` | `UC-MH-10-TC-SEC-001` |
| `VG-GAP` | Characterize each known gap without claiming a false completed path | `COND-GAP` | `UC-MH-10-TC-GAP-001` |
...
- `cd 05_Development/CareBridgeAPI && ./mvnw -Dtest=HealthMetricServiceEpdsEventTest test`
- `cd 05_Development/CareBridgeAPI && ./mvnw -Dtest=EpdsSeverityPolicyTest test`
- `cd 05_Development/CareBridgeMobileApp && flutter test test/features/healthRecords/epds_screen_test.dart`
