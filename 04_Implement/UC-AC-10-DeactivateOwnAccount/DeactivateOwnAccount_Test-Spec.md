...
# TEST SPECIFICATION — Deactivate Own Account

| Field | Value |
| --- | --- |
| Document ID | `UC-AC-10-TEST-SPEC` |
| Version | `0.1` |
| Date | `2026-08-23` |
| Status | `Draft` |
| Feature / Gap ID | `UC-AC-10` |
| Module | `Access, Identity, and Trust` |
| Paired TDS | `UC-AC-10-TDS` |
| Priority | `High` |
...
| Reviewer / Approver |  |
| Source Baseline | Current worktree `2026-08-23`; SRS/TDS `UC-AC-10` and exact code/test sources below |
...
| --- | --- | --- |
| Actor goal | Deactivate the authenticated account through the supported confirmation flow. | SRS `UC-AC-10` |
| Current state | `High` confidence; gaps are listed in Section 2 | Exact current code/test sources below |
| Entry points | Mobile `/profile` to nested Deactivate Account screen | Current client/router evidence |
...
| Primary operations | Open the deactivation confirmation flow.; Confirm the authenticated account action.; Deactivate the account and terminate access according to the server policy. | SRS `UC-AC-10` Normal Flow |
...
| --- | --- | --- | --- | --- |
| `SRC-SRS` | Requirement | `02_Requirements/SRS/Report3_Functional_Specifications.md` — UC-AC-10 | 2026-08-23 | Draft code-first requirement |
| `SRC-TDS` | Design | Paired `UC-AC-10-TDS` | 0.1 | Draft design |
...
| `SRC-CODE-02` | Current code | `05_Development/CareBridgeMobileApp/lib/features/auth/screens/deactivate_account_screen.dart` | Worktree `2026-08-23` | Current-state evidence |
| `SRC-TEST-01` | Existing test | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/security/AuthServiceDeactivateTest.java` | Worktree `2026-08-23` | Current-state evidence |
...
| --- | --- | --- | --- | --- | --- |
| `LI-01` | No focused Mobile widget test was found. | Could create false completed coverage or wrong contract | Keep as current limitation/Open until the cited client/server mismatch is resolved | SRS `UC-AC-10` gap plus current code | Open |
...
| --- | --- | --- | --- | --- | --- | --- |
| `RISK-01` | Failure of: Open the deactivation confirmation flow. | High | Medium | High | Unit/Integration/Contract/applicable UI | `COND-01` |
| `RISK-02` | Failure of: Confirm the authenticated account action. | High | Medium | High | Unit/Integration/Contract/applicable UI | `COND-02` |
| `RISK-03` | Failure of: Deactivate the account and terminate access according to the server policy. | High | Medium | High | Unit/Integration/Contract/applicable UI | `COND-03` |
...
| --- | --- | --- | --- | --- |
| `BASIS-01` | `UC-AC-10-FR-01` — Open the deactivation confirmation flow. | SRS `UC-AC-10` Normal Flow 1; TDS Section 2 | Open the deactivation confirmation flow. | `COND-01` |
| `BASIS-02` | `UC-AC-10-FR-02` — Confirm the authenticated account action. | SRS `UC-AC-10` Normal Flow 2; TDS Section 2 | Confirm the authenticated account action. | `COND-02` |
| `BASIS-03` | `UC-AC-10-FR-03` — Deactivate the account and terminate access according to the server policy. | SRS `UC-AC-10` Normal Flow 3; TDS Section 2 | Deactivate the account and terminate access according to the server policy. | `COND-03` |
...
| --- | --- | --- | --- | --- | --- |
| `COND-01` | `BASIS-01` / `RISK-01` | Open the deactivation confirmation flow. | Mobile / Backend | Positive + applicable boundary/state coverage | `UC-AC-10-TC-001` |
| `COND-02` | `BASIS-02` / `RISK-02` | Confirm the authenticated account action. | Mobile / Backend | Positive + applicable boundary/state coverage | `UC-AC-10-TC-002` |
| `COND-03` | `BASIS-03` / `RISK-03` | Deactivate the account and terminate access according to the server policy. | Mobile / Backend | Positive + applicable boundary/state coverage | `UC-AC-10-TC-003` |
| `COND-API-001` | Exact handler/client composition contract | DELETE /api/v1/auth/deactivate → deactivate contract | Applicable backend/client contract layer | Contract + DTO/status/authorization evidence | `UC-AC-10-TC-API-001` |
| `COND-API-001-VAL` | Exact handler/client composition contract | DELETE /api/v1/auth/deactivate rejects a declared request-field boundary | Applicable backend/client contract layer | Contract + DTO/status/authorization evidence | `UC-AC-10-TC-API-001-VAL` |
| `COND-BR-01` | `BR-01` | Only the authenticated account may initiate self-deactivation. | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/security/controller/AuthController.java` | Negative / decision / state | `UC-AC-10-TC-BR-001` |
| `COND-BR-02` | `BR-02` | Deactivation is not a client-only logout and must use the server lifecycle. | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/security/controller/AuthController.java` | Negative / decision / state | `UC-AC-10-TC-BR-002` |
| `COND-AUTH` | `RISK-AUTH` | Reject wrong authentication, role, ownership, membership, consent, or state scope | All protected layers | Security | `UC-AC-10-TC-SEC-001` |
| `COND-GAP` | `RISK-GAP` | Characterize current limitation/reachability without false completion | Applicable layer | Gap/Regression | `UC-AC-10-TC-GAP-001` |
...
### UC-AC-10-TC-001 — Open the deactivation confirmation flow

| Field | Specification |
| --- | --- |
| Stable ID | `UC-AC-10-TC-001` |
| Severity | `High` |
...
| Oracle Source | `SRS UC-AC-10 Normal Flow 1; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/security/controller/AuthController.java` |
| Preconditions | Synthetic `Authenticated User` identity; eligible current resource state; isolated provider/database fixtures |
| Intended Test File | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/security/AuthServiceDeactivateTest.java` |
...
1. Open the deactivation confirmation flow.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | The observable outcome is exactly `Open the deactivation confirmation flow.` The applicable handler, request/response type, status, and authorization evidence are those enumerated in paired TDS Sections 2, 9, 10, and 16; no additional state is claimed. | `SRS UC-AC-10 Normal Flow 1; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/security/controller/AuthController.java` |
...
| UI state | Reachable client displays the canonical outcome/error without false success. | SRS `UC-AC-10` entry point and current client evidence |
...
The contract differs from the cited expected result (The observable outcome is exactly `Open the deactivation confirmation flow.` The applicable handler, request/response type, status, and authorization evidence are those enumerated in paired TDS Sections 2, 9, 10, and 16; no additional state is claimed.), mutates data on a rejected/read path, leaks protected data, or triggers duplicate/unauthorized side effects.
...
### UC-AC-10-TC-002 — Confirm the authenticated account action

| Field | Specification |
| --- | --- |
| Stable ID | `UC-AC-10-TC-002` |
| Severity | `High` |
| Test Condition | `COND-02` |
| Test Level | `Unit / Integration / Contract / applicable UI` |
| Platform / Layer | `Mobile / Backend` |
| Technique | `Requirement coverage + state/authorization partition as applicable` |
| Oracle Source | `SRS UC-AC-10 Normal Flow 2; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/security/controller/AuthController.java` |
| Preconditions | Synthetic `Authenticated User` identity; eligible current resource state; isolated provider/database fixtures |
| Intended Test File | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/security/AuthServiceDeactivateTest.java` |
| Initial Status | `🔴 Not written / existing evidence must be rerun` |
...
1. Confirm the authenticated account action.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | The observable outcome is exactly `Confirm the authenticated account action.` The applicable handler, request/response type, status, and authorization evidence are those enumerated in paired TDS Sections 2, 9, 10, and 16; no additional state is claimed. | `SRS UC-AC-10 Normal Flow 2; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/security/controller/AuthController.java` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
The contract differs from the cited expected result (The observable outcome is exactly `Confirm the authenticated account action.` The applicable handler, request/response type, status, and authorization evidence are those enumerated in paired TDS Sections 2, 9, 10, and 16; no additional state is claimed.), mutates data on a rejected/read path, leaks protected data, or triggers duplicate/unauthorized side effects.
...
### UC-AC-10-TC-003 — Deactivate the account and terminate access according to the server policy

| Field | Specification |
| --- | --- |
| Stable ID | `UC-AC-10-TC-003` |
| Severity | `High` |
| Test Condition | `COND-03` |
| Test Level | `Unit / Integration / Contract / applicable UI` |
| Platform / Layer | `Mobile / Backend` |
| Technique | `Requirement coverage + state/authorization partition as applicable` |
| Oracle Source | `SRS UC-AC-10 Normal Flow 3; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/security/controller/AuthController.java` |
| Preconditions | Synthetic `Authenticated User` identity; eligible current resource state; isolated provider/database fixtures |
| Intended Test File | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/security/AuthServiceDeactivateTest.java` |
| Initial Status | `🔴 Not written / existing evidence must be rerun` |
...
1. Deactivate the account and terminate access according to the server policy.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | The observable outcome is exactly `Deactivate the account and terminate access according to the server policy.` The applicable handler, request/response type, status, and authorization evidence are those enumerated in paired TDS Sections 2, 9, 10, and 16; no additional state is claimed. | `SRS UC-AC-10 Normal Flow 3; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/security/controller/AuthController.java` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
The contract differs from the cited expected result (The observable outcome is exactly `Deactivate the account and terminate access according to the server policy.` The applicable handler, request/response type, status, and authorization evidence are those enumerated in paired TDS Sections 2, 9, 10, and 16; no additional state is claimed.), mutates data on a rejected/read path, leaks protected data, or triggers duplicate/unauthorized side effects.
...
### UC-AC-10-TC-API-001 — DELETE /api/v1/auth/deactivate → deactivate contract

| Field | Specification |
| --- | --- |
| Stable ID | `UC-AC-10-TC-API-001` |
| Severity | `High` |
...
1. Invoke `DELETE /api/v1/auth/deactivate` so `deactivate` receives principal `principal`: `Principal`; body `request`: `DeactivateRequest`; satisfy `No @PreAuthorize on handler/class; effective access comes from the security chain` and the extracted `DeactivateRequest` constraints.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | Success status `200` returns `ResponseEntity<ApiResponse<Void>>` with payload fields Not applicable or unresolved from the handler import; the request body is `DeactivateRequest` with `confirmPassword`: `String` (@NotBlank(message = "Password confirmation is required")); `reason`: `String` (@Size(max = 1000, message = "Reason must be at most 1000 characters")). | `05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/security/controller/AuthController.java` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
The contract differs from the cited expected result (Success status `200` returns `ResponseEntity<ApiResponse<Void>>` with payload fields Not applicable or unresolved from the handler import; the request body is `DeactivateRequest` with `confirmPassword`: `String` (@NotBlank(message = "Password confirmation is required")); `reason`: `String` (@Size(max = 1000, message = "Reason must be at most 1000 characters")).), mutates data on a rejected/read path, leaks protected data, or triggers duplicate/unauthorized side effects.
...
### UC-AC-10-TC-API-001-VAL — DELETE /api/v1/auth/deactivate rejects a declared request-field boundary

| Field | Specification |
| --- | --- |
| Stable ID | `UC-AC-10-TC-API-001-VAL` |
| Severity | `High` |
...
1. Submit `DeactivateRequest` with one field violating its cited validator: `confirmPassword`: `String` (@NotBlank(message = "Password confirmation is required")); `reason`: `String` (@Size(max = 1000, message = "Reason must be at most 1000 characters"))
...
### UC-AC-10-TC-BR-001 — Enforce business rule 1

| Field | Specification |
| --- | --- |
| Stable ID | `UC-AC-10-TC-BR-001` |
| Severity | `High` |
...
1. Exercise the eligible and ineligible partitions for this rule: Only the authenticated account may initiate self-deactivation.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | The server enforces: Only the authenticated account may initiate self-deactivation. No disallowed state or protected data is produced. | `TDS BR-01; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/security/controller/AuthController.java` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
The contract differs from the cited expected result (The server enforces: Only the authenticated account may initiate self-deactivation. No disallowed state or protected data is produced.), mutates data on a rejected/read path, leaks protected data, or triggers duplicate/unauthorized side effects.
...
### UC-AC-10-TC-BR-002 — Enforce business rule 2

| Field | Specification |
| --- | --- |
| Stable ID | `UC-AC-10-TC-BR-002` |
| Severity | `High` |
...
1. Exercise the eligible and ineligible partitions for this rule: Deactivation is not a client-only logout and must use the server lifecycle.

**Assert — observable result**

| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | The server enforces: Deactivation is not a client-only logout and must use the server lifecycle. No disallowed state or protected data is produced. | `TDS BR-02; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/security/controller/AuthController.java` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
### UC-AC-10-TC-SEC-001 — Reject wrong authentication, role, ownership, membership, or consent scope

| Field | Specification |
| --- | --- |
| Stable ID | `UC-AC-10-TC-SEC-001` |
| Severity | `Critical` |
...
### UC-AC-10-TC-GAP-001 — Characterize the current documented limitation

| Field | Specification |
| --- | --- |
| Stable ID | `UC-AC-10-TC-GAP-001` |
| Severity | `High` |
| Test Condition | `COND-GAP` |
| Test Level | `Unit / Integration / Contract / applicable UI` |
| Platform / Layer | `Mobile / Backend` |
| Technique | `Requirement coverage + state/authorization partition as applicable` |
| Oracle Source | `SRS UC-AC-10 Known Gaps / Exclusions; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/security/controller/AuthController.java` |
| Preconditions | Synthetic `Authenticated User` identity; eligible current resource state; isolated provider/database fixtures |
| Intended Test File | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/security/AuthServiceDeactivateTest.java` |
| Initial Status | `🔴 Not written / existing evidence must be rerun` |
...
| Assertion area | Expected result | Oracle Source |
| --- | --- | --- |
| Response / return | The test records the current limitation without inventing a completed path: No focused Mobile widget test was found. | `SRS UC-AC-10 Known Gaps / Exclusions; 05_Development/CareBridgeAPI/src/main/java/com/carebridge/backend/security/controller/AuthController.java` |
| Persistence | Only the state mutation required by this behavior occurs; read/rejection paths assert zero writes. | Paired TDS Sections 5–6 plus the exact handler/service oracle cited by this case; use repository state/spy evidence only when that call path mutates |
| Audit | Feature-owned audit entry only where current code defines it; otherwise `Not applicable — no audited mutation evidenced`. | Paired TDS Sections 7 and 10; current implementation source |
| Event / notification | Only an event/notification explicitly reachable from the cited call path; otherwise `Not applicable — no publisher/listener evidenced`. | Paired TDS Section 7 and current publisher/listener source when present |
| Provider side effect | Exact call count/payload only when the cited call path uses a provider; otherwise `Not applicable — no provider in this operation`. | Current adapter source cited by the paired TDS when present |
...
| --- | --- | --- |
| Open the deactivation confirmation flow. | `COND-01` | `UC-AC-10-TC-001` |
| Confirm the authenticated account action. | `COND-02` | `UC-AC-10-TC-002` |
| Deactivate the account and terminate access according to the server policy. | `COND-03` | `UC-AC-10-TC-003` |
| DELETE /api/v1/auth/deactivate → deactivate contract | `COND-API-001` | `UC-AC-10-TC-API-001` |
| DELETE /api/v1/auth/deactivate rejects a declared request-field boundary | `COND-API-001-VAL` | `UC-AC-10-TC-API-001-VAL` |
| Business-rule partitions | `COND-BR-*` | `UC-AC-10-TC-BR-*` |
| Authentication / authorization / ownership / consent | `COND-AUTH` | `UC-AC-10-TC-SEC-001` |
| Current gap / reachability boundary | `COND-GAP` | `UC-AC-10-TC-GAP-001` |

## 5. Red-Green-Refactor Tracker

| TC ID | Intended file | Red evidence | Green evidence | Refactor verification | Status |
| --- | --- | --- | --- | --- | --- |
| `UC-AC-10-TC-001` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/security/AuthServiceDeactivateTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-AC-10-TC-002` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/security/AuthServiceDeactivateTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-AC-10-TC-003` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/security/AuthServiceDeactivateTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-AC-10-TC-API-001` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/security/AuthServiceDeactivateTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-AC-10-TC-API-001-VAL` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/security/AuthServiceDeactivateTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-AC-10-TC-BR-001` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/security/AuthServiceDeactivateTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-AC-10-TC-BR-002` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/security/AuthServiceDeactivateTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-AC-10-TC-SEC-001` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/security/AuthServiceDeactivateTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
| `UC-AC-10-TC-GAP-001` | `05_Development/CareBridgeAPI/src/test/java/com/carebridge/backend/security/AuthServiceDeactivateTest.java` | Not run | Not run | Not run | 🔴 Not written / rerun required |
...
| Generic test matrix | Case titles/actions reference `Deactivate Own Account` operations and rules. | Pass |
