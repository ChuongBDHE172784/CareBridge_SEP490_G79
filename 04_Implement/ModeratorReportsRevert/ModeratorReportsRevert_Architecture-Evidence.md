> [!IMPORTANT]
> Historical resolved-report subflow evidence for `UC-AD-17`; this is not a canonical current TDS. Current code and the canonical code-first specification override conflicts.
...
| 2026-07-22 | AI Agent — Amelia (Dev Agent, session CommunityTopicManagement) | **Đổi tên file migration** (không đổi nội dung SQL): `V20260720100000__add_content_report_revert_columns.sql` → `V20260720100001__add_content_report_revert_columns.sql`. Lý do: phát hiện trùng version `20260720100000` với `V20260720100000__secure_baby_journey_linkage.sql` (một migration khác không liên quan), khiến Flyway từ chối validate và chặn TOÀN BỘ Testcontainers integration test trong dự án. User xác nhận qua AskUserQuestion. An toàn với DB dev chung: theo ghi chú ngày 2026-07-20 ở trên, migration này chưa từng chạy qua Flyway trên DB đó (`flyway.enabled=false`, 2 cột được áp bằng `hibernate.ddl-auto=update`) nên đổi tên không tạo lệch `flyway_schema_history` ở đó. Chi tiết đầy đủ ở `04_Implement/CommunityTopicManagement/CommunityTopicManagement_TDS.md` CHANGELOG cùng ngày. |
...
> Xem file `ModeratorReportsRevert_Verification-Evidence.md` (theo `PHASE-4_Test-Spec.md` template) cho toàn bộ test case chi tiết (`MRR-TC-xxx`).
