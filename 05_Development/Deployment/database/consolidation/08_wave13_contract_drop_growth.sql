...
BEGIN
    ...
    FOR v_missing IN
        SELECT column_name FROM unnest(ARRAY['measurement_group_id', 'deleted_at']) AS column_name
         WHERE NOT EXISTS (
            SELECT 1 FROM information_schema.columns
             WHERE table_schema = 'public' AND table_name = 'health_observations'
               AND columns.column_name = unnest.column_name)
    LOOP
        RAISE EXCEPTION 'WAVE13_REGRESSION: health_observations.% was removed', v_missing;
    END LOOP;
