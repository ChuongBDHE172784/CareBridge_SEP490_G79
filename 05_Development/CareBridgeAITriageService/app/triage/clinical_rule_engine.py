...
from app.triage.state import matched_rule_trace_to_audit
...
def clinical_rule_engine(
    ...
    provisional_questions, provisional_fields = _pediatric_yellow_follow_up(
        state, result.decisive_rule_ids, outcome
    )
    if provisional_questions:
        # This is an information-sufficiency hold, not a clinical threshold change: the
        # deterministic rule remains matched and auditable, but a vague paediatric symptom
        # must collect the rule's own temperature/hydration questions before public release.
        outcome = "NEEDS_MORE_INFO"
    ...
    return {
        ...
        "candidateQuestionIds": list(provisional_questions or result.question_ids),
        "plannedQuestionIds": [],
...
def _pediatric_yellow_follow_up(
    state: Mapping[str, object], decisive_rule_ids: object, outcome: object
) -> tuple[tuple[str, ...], tuple[str, ...]]:
    """Hold the broad paediatric symptom rule until its declared detail questions resolve.

    ``PED_YELLOW_001`` intentionally catches broad fever/diarrhoea/vomiting/rash/cough input.
    A bare fever signal alone is not enough context for a useful public explanation, so the
    existing catalogue asks temperature and hydration. RED gates still run before this helper.
    """

    stage = _value(state.get("stage"))
    rules = set(decisive_rule_ids) if type(decisive_rule_ids) in {list, tuple} else set()
    if (
        outcome != "YELLOW"
        or "PED_YELLOW_001" not in rules
        or stage not in {CareStage.INFANT_0_12M.value, CareStage.TODDLER_12_24M.value}
    ):
        return (), ()

    answered = set(state.get("answeredQuestionIds", []))
    measurements = state.get("measurements") if type(state.get("measurements")) is dict else {}
    signals = _trusted_signals(state.get("signals"))
    questions: list[str] = []
    fields: list[str] = []
    if "Q_BABY_TEMPERATURE" not in answered and "temperatureC" not in measurements:
        questions.append("Q_BABY_TEMPERATURE")
        fields.append("temperature_band")
    hydration_resolved = any(
        signal_presence(signals, code) not in {
            Presence.UNKNOWN,
            Presence.CONFLICTED,
            Presence.UNAWARE_OR_UNMEASURABLE,
        }
        for code in ("DEHYDRATION_SIGNS", "SEVERE_DEHYDRATION")
    )
    if "Q_BABY_HYDRATION" not in answered and not hydration_resolved:
        questions.append("Q_BABY_HYDRATION")
        fields.append("hydration_status")
    return tuple(questions), tuple(fields)
