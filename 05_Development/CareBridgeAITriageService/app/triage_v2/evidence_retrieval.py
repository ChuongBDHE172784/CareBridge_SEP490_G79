    ...
    *, corpus_dir: Path = MEDICAL_SOURCES_DIR, allowed_domains: set[str], target: str,
    ...
    for path in sorted(corpus_dir.glob("*.md")):
        try:
            ...
            stages = tuple(str(item) for item in metadata.get("applicableStages", ()))
            ...
            if target not in targets or stage not in stages or language not in languages:
                continue
...
def retrieve_verified_evidence(
    ...
    ranked = sorted(
        ...
        key=lambda item: (-_vietnam_priority(item[0]), -item[1], item[0].source_id),
    )
