"""Isolated deterministic LangGraph for AI Triage V2."""
...
from app.triage_v2.coverage_resolver import coverage_resolver
...
from app.triage_v2.question_resume import follow_up_interrupt, idempotency_guard, question_planner
from app.triage_v2.renderer_audit import audit_node, deterministic_renderer, green_release_gate
from app.triage_v2.signal_normalizer import signal_normalizer
from app.triage_v2.stage_context_resolver import stage_context_resolver
from app.triage_v2.stage_safety_gate import stage_safety_gate
from app.triage_v2.state import TriageV2State
from app.triage_v2.target_entity_resolver import target_entity_resolver
...
def _terminal(state: TriageV2State) -> str:
    return "terminal" if state.get("stopConversation") is True else "continue"


def _idempotency_route(state: TriageV2State) -> str:
    return (
        "terminal"
...
def _follow_up_route(state: TriageV2State) -> str:
    return (
        "interrupt"
...
def build_triage_v2_graph():
    graph = StateGraph(TriageV2State)
    nodes = {
...
triage_v2_graph = build_triage_v2_graph()
