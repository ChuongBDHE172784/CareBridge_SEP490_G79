...
def extract_reported_measurements(
    ...
    ages = set() if hypothetical else _baby_month_values(message, folded, target)
    temperatures = _temperature_values(message, folded, target)
    ...
    return ReportedMeasurements(measurements, baby_age, frozenset(conflicts))


def _baby_month_values(message: str, folded: str, target: object) -> set[int]:
    values: set[int] = set()
...
def _temperature_values(message: str, folded: str, target: object) -> set[float]:
