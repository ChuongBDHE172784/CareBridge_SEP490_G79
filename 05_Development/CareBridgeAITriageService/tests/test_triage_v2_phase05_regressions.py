...
from app.triage_v2.extraction import ValidatedExtraction
from app.triage_v2.graph import build_triage_v2_graph, graph_config
from app.triage_v2.renderer_audit import deterministic_renderer
...
def _invoke(state):
    return build_triage_v2_graph().invoke(state, graph_config(state["sessionId"]))
...
def test_bug_5_inv_5_llm_observation_conflicts_instead_of_overwriting_answer(monkeypatch):
    ...
    request = api.TriageV2TurnRequest.model_validate({
...
def test_bug_7_v2_extraction_forwards_a_bounded_deadline():
    ...
    probe = Probe()
    probe.extract_triage_v2(text="Tôi bị đau", deadline=123.0)
