...
from app.triage.extraction import TriageExtraction
...
def test_internal_endpoint_is_disabled_without_shared_secret(monkeypatch):
    monkeypatch.setattr(api, "TRIAGE_INTERNAL_API_KEY", "")
    response = client.post("/internal/triage/turn", json=_request())
    assert response.status_code == 503


def test_internal_endpoint_rejects_wrong_secret(monkeypatch):
    monkeypatch.setattr(api, "TRIAGE_INTERNAL_API_KEY", "expected-secret")
...
def test_turn_returns_deterministic_red_and_no_links(monkeypatch):
    ...
    assert state["evidenceStatus"] == "PENDING"
    assert "khó thở" in state["rationale"].lower()
...
def test_persisted_state_identity_cannot_be_swapped(monkeypatch):
    monkeypatch.setattr(api, "TRIAGE_INTERNAL_API_KEY", "expected-secret")
    previous = api.execute_turn(api.TriageTurnRequest.model_validate(_request())).state
    response = client.post(
        "/internal/triage/turn",
        headers={"X-CareBridge-Internal-Key": "expected-secret"},
        json=_request(
...
def test_vague_baby_fever_asks_focused_details_before_yellow(monkeypatch):
    monkeypatch.setattr(api, "TRIAGE_INTERNAL_API_KEY", "expected-secret")
    monkeypatch.setattr(api, "get_gemini_client", lambda: None)
    response = client.post(
        "/internal/triage/turn",
    ...
    assert response.status_code == 200
    state = response.json()["state"]
    assert state["triageOutcome"] == "NEEDS_MORE_INFO"
    assert state["stopConversation"] is False
    assert "Q_BABY_TEMPERATURE" in state["plannedQuestionIds"]
    assert "Q_BABY_HYDRATION" in state["plannedQuestionIds"]
...
def test_grounded_extraction_reenters_deterministic_global_safety(monkeypatch):
    ...
    extraction = TriageExtraction.model_validate({
...
def test_evidence_failure_cannot_erase_completed_yellow(monkeypatch):
    ...
    monkeypatch.setattr(api, "TRIAGE_INTERNAL_API_KEY", "expected-secret")
    monkeypatch.setattr(api, "get_gemini_client", lambda: None)
    monkeypatch.setattr(api, "build_triage_graph", lambda: FakeGraph())
    ...
    assert response.json()["state"]["evidenceStatus"] == "REJECTED"
...
def test_answered_questions_accumulate_without_duplicates():
    ...
    state = api._turn_state(api.TriageTurnRequest(**_request(
    ...
    resumed = api._turn_state(api.TriageTurnRequest(**_request(
...
def test_numeric_and_option_answers_use_independent_ledgers():
    request = api.TriageTurnRequest(**_request(
        signals={},
        answeredQuestionIds=["Q_BABY_AGE_MONTHS", "Q_GLOBAL_DANGER"],
        submittedOptionQuestionIds=["Q_GLOBAL_DANGER"],
        submittedOptionCodes=["DANGER_NONE"],
    ))

    state = api._turn_state(request)

    assert state["answeredQuestionIds"] == ["Q_BABY_AGE_MONTHS", "Q_GLOBAL_DANGER"]
    assert state["submittedOptionQuestionIds"] == ["Q_GLOBAL_DANGER"]
    assert state["submittedOptionCodes"] == ["DANGER_NONE"]
