...
from app.triage.api import TriageTurnRequest, _turn_state
from app.triage.graph import build_triage_graph, graph_config, triage_graph
...
def _invoke(state):
    return triage_graph.invoke(state, graph_config(state["sessionId"]))
...
def test_registry_unavailable_is_rendered_as_controlled_non_green(monkeypatch):
    monkeypatch.setattr(
        "app.triage.global_safety_gate.get_registry",
...
def test_real_resume_enforces_duplicate_and_stale_guards(resume_payload, expected_action):
    ...
    config = graph_config(state["sessionId"])
    triage_graph.invoke(state, config)

    resumed = triage_graph.invoke(Command(resume=resume_payload), config)
...
def test_resume_schema_rejects_checkpoint_and_disposition_overrides(forbidden):
    ...
    with pytest.raises(ValueError, match="INVALID_RESUME_FIELDS"):
        triage_graph.invoke(Command(resume=payload), config)
...
def test_resume_rejects_checkpoint_thread_that_does_not_match_session_identity():
    ...
    triage_graph.invoke(state, mismatched)
...
def test_four_turn_baby_flow_keeps_intent_and_never_repeats_questions():
    ...
    for turn_index, message in enumerate(messages):
        ...
        result = dict(build_triage_graph().invoke(state, graph_config(request.sessionId)))
...
def test_graph_is_isolated_from_legacy_gemini_rag_java_and_db_modules():
    ...
    source = inspect.getsource(inspect.getmodule(triage_graph.__class__))
    graph_source = inspect.getsource(__import__("app.triage.graph", fromlist=["*"]))
    imports = [
    ...
    assert imports and all(
        line.startswith(("from app.triage", "from langgraph", "from __future__"))
        for line in imports
    ), imports
    assert source
