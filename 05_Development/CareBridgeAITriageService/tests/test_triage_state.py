...
import app.triage as triage_v2
...
from app.triage import TriageState, create_initial_state
from app.triage.state import MAXIMUM_QUESTION_ROUNDS, matched_rule_trace_to_audit
...
def test_typed_contract_requires_every_minimum_phase_2_field_without_freezing_extensions():
    ...
    assert EXPECTED_STATE_KEYS <= TriageState.__required_keys__
    assert TriageState.__optional_keys__ == frozenset()
    assert set(_state()) == TriageState.__required_keys__


def test_package_exports_only_the_state_contract_and_initializer():
    assert triage_v2.__all__ == ["TriageState", "create_initial_state"]
