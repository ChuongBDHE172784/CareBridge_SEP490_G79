...
from app.triage.evidence_retrieval import (
...
def test_publisher_identity_without_document_verification_is_rejected(tmp_path):
    write_source(tmp_path)
    path = tmp_path / "SRC_1.md"
    path.write_text(
        path.read_text(encoding="utf-8").replace(
...
def test_canonical_stage_matches_legacy_corpus_stage_alias(tmp_path):
    write_source(tmp_path)
    path = tmp_path / "SRC_1.md"
    path.write_text(
        path.read_text(encoding="utf-8").replace(
            "applicableStages: [PREGNANCY]", "applicableStages: [POSTPARTUM]"
        ),
        encoding="utf-8",
    )
...
def test_repeated_observation_list_keeps_present_signal_in_query(tmp_path):
    write_source(tmp_path, body="SEIZURE co giat GLOBAL_RED_001")
    repeated = state()
    repeated["signals"] = {
        "SEIZURE": [
            {"presence": "UNKNOWN"},
    ...
    citations = retrieve_verified_evidence(
        repeated, allowed_domains={"health.example"}, corpus_dir=tmp_path
    )

    assert [citation["sourceId"] for citation in citations] == ["SRC_1"]
...
def test_v2_retriever_has_no_runtime_web_search_or_agent_loop():
    import app.triage.evidence_retrieval as module
