...
from app.triage_v2.extraction import TriageV2Extraction, extract_and_validate
...
def test_schema_cannot_carry_outcome_action_url_diagnosis_or_treatment():
    for forbidden in ("riskLevel", "triageOutcome", "actionCode", "stopConversation",
        ...
        try:
            TriageV2Extraction.model_validate(candidate)
        except ValueError:
            pass
        else:
...
def test_gemini_v2_uses_json_schema_and_sends_only_bounded_sanitized_text():
    ...
    result = client.extract_triage_v2(text="Tôi là Jane, email jane@example.com. Tôi bị co giật")
...
def test_timeout_invalid_json_empty_and_unsupported_enum_fail_closed(caplog):
    for response in (
        ...
        assert GeminiClient(settings(), Sdk(response)).extract_triage_v2(text="co giật") is None
    ...
    assert GeminiClient(settings(), timeout_sdk).extract_triage_v2(text="co giật") is None
...
def test_transport_schema_drops_keywords_the_model_api_rejects():
    ...
    schema = gemini_transport_schema(TriageV2Extraction)
...
def test_transport_schema_still_describes_the_whole_extraction():
    ...
    assert set(schema["properties"]) == set(TriageV2Extraction.model_fields)
...
def test_transport_schema_never_offers_an_outcome_field():
    ...
    rendered = json.dumps(gemini_transport_schema(TriageV2Extraction))
