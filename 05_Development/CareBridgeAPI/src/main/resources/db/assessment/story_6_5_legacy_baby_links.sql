...
WITH latest_evidence AS (
  SELECT DISTINCT ON (journey_id) journey_id, owner_user_id, outcome_type
  FROM pregnancy_outcome_evidence ORDER BY journey_id, revision_number DESC
), classified AS (
  SELECT CASE
  ...
  LEFT JOIN mother_journeys j ON j.journey_id=b.related_journey_id
