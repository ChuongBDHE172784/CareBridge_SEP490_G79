CREATE TABLE system_configurations (
    ...
    api_rate_limit INTEGER NOT NULL CHECK (
        api_rate_limit BETWEEN 1 AND 100000
    ),
    connection_timeout_ms INTEGER NOT NULL CHECK (
        connection_timeout_ms BETWEEN 1000 AND 300000
    ),
    max_upload_size_mb INTEGER NOT NULL CHECK (
        max_upload_size_mb BETWEEN 1 AND 1024
    ),
