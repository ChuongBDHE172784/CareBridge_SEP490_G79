...
INSERT INTO public.care_tasks (
    task_id,task_type,owner_user_id,care_subject_id,journey_id,baby_id,title,
    scheduled_at,recurrence_rule,recurrence_type,recurrence_end_date,fcm_job_id,
    snoozed_until,status,created_at,updated_at)
SELECT r.reminder_id,'SCHEDULED_REMINDER',r.owner_user_id,cs.care_subject_id,r.journey_id,r.baby_id,
       r.title,r.scheduled_at,r.recurrence_rule,r.recurrence_type,
...
ON CONFLICT (task_id) DO UPDATE SET
  ...
  journey_id=excluded.journey_id,baby_id=excluded.baby_id,
...
BEGIN
  IF to_regclass('public.legacy_care_tasks') IS NOT NULL THEN
      INSERT INTO public.care_tasks (
          task_id,task_type,care_group_id,creator_user_id,assignee_user_id,care_subject_id,title,
          description,scheduled_at,status,completed_at,created_at,updated_at)
      SELECT t.care_task_id,'MANUAL_TASK',t.care_group_id,t.assigned_by,t.assigned_to,cg.care_subject_id,
      ...
      ON CONFLICT (task_id) DO UPDATE SET
        ...
        title=excluded.title,description=excluded.description,scheduled_at=excluded.scheduled_at,
...
BEGIN
  IF (SELECT count(*) FROM public.reminders) <>
     (SELECT count(*) FROM public.care_tasks WHERE task_type='SCHEDULED_REMINDER') THEN
    RAISE EXCEPTION 'WAVE6_RECONCILIATION: scheduled care items';
  END IF;
  IF to_regclass('public.legacy_care_tasks') IS NOT NULL AND
     (SELECT count(*) FROM public.legacy_care_tasks) <>
     (SELECT count(*) FROM public.care_tasks WHERE task_type='MANUAL_TASK') THEN
  ...
  END IF;
  IF (to_regclass('public.legacy_care_tasks') IS NOT NULL AND EXISTS (SELECT 1 FROM public.legacy_care_tasks t LEFT JOIN public.care_groups g
...
BEGIN
  FOR m IN SELECT * FROM (VALUES
    ('reminders','care_tasks','reminder_id','task_id'),
    ('legacy_care_tasks','care_tasks','care_task_id','task_id'),
...
CREATE INDEX IF NOT EXISTS care_tasks_context_ix
  ON public.care_tasks(owner_user_id,journey_id,baby_id,status,scheduled_at) WHERE owner_user_id IS NOT NULL;
...
DROP TABLE IF EXISTS public.legacy_care_tasks;
DROP TABLE IF EXISTS public.reminders;

DO $wave6_absence_gate$
DECLARE name text;
BEGIN
  FOREACH name IN ARRAY ARRAY['reminders','legacy_care_tasks','user_checklist_items',
