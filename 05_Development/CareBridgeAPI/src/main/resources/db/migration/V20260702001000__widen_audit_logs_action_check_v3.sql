-- Widen audit_logs_action_check to add CONTENT_DECIDED (UC-108), added to the Java AuditAction
-- enum. Proactive fix (learned from the UC-106/UC-107 drift where new AuditAction values were
-- added to Java without updating this CHECK constraint) — done in the SAME migration as the
-- enum change this time, not as a follow-up fix.
