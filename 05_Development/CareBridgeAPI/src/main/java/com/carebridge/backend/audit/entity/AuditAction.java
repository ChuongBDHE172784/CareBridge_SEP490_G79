package com.carebridge.backend.audit.entity;

public enum AuditAction {
    LOGIN,
    FEDERATED_LOGIN,
    FEDERATED_REGISTRATION,
    FEDERATED_IDENTITY_LINKED,
    LOGOUT,
    OTP_SENT,
    OTP_VERIFIED,
    OTP_RESENT,
    CONSENT_GRANTED,
    CONSENT_REVOKED,
    CREATE_HEALTH_RECORD,
    VIEW_HEALTH_RECORD,
    EXPERT_VERIFICATION,
    MODERATION_ACTION,
    AI_TRIAGE,
    PAYMENT,
    SECURITY_EVENT,
    VIEW_AUDIT_LOG,
    USER_REGISTRATION_COMPLETED,
    SESSION_REVOKED,
    CONTENT_CREATED,
    CONTENT_UPDATED,
    CONTENT_HIDDEN,
    // Retained on purpose. The Partner programme was retired and nothing writes
    // these any more, but audit_events rows from before the retirement still hold
    // these values and must keep deserializing and reporting. Removing them is a
    // separate migration, gated on the distinct audit value count reaching 0.
    PARTNER_PROFILE_CREATED,
    PARTNER_PROFILE_UPDATED,
    PARTNER_SERVICE_SUBMITTED,
    PARTNER_CAMPAIGN_SUBMITTED,
    PARTNER_PROFILE_DECISION,
    PARTNER_CONTENT_DECISION,
    PARTNER_CONTENT_REMOVED,
    CONTENT_CATEGORY_MANAGED,
    CONTENT_UNPUBLISHED,
    COMMUNITY_QUESTION_CREATED,
    COMMUNITY_QUESTION_EDITED,
    COMMUNITY_ANSWER_POSTED,
    COMMUNITY_ANSWER_LIKED,
    COMMUNITY_BOOKMARK_TOGGLED,
    COMMUNITY_ANSWER_UNLIKED,
    MODERATION_QUEUE_VIEWED,
    PASSWORD_RESET_REQUESTED,
    PASSWORD_RESET_COMPLETED,
    PASSWORD_CHANGED,
    PROFILE_UPDATED,
    PROFILE_VIEWED,
    PRIVACY_SETTINGS_ACCESSED,
    PRIVACY_SETTINGS_UPDATED,
    NOTIFICATION_SENT,
    NOTIFICATION_FAILED,
    SECURITY_INCIDENT_INVESTIGATED,
    SECURITY_EVENT_REVIEWED,
    SECURITY_NOTE_ADDED,
    JOURNEY_CREATED,
    JOURNEY_UPDATED,
    PREGNANCY_OUTCOME_RECORDED,
    MOTHER_BASELINE_SUBMITTED,
    BABY_PROFILE_CREATED,
    BABY_PROFILE_UPDATED,
    BABY_PROFILE_ARCHIVED,
    BABY_ACTIVE_PROFILE_SWITCHED,
    BABY_JOURNEY_LINK_ACCEPTED,
    BABY_JOURNEY_LINK_REJECTED,
    HEALTH_RECORD_ADDED,
    HEALTH_RECORD_UPDATED,
    HEALTH_RECORD_ARCHIVED,
    HEALTH_METRIC_ADDED,
    HEALTH_METRIC_UPDATED,
    METRIC_ADDED,
    METRIC_UPDATED,
    POSTPARTUM_LOG_ADDED,
    POSTPARTUM_LOG_UPDATED,
    POSTPARTUM_LOG_DELETED,
    HEALTH_METRIC_DELETED,
    REMINDER_CREATED,
    REMINDER_COMPLETED,
    REMINDER_SKIPPED,
    REMINDER_CANCELLED,
    CARE_GROUP_CREATED,
    CARE_GROUP_DELETED,
    CARE_GROUP_MEMBER_INVITED,
    CARE_GROUP_INVITE_ACCEPTED,
    CARE_GROUP_INVITE_DECLINED,
    CARE_GROUP_INVITE_REVOKED,
    CARE_GROUP_MEMBER_REMOVED,
    CARE_GROUP_MEMBER_LEFT,
    CARE_GROUP_PERMISSION_UPDATED,
    CARE_GROUP_CONTEXT_RELINKED,
    FILE_UPLOADED,
    FILE_VIEWED,
    FILE_DELETED,
    FILE_ORPHAN_PURGED,
    CONTENT_DELETED,
    NOTIFICATION_PREFERENCES_UPDATED,
    NOTIFICATION_PREFERENCES_VIEWED,
    NOTIFICATIONS_READ,
    COMMUNITY_QUESTION_DELETED,
    COMMUNITY_ANSWER_EDITED,
    COMMUNITY_ANSWER_DELETED,
    CONTENT_DECIDED,
    RED_FLAG_RULE_CREATED,
    RED_FLAG_RULE_UPDATED,
    RED_FLAG_RULE_DELETED,
    COMMUNITY_QUESTION_LIKED,
    COMMUNITY_QUESTION_UNLIKED,
    USER_ACCOUNT_STATUS_CHANGED,
    ACCOUNT_LOCK_APPEAL_SUBMITTED,
    ACCOUNT_LOCK_APPEAL_REVIEWED,
    STAFF_ACCOUNT_CREATED,
    ROLE_PERMISSION_UPDATED,
    POSTURE_CONFIG_CREATED,
    POSTURE_CONFIG_UPDATED,
    POSTURE_CONFIG_ACTIVATED,
    EXERCISE_CREATED,
    EXERCISE_UPDATED,
    EXERCISE_ACTIVATED,
    EXERCISE_DISABLED,
    CONTENT_REPORTED,
    COMMUNITY_PROFILE_CREATED,
    COMMUNITY_PROFILE_UPDATED,
    BABY_LOG_ADDED,
    BABY_LOG_UPDATED,
    BABY_LOG_DELETED,
    BABY_DAILY_LOG_DELETED,
    MILESTONE_RECORDED,
    MILESTONE_UPDATED,
    MILESTONE_DELETED,
    HEALTH_SUMMARY_GENERATED,
    HEALTH_SUMMARY_SHARED,
    VACCINATION_RECORD_ADDED,
    VACCINATION_RECORD_UPDATED,
    VACCINATION_RECORD_DELETED,
    VACCINATION_COMPLETED,
    VACCINATION_POSTPONED,
    VACCINATION_BOOK_INITIALIZED,
    VACCINATION_BOOK_REALIGNED,
    GROWTH_MEASUREMENT_ADDED,
    GROWTH_MEASUREMENT_UPDATED,
    GROWTH_MEASUREMENT_DELETED,
    EXPENSE_CREATED,
    EXPENSE_UPDATED,
    EXPENSE_DELETED,
    CHECKLIST_ITEM_ADDED,
    CHECKLIST_ITEM_COMPLETED,
    CHECKLIST_ITEM_UPDATED,
    CHECKLIST_ITEM_DELETED,
    CARE_TASK_ASSIGNED,
    CARE_TASK_UPDATED,
    CARE_TASK_CANCELLED,
    CARE_GROUP_INVITATION_ACCEPTED,
    CARE_TASK_STATUS_UPDATED,
    FAMILY_ALERT_VIEWED,
    DIRECT_CONVERSATION_OPENED,
    DIRECT_MESSAGE_SENT,
    DIRECT_CHAT_ACCESS_DENIED,
    DIRECT_CALL_INITIATED,
    DIRECT_CALL_STATE_CHANGED,
    DIRECT_CALL_ACCESS_DENIED,
    DIRECT_CALL_MISSED_BY_TIMEOUT,
    FIREBASE_CUSTOM_TOKEN_ISSUED,
    CHECKLIST_TEMPLATE_CREATED,
    CHECKLIST_TEMPLATE_UPDATED,
    CHECKLIST_TEMPLATE_ARCHIVED,
    CHECKLIST_TEMPLATE_DECIDED,
    CHECKLIST_DISTRIBUTED,
    CHECKLIST_ASSIGNED,
    CHECKLIST_COMPLETED,
    CHECKLIST_SKIPPED,
    CHECKLIST_REOPENED,
    CHECKLIST_CANCELLED,
    CHECKLIST_RECONCILIATION_FAILED,
    CHECKLIST_MIGRATION_QUARANTINED,
    CHECKLIST_ACCESS_BASELINE,
    CHECKLIST_ACCESS_REVOKED,
    CHECKLIST_QUARANTINE_VIEWED,
    CHECKLIST_QUARANTINE_RESOLVED,
    CHECKLIST_RETENTION_PURGED,
    SAFETY_MONITORING_ENABLED,
    SAFETY_MONITORING_DISABLED,
    SAFETY_EVENT_RECORDED,
    SAFETY_EVENT_RESPONDED,
    SAFETY_EVENT_ESCALATED,
    EMERGENCY_ALERT_DELIVERY,

    // CB-MOD-IMP-016: AI-assisted content moderation
    AI_POLICY_CREATED,
    AI_POLICY_UPDATED,
    AI_POLICY_STATUS_CHANGED,
    AI_POLICY_TEST_RUN,
    AI_SCAN_COMPLETED,
    AI_SCAN_FAILED,
    AI_CASE_CREATED,
    AI_FEEDBACK_SUBMITTED,
    AI_RESCAN_REQUESTED,
    REPORT_CLAIMED,
    REPORT_RELEASED,
    SYSTEM_CONFIGURATION_UPDATED,

    // Categories already present in audit_events but absent from this enum, which made
    // every JPA read covering them fail with "No enum constant". audit_events rejects all
    // UPDATE and DELETE (carebridge_reject_mutation) apart from a narrow seven-year
    // retention path for CHECKLIST_* rows, so the rows cannot be rewritten — an append-only
    // log obliges this enum to remain able to read everything ever written to it.
    // Measured on the live database: 60 rows across these eight categories.
    AUTHENTICATION,
    BASELINE_CONTEXT,
    MODERATION_HIDE,
    MODERATION_LOCK,
    MOTHER_JOURNEY_TRANSITION,
    PREGNANCY_OUTCOME_EVIDENCE,
    SAFETY_OUTCOME,
    SECURITY
}
