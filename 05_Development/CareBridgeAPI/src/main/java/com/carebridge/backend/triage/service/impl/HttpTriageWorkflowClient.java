...
@Component
public class HttpTriageWorkflowClient implements TriageWorkflowClient {
    ...
    public HttpTriageWorkflowClient(
    ...
    public WorkflowResult executeTurn(Map<String, Object> request) {
        if (internalKey == null || internalKey.isBlank()) {
            throw new IllegalStateException("Triage internal client is disabled");
        }
        try {
            HttpRequest httpRequest = HttpRequest.newBuilder()
                    .uri(URI.create(baseUrl + "/internal/triage/turn"))
                    .timeout(timeout)
            ...
            if (response.statusCode() < 200 || response.statusCode() >= 300) {
                throw new IllegalStateException("Triage workflow returned HTTP " + response.statusCode());
            }
            ...
            throw new IllegalStateException("Triage workflow unavailable", failure);
    ...
    private static Map<String, Object> map(Object value) {
        if (!(value instanceof Map<?, ?>)) throw new IllegalStateException("Invalid triage response");
        return (Map<String, Object>) value;
    }

    private static String text(Object value) {
        if (!(value instanceof String text) || text.isBlank()) {
            throw new IllegalStateException("Invalid triage response");
        }
        return text;
    }
}
