...
public class PediatricRiskRules {
    ...
    public List<String> questions(RunIntakeRequest request) {
        ...
        boolean hasContext = (request.getSymptoms() != null && !request.getSymptoms().isBlank())
                || (request.getParentFreeText() != null && !request.getParentFreeText().isBlank())
                || (request.getSymptomList() != null && !request.getSymptomList().isEmpty());
        if (request.getChildAgeMonths() == null) {
            questions.add("Trẻ hiện bao nhiêu tháng tuổi?");
        }
        if (!hasContext && request.getBreathingStatus() == null) {
            questions.add("Trẻ thở bình thường hay có khó thở/thở nhanh/tím tái?");
        }
        if (!hasContext && request.getConsciousnessStatus() == null) {
            questions.add("Trẻ tỉnh táo bình thường hay lơ mơ/li bì/khó đánh thức?");
        }
        if (!hasContext && request.getFeedingStatus() == null && questions.size() < 3) {
    ...
    public RuleOutcome apply(RunIntakeRequest request, List<String> symptoms) {
        ...
        addIf(symptoms.contains("breathing_difficulty") || symptoms.contains("cyanosis"), matchedRules, redFlags,
        ...
        addIf(symptoms.contains("lethargy"), matchedRules, redFlags,
                "RED_LETHARGY", "Lơ mơ, li bì hoặc khó đánh thức");
        addIf(symptoms.contains("poor_feeding"), matchedRules, redFlags,
                "RED_POOR_FEEDING", "Bỏ bú hoặc không uống được");
        addIf(symptoms.contains("dehydration") && symptoms.contains("diarrhea"), matchedRules, redFlags,
                "RED_DIARRHEA_DEHYDRATION", "Tiêu chảy kèm dấu hiệu mất nước");
        addIf(symptoms.contains("dehydration") && !symptoms.contains("diarrhea"), matchedRules, redFlags,
                "RED_SEVERE_DEHYDRATION", "Dấu hiệu mất nước nặng");
        addIf(symptoms.contains("vomiting") && containsAny(request.getVomiting(), "lien tuc", "nhieu"), matchedRules, redFlags,
