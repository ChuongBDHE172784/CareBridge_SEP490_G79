...
class ConsultationRequestNotificationWriter {
    ...
    boolean insertIfAbsent(NotificationRecord candidate) {
        String eventType = candidate.getMetadata() == null
                ? null
                : candidate.getMetadata().get("eventType");
        if (eventType == null || eventType.isBlank()) {
            throw new IllegalArgumentException("Consultation notification eventType is required");
        }
        candidate.setId(idempotencyId(candidate.getUserId(), candidate.getReferenceId(), eventType));
    ...
    private static UUID idempotencyId(UUID userId, UUID requestId, String eventType) {
        String key = "CONSULTATION|CONSULTATION_REQUEST|" + userId + '|' + requestId + '|' + eventType;
    ...
    UUID claim(UUID id) {
        ...
        int claimed = jdbcTemplate.update("""
                UPDATE notification_records
                   SET status = 'PROCESSING', processing_started_at = now(), claim_token = ?
                 WHERE id = ?
                   AND (status = 'PENDING'
    ...
    @Transactional(propagation = Propagation.REQUIRES_NEW)
    boolean complete(NotificationRecord record, UUID claimToken) {
        return jdbcTemplate.update("""
                UPDATE notification_records
