...
@Service @RequiredArgsConstructor public class SponsoredCampaignServiceImpl implements SponsoredCampaignService {
 ...
 @Override @Transactional(readOnly=true) public Page<SponsoredCampaignListItemResponse> getOwnCampaigns(UUID actorId,Pageable pageable){
  UUID organizationId=organizationRepository.findByRepresentativeUserId(actorId).orElseThrow(PartnerException::campaignOrganizationNotFound).getId();
  return campaignRepository.findByPartnerIdAndRemovedFalse(organizationId,pageable).map(mapper::toListItem);
 }
}
