    ...
    public DeliveryAction insertIntent(
        ...
        Optional<DeliveryAction> inserted = queryOne("""
                INSERT INTO safety_event_actions (
                    ...
                    owner_user_id, recipient_user_id, device_token_id,
                    device_identifier, notification_record_id, attempt_number,
                    idempotency_key, delivery_status, created_at,
                    action_phase, alert_generation, fence_token
                )
                SELECT gen_random_uuid(), event.safety_event_id, 'DELIVERY',
                       ...
                       'delivery:' || event.safety_event_id::text || ':'
                           ...
                           || CAST(? AS text) || ':intent',
                       'PENDING', now(), 'INTENT', event.alert_generation,
                ...
                RETURNING safety_event_action_id, notification_record_id, attempt_number
                """, recipientUserId, deviceTokenId, deviceTokenId,
    ...
    public boolean appendResult(
        ...
        return jdbcTemplate.update("""
                INSERT INTO safety_event_actions (
                    ...
                    created_at, delivered_at, action_phase, alert_generation,
                    fence_token, related_action_id
                )
                SELECT gen_random_uuid(), intent.safety_event_id, 'DELIVERY',
                       intent.owner_user_id, intent.recipient_user_id,
                       intent.device_token_id, intent.device_identifier,
                       intent.notification_record_id, greatest(?, 1),
                       'delivery:' || intent.safety_event_id::text || ':'
                           || event.alert_generation::text || ':'
                           || intent.device_token_id::text || ':result',
                       ?, ?, ?, now(), CASE WHEN ? THEN now() ELSE NULL END,
                       'RESULT', event.alert_generation, event.alert_claim_token,
                       intent.safety_event_action_id
                  FROM safety_event_actions intent
                  JOIN safety_events event
                    ON event.safety_event_id = intent.safety_event_id
                 WHERE intent.safety_event_action_id = ?
    ...
    private Optional<DeliveryAction> queryOne(String sql, Object... parameters) {
        return jdbcTemplate.query(sql, resultSet -> resultSet.next()
                        ? Optional.of(new DeliveryAction(
                                resultSet.getObject("safety_event_action_id", UUID.class),
