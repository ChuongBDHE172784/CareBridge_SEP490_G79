...
public class TriageMetrics {
    private static final Logger log = LoggerFactory.getLogger(TriageMetrics.class);
    ...
    public void recordTurn(String outcome, long latencyMs, int questionCount, boolean targetConflict) {
        ...
        log.info("triage_turn outcome={} total={}", safeOutcome, total);
    }

    public void recordFailure(Failure failure) {
        long count = failures.get(failure).incrementAndGet();
        log.warn("triage_failure category={} total={}", failure, count);
    }
