...
export default function ExpertVerificationQueuePage() {
	...
	const [_viewFileUrl, _setViewFileUrl] = useState<string | null>(null);
							...
							<div className="flex-1 p-5 space-y-4 overflow-y-auto">
								...
								<div className="rounded-2xl border border-outline-variant/60 bg-surface p-4 space-y-3">
									...
									{selected.experienceYears != null && (
										<p className="text-sm"><strong>Kinh nghiệm:</strong> {selected.experienceYears} năm</p>
									)}
										...
										{(() => {
											...
											if (isImage) {
												return (
													<img
														...
														alt="Tài liệu chứng chỉ"
														className="max-w-full max-h-[60vh] object-contain rounded-lg border border-gray-200 cursor-pointer"
														onClick={() => _setViewFileUrl(selected.fileUrl)}
													/>
												);
											}
											if (isPdf) {
											...
											return (
												<a
													...
													className="inline-block mt-2 text-blue-600 text-sm font-semibold underline hover:text-blue-800"
												>
