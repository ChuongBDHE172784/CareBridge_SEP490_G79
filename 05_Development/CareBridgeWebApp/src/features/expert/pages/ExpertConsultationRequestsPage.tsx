...
export default function ExpertConsultationRequestsPage() {
  ...
  const fetchRequests = useCallback(async (p: number) => {
    try {
      setLoading(true);
      const { data } = await apiClient.get(`/api/v1/consultation-requests/assigned?size=20&page=${p}`);
  ...
  const handleAction = async (id: string, action: 'accept' | 'reject') => {
    try {
      if (action === 'accept') {
        ...
        await apiClient.post(`/api/v1/consultation-requests/${id}/reject`, { reason: 'Chuyên gia bận' });
      }
      // Refresh list
      fetchRequests(0);
    } catch (e: unknown) {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="mb-6">
        ...
        <p className="text-gray-500 mt-1">Quản lý các yêu cầu tư vấn từ mẹ bầu</p>
      </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
        ...
        {requests.length === 0 && !loading && !error && (
          <div className="p-12 text-center">
            ...
            <p className="mt-4 text-gray-500 font-medium">Chưa có yêu cầu tư vấn nào</p>
          </div>
        )}

        <div className="divide-y divide-gray-100">
          {requests.map((req) => (
            <div key={req.id} className="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-gray-50">
              <div>
                <h3 className="font-semibold text-gray-900">{req.counterpartDisplayName || 'Người dùng ẩn danh'}</h3>
                <p className="text-sm text-gray-600 mt-1">Chủ đề: {req.topic}</p>
                <div className="flex items-center gap-3 mt-2 text-xs">
                  <span className={`px-2 py-1 rounded-full font-medium ${STATUS_MAP[req.status]?.cls || 'bg-gray-100'}`}>
                    {STATUS_MAP[req.status]?.label || req.status}
                  </span>
                  <span className="text-gray-400">•</span>
                  <span className="text-gray-500">{timeAgo(req.createdAt)}</span>
                </div>
              </div>
