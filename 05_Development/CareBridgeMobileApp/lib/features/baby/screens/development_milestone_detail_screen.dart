...
class _DevelopmentMilestoneDetailScreenState
  ...
  Future<void> _fetchMilestoneDetail() async {
    ...
    try {
      final milestone =
          widget.initialMilestone ??
          await _service.getMilestone(widget.babyId, widget.milestoneId);
      final achieved = milestone.achievedDate;
      _milestoneDetail = {
        'title': milestone.milestoneType.displayLabel,
        'category': milestone.sourceType ?? 'MOTHER_INPUT',
        'description': milestone.note ?? 'Cột mốc đã được ghi nhận.',
        'dateAchieved':
            '${achieved.day.toString().padLeft(2, '0')}/'
            '${achieved.month.toString().padLeft(2, '0')}/${achieved.year}',
        'ageAtMilestone': 'Theo hồ sơ của bé',
        'motherNote': milestone.note,
        'imageUrl': null,
        'mediaCount': 0,
      };
    } catch (e) {
  ...
  Future<void> _editMilestoneNote() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Đã cập nhật cột mốc')));
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(
        context,
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Stack(
                          children: [
                            if (_milestoneDetail!['imageUrl'] != null)
                              Image.network(
                            ...
                            if ((_milestoneDetail!['mediaCount'] as int) > 0)
                              Positioned(
                                bottom: 16,
                                right: 16,
                                child: Container(
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 12),
                        Text(
                          _milestoneDetail!['motherNote'] ?? 'Chưa có ghi chú.',
                          style: const TextStyle(
                            fontSize: 16,
                  ...
                  TextButton.icon(
                    ...
                    style: TextButton.styleFrom(
                      ...
                      shape: RoundedRectangleBorder(
                        borderRadius: BorderRadius.circular(24),
                        side: BorderSide(color: errorColor.withAlpha(76)),
                      ),
