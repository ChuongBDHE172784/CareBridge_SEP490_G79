...
class _EditBabyDailyLogScreenState extends State<EditBabyDailyLogScreen>
  ...
  void _prefillFromLog() {
    ...
    if (log.quantity != null) _quantityCtrl.text = log.quantity!.toStringAsFixed(0);
  ...
  String _buildNoteFromFields() {
    switch (_selectedType) {
      case LogType.sleep:
        final parts = <String>[];
        if (_sleepDurationCtrl.text.isNotEmpty) parts.add('Thời gian: ${_sleepDurationCtrl.text} phút');
      ...
      case LogType.symptom:
        final parts = <String>[];
        if (_temperatureCtrl.text.isNotEmpty) parts.add('Nhiệt độ: ${_temperatureCtrl.text}°C');
        if (_symptomsCtrl.text.isNotEmpty) parts.add('Triệu chứng: ${_symptomsCtrl.text}');
        if (_noteCtrl.text.isNotEmpty) parts.add(_noteCtrl.text);
        return parts.join(' | ');
      default:
  ...
  Widget _buildTabBar() {
    return Container(
      ...
      child: TabBar(
        ...
        labelStyle: const TextStyle(fontFamily: 'Lexend', fontSize: 11, fontWeight: FontWeight.w600),
        unselectedLabelStyle: const TextStyle(fontFamily: 'Lexend', fontSize: 11),
        labelColor: _primary,
  ...
  Widget _buildDateTimeSection() {
    return Container(
      ...
      child: Column(
        ...
        children: [
          const Text('Thời gian', style: TextStyle(fontFamily: 'Lexend', fontSize: 13, fontWeight: FontWeight.w600, color: _onSurface)),
          const SizedBox(height: 12),
          Row(
            children: [
              Expanded(child: _DateTimeChip(label: 'Bắt đầu', dateTime: _startedAt, onTap: () => _pickDateTime(isStart: true))),
              const SizedBox(width: 12),
              Expanded(child: _DateTimeChip(label: 'Kết thúc', dateTime: _endedAt, onTap: () => _pickDateTime(isStart: false))),
            ],
          ),
        ],
      ),
    ...
    switch (_selectedType) {
      case LogType.feeding:
        return Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Lượng bú / ăn', style: TextStyle(fontFamily: 'Lexend', fontSize: 13, fontWeight: FontWeight.w600, color: _onSurface)),
            const SizedBox(height: 12),
            Row(
              children: [
                Expanded(
      ...
      case LogType.sleep:
        return Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Giấc ngủ', style: TextStyle(fontFamily: 'Lexend', fontSize: 13, fontWeight: FontWeight.w600, color: _onSurface)),
            const SizedBox(height: 12),
            TextFormField(
            ...
            const SizedBox(height: 12),
            const Text('Chất lượng', style: TextStyle(fontFamily: 'Lexend', fontSize: 12, color: _onSurfaceVariant)),
      ...
      case LogType.diaper:
        return Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Trạng thái tã', style: TextStyle(fontFamily: 'Lexend', fontSize: 13, fontWeight: FontWeight.w600, color: _onSurface)),
            const SizedBox(height: 12),
            Wrap(
              spacing: 8,
              runSpacing: 8,
      ...
      case LogType.symptom:
        return Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Chỉ số sức khỏe', style: TextStyle(fontFamily: 'Lexend', fontSize: 13, fontWeight: FontWeight.w600, color: _onSurface)),
            const SizedBox(height: 12),
            TextFormField(
            ...
            TextFormField(
              ...
              decoration: _inputDeco('Triệu chứng', hint: 'Sốt nhẹ, ho, sổ mũi...'),
            ),
          ],
        );
    }
      ...
      children: [
        ElevatedButton(
          ...
          child: _isSaving
              ...
              : const Text('Lưu thay đổi', style: TextStyle(fontFamily: 'Lexend', fontSize: 16, fontWeight: FontWeight.w700)),
        ),
        const SizedBox(height: 12),
        OutlinedButton.icon(
          onPressed: _confirmDelete,
          icon: const Icon(Icons.delete_outline_rounded, size: 18),
          label: const Text('Xóa nhật ký', style: TextStyle(fontFamily: 'Lexend', fontSize: 15, fontWeight: FontWeight.w600)),
          ...
          child: Container(
            ...
            child: const Column(
              ...
              children: [
                ...
                SizedBox(height: 12),
                Text('Đã lưu thành công!', style: TextStyle(fontFamily: 'Lexend', fontSize: 15, fontWeight: FontWeight.w600, color: _onSurface)),
              ],
            ),
          ),
        ),
...
class _DateTimeChip extends StatelessWidget {
  const _DateTimeChip({required this.label, required this.dateTime, required this.onTap});
        ...
        child: Column(
          ...
          children: [
            ...
            Container(
              width: 64,
              height: 64,
              decoration: BoxDecoration(color: const Color(0xFFFFEDEA), borderRadius: BorderRadius.circular(20)),
            ...
            ElevatedButton(
              ...
              child: const Text('Xác nhận xóa', style: TextStyle(fontFamily: 'Lexend', fontSize: 15, fontWeight: FontWeight.w700)),
            ),
            const SizedBox(height: 8),
            TextButton(
              onPressed: () => Navigator.of(context).pop(),
              child: const Text('Hủy bỏ', style: TextStyle(fontFamily: 'Lexend', fontSize: 14, color: _primary)),
            ),
          ],
        ),
      ),
