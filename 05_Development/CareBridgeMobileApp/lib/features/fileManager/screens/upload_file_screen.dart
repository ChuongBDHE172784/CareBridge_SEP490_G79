  ...
  void _pickFile() {
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        title: const Text('Chọn tệp', style: TextStyle(fontFamily: 'Lexend', fontWeight: FontWeight.w600)),
        content: const Text('Chức năng chọn tệp cần package "file_picker".\nSẽ tích hợp ở bước tiếp theo.',
            style: TextStyle(fontFamily: 'Lexend')),
        actions: [
          FilledButton(
            onPressed: () => Navigator.pop(context),
            style: FilledButton.styleFrom(backgroundColor: _primaryContainer),
            child: const Text('Đã hiểu', style: TextStyle(fontFamily: 'Lexend')),
          ),
        ],
      ),
    );
  ...
  Widget _buildDropZone() {
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Column(
          children: [
            Icon(Icons.cloud_upload_outlined, size: 52, color: _primaryContainer.withAlpha(204)),
            const SizedBox(height: 12),
            const Text('Nhấn để chọn tệp',
                style: TextStyle(fontFamily: 'Lexend', fontSize: 18, fontWeight: FontWeight.w600, color: Color(0xFF271812))),
            const SizedBox(height: 4),
            Text('Hỗ trợ: PDF, JPG, PNG, MP4 (tối đa 10 MB)',
                style: TextStyle(fontFamily: 'Lexend', fontSize: 12, color: _onSurfaceVariant.withAlpha(179))),
          ],
        ),
      ),
    );
  ...
  Widget _buildFilePreview() {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: const Color(0xFF5A463F).withAlpha(13), blurRadius: 8, offset: const Offset(0, 3))],
      ),
      child: Row(
        children: [
          const Icon(Icons.insert_drive_file, color: Color(0xFFC98C7B), size: 32),
          const SizedBox(width: 12),
          Expanded(
            child: Text(_pickedFile!.path.split(Platform.pathSeparator).last,
                style: const TextStyle(fontFamily: 'Lexend', fontSize: 14, color: Color(0xFF271812)),
      ...
      children: [
        ...
        TextField(
          ...
          decoration: InputDecoration(
            ...
            hintStyle: const TextStyle(fontFamily: 'Lexend', color: Color(0xFF9C857C)),
            filled: true,
            fillColor: Colors.white,
            border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: _outlineVariant.withAlpha(128))),
            enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: _outlineVariant.withAlpha(77))),
            focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: Color(0xFFC98C7B))),
          ),
        ),
        ...
        DropdownButtonFormField<FileCategory>(
          ...
          style: const TextStyle(fontFamily: 'Lexend', color: Color(0xFF271812)),
          items: FileCategory.values.map((c) => DropdownMenuItem(
            value: c,
            child: Text(c.displayLabel, style: const TextStyle(fontFamily: 'Lexend')),
          )).toList(),
              ...
              child: GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(color: selected ? const Color(0xFFC98C7B) : const Color(0xFFD6C2BD)),
                  ),
        ...
        Container(
          ...
          child: Row(
            children: [
              Icon(
                _visibility == FileVisibility.private ? Icons.lock_outline : Icons.share,
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      _visibility == FileVisibility.private ? 'Riêng tư' : 'Chia sẻ với nhóm',
                      style: const TextStyle(fontFamily: 'Lexend', fontSize: 16, fontWeight: FontWeight.w600, color: Color(0xFF271812)),
                    ),
                    Text(
                      _visibility == FileVisibility.private ? 'Chỉ mình bạn xem được' : 'Nhóm chăm sóc có thể xem',
              ...
              Switch(
                ...
                onChanged: (v) => setState(() => _visibility = v ? FileVisibility.shared : FileVisibility.private),
              ),
            ],
          ),
        ),
    ...
    return SizedBox(
      ...
      child: FilledButton.icon(
        ...
        label: Text(
          ...
          style: const TextStyle(fontFamily: 'Lexend', fontSize: 20, fontWeight: FontWeight.w600),
        ),
      ),
    );
  }
