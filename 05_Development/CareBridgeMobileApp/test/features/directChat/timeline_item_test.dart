...
void main() {
  ...
  group('mergeTimelineItems', () {
    test('reconciles an optimistic entry with the server-confirmed message, no duplicate', () {
    ...
    test('does not duplicate the same server message fetched twice (idempotent merge)', () {
    ...
    test('keeps a failed optimistic entry distinct from unrelated confirmed messages', () {
      ...
      expect(merged.any((i) => i.sendStatus == ChatSendStatus.failed), isTrue);
    });

    test('sorts merged items by sortTimestamp regardless of MESSAGE/CALL_EVENT kind', () {
      ...
      final merged = mergeTimelineItems(const [], [laterMessage, call, earlierMessage]);
