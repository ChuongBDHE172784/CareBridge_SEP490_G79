...
//        byte[] tail = new byte[] { 0x2 }; // reason
    ...
    protected void handleHeartrate(byte[] value) {
        if (value.length == 2 && value[0] == 0) {
            ...
            // CareBridge: broadcast realtime heart rate to Health Test app
            nodomain.freeyourgadget.gadgetbridge.carebridge.CareBridgeBroadcastSender
                    .sendHeartRate(getContext(), hrValue, getDevice().getName());
        }
    }
