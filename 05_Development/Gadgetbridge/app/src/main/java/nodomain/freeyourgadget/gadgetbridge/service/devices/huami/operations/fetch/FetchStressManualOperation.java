...
import nodomain.freeyourgadget.gadgetbridge.carebridge.CareBridgeBroadcastSender;
...
public class FetchStressManualOperation extends AbstractRepeatingFetchOperation {
    ...
    protected boolean handleActivityData(final GregorianCalendar timestamp, final byte[] bytes) {
        ...
        if (!samples.isEmpty()) {
            // CareBridge: broadcast the most recent manual stress value to Health Test app
            // Stress scale: 0-39=Relaxed, 40-59=Mild, 60-79=Moderate, 80-100=High
            final HuamiStressSample latest = samples.get(samples.size() - 1);
            CareBridgeBroadcastSender.sendStress(
                    getContext(),
                    latest.getStress(),
                    getDevice().getName(),
                    latest.getTimestamp()
