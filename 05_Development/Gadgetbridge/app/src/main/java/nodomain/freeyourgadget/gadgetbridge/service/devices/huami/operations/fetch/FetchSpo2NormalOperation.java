...
public class FetchSpo2NormalOperation extends AbstractRepeatingFetchOperation {
    ...
    protected boolean handleActivityData(final GregorianCalendar timestamp, final byte[] bytes) {
        ...
        if (!samples.isEmpty()) {
            // CareBridge: broadcast the most recent SpO2 value to Health Test app
            final HuamiSpo2Sample latest = samples.get(samples.size() - 1);
            CareBridgeBroadcastSender.sendSpo2(
                    getContext(),
                    latest.getSpo2() & 0xff,
                    getDevice().getName()
            );
