# CareBridge - MF-01 Integration Tests - Supabase Sample Accounts — Hướng dẫn nhập request trực tiếp trong Postman
...
- Không chạy test Register và OTP kích hoạt đăng ký; sample account đã ở trạng thái ACTIVE.
- `{{samplePassword}}`: lấy giá trị `CAREBRIDGE_DEV_SEED_PASSWORD` từ `05_Development/CareBridgeAPI/.env`; không chụp hoặc commit giá trị này.
- Sample đã xác minh trên Supabase: `mother@carebridge.dev`, `family@carebridge.dev`, `expert@carebridge.dev`, `moderator@carebridge.dev`, `content@carebridge.dev`, `admin@carebridge.dev`.
- Các request đổi password, deactivate, đổi role/status hoặc revoke session của tài khoản chung được đánh dấu `CAUTION`; không Send nếu chưa có phương án restore.
- Không có sample phone được xác minh; các case login bằng phone được đánh dấu riêng và không nên Send khi `{{samplePhone}}` còn trống.
...
## 00 - Login Supabase Sample Accounts

### Login sample MOTHER - mother@carebridge.dev
...
### Login sample FAMILY - family@carebridge.dev
...
### Login sample EXPERT - expert@carebridge.dev
...
### Login sample MODERATOR - moderator@carebridge.dev
...
### Login sample CONTENT_ADMIN - content@carebridge.dev
...
### Login sample SYSTEM_ADMIN - admin@carebridge.dev
...
### [NEEDS VERIFIED SAMPLE PHONE] MF01-VALID-011 - Từ chối login khi gửi cả phone và email.
...
### [NEEDS VERIFIED SAMPLE PHONE] MF01-AUTH-002 - Login bằng phone và password hợp lệ.
...
### [CAUTION - SHARED SAMPLE] MF01-UPDATE-016 - Đổi mật khẩu hợp lệ cho user đang đăng nhập.
...
```json
{
  ...
  "confirmPassword": "{{sampleNewPassword}}"
}
```
...
### [CAUTION - SHARED SAMPLE] MF01-UPDATE-017 - Đặt lại mật khẩu bằng recovery token hợp lệ.
...
### [CAUTION - SHARED SAMPLE] MF01-VALID-013 - Từ chối reset password khi confirm không khớp.
...
### [CAUTION - SHARED SAMPLE] MF01-VALID-014 - Từ chối change password khi current password sai.
...
### [CAUTION - SHARED SAMPLE] MF01-REVOKE-001 - Logout bằng refresh token của session hiện tại.
...
### [CAUTION - SHARED SAMPLE] MF01-REVOKE-002 - Logout không gửi refresh token, dùng JWT sid hiện tại.
...
### [CAUTION - SHARED SAMPLE] MF01-REVOKE-007 - Vô hiệu hóa tài khoản cá nhân bằng password đúng.
...
### [CAUTION - SHARED SAMPLE] MF01-ACCESS-012 - System Admin không thể tự deactivate.
...
### [CAUTION - SHARED SAMPLE] MF01-INT-011 - Deactivation thu hồi đồng thời session, refresh token và FCM token.
...
### [CAUTION - SHARED SAMPLE] MF01-REVOKE-006 - Hủy đăng ký FCM device token của chính user.
...
### [CAUTION - SHARED SAMPLE] MF01-REVOKE-003 - Thu hồi một session khác của chính user.
...
### [CAUTION - SHARED SAMPLE] MF01-REVOKE-004 - Thu hồi tất cả session khác, giữ current session.
...
### [CAUTION - SHARED SAMPLE] MF01-REVOKE-010 - Từ chối revoke chính current session qua session endpoint.
...
### [CAUTION - SHARED SAMPLE] MF01-ACCESS-004 - User không thể revoke session của tài khoản khác.
...
### [CAUTION - SHARED SAMPLE] [NEEDS VERIFIED SAMPLE PHONE] MF01-ADD-008 - System Admin tạo staff account MODERATOR.
...
### [CAUTION - SHARED SAMPLE] MF01-UPDATE-011 - System Admin khóa tài khoản khác với reason hợp lệ.
...
### [CAUTION - SHARED SAMPLE] MF01-UPDATE-012 - System Admin mở khóa tài khoản khác với CSKH ticket.
...
### [CAUTION - SHARED SAMPLE] MF01-UPDATE-013 - System Admin disable tài khoản khác.
...
### [CAUTION - SHARED SAMPLE] MF01-UPDATE-014 - System Admin đổi role giữa các staff governance role.
...
### [CAUTION - SHARED SAMPLE] MF01-VALID-019 - Từ chối status update khi không có enabled/locked.
...
### [CAUTION - SHARED SAMPLE] MF01-VALID-020 - Từ chối lock account khi thiếu reason.
...
### [CAUTION - SHARED SAMPLE] MF01-ACCESS-010 - System Admin không thể đổi trạng thái chính mình.
...
### [CAUTION - SHARED SAMPLE] MF01-ACCESS-011 - System Admin không thể đổi role chính mình.
...
### [CAUTION - SHARED SAMPLE] MF01-INT-010 - Lỗi audit bắt buộc rollback admin status mutation.
...
Hướng dẫn này chứa 119 HTTP request thuộc MF-01. Các trường hợp UI-only hoặc chức năng chỉ có trong Report 3 nhưng chưa có endpoint không được tạo request giả.
